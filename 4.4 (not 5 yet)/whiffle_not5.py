...
# Detection settings (will be adjustable via sliders)
BALL_COLOR_RANGE = {
    "lower_white": [0, 0, 180],
...
# Default HSV values for reset
DEFAULT_BALL_COLOR_RANGE = BALL_COLOR_RANGE.copy()

MIN_CONTOUR_AREA = 30
MIN_RADIUS = 6
RED_MIN_CONTOUR_AREA = 50
RED_MIN_RADIUS = 10
...
if system == "Windows":
    ...
    ALTERNATE_WEBCAM_BACKEND = cv2.CAP_MSMF  # Fallback for Windows
elif system == "Darwin":
...
def load_point_zones(filename=CALIBRATION_FILE):
    try:
        ...
        print(f"Error loading point zones: {e}")
        tk.messagebox.showerror("Error", f"Failed to load calibration file: {e}")
        return [], None
...
def save_point_zones(point_zones, special_hole, filename=CALIBRATION_FILE):
    try:
        ...
        print(f"Error saving point zones: {e}")
        tk.messagebox.showerror("Error", f"Failed to save calibration file: {e}")
    ...
    try:
        ...
        if os.path.exists(HIGH_SCORE_FILE):
            try:
                ...
                print(f"Error loading high score from file: {e}")
        return []
...
def save_high_score(initials="N/A", new_score=None, leaderboard=None):
    ...
    data = {
        "initials": initials if initials.strip() else "N/A",
        "score": new_score,
        "created_at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00", time.gmtime())
    }
    
    try:
        response = requests.post(LEADERBOARD_ENDPOINT, headers=headers, json=data, timeout=5)
        response.raise_for_status()
        print(f"Score uploaded: {initials} - {new_score}")
    except requests.RequestException as e:
        print(f"Offline mode: {e}. Queuing score.")
        offline_file = "offline_scores.json"
        offline_data = {"initials": initials, "score": new_score, "timestamp": time.time()}
        try:
            with open(offline_file, "a") as f:
                json.dump(offline_data, f)
                f.write("\n")
        except Exception as e:
            print(f"Error queuing offline score: {e}")
        threading.Thread(target=retry_upload, args=(initials, new_score), daemon=True).start()
...
def retry_upload(initials, score):
    time.sleep(10)
    headers = {
        "apikey": SUPABASE_API_KEY,
        "Authorization": f"Bearer {SUPABASE_API_KEY}",
    ...
    try:
        ...
        response.raise_for_status()
        print(f"Offline score uploaded: {initials} - {score}")
        with open("offline_scores.json", "r+") as f:
            lines = [json.loads(line) for line in f if line.strip()]
            lines = [l for l in lines if not (l["initials"] == initials and l["score"] == score)]
            f.seek(0)
            f.truncate()
            for line in lines:
                json.dump(line, f)
        ...
        print("Retry failed. Keeping in queue.")
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, 'r') as f:
                config = json.load(f)
                # Update BALL_COLOR_RANGE with saved values if they exist
                if "ball_color_range" in config:
                    global BALL_COLOR_RANGE
                    BALL_COLOR_RANGE = config["ball_color_range"]
                return config
        return {"sound_effects_enabled": True, "tutorial_shown": False}
    except Exception as e:
        print(f"Error loading config: {e}")
        tk.messagebox.showerror("Error", f"Failed to load configuration: {e}")
        return {"sound_effects_enabled": True, "tutorial_shown": False}
...
def save_config(sound_effects_enabled, tutorial_shown):
    try:
        data = {
            ...
            "ball_color_range": BALL_COLOR_RANGE  # Save the adjusted HSV values
        }
...
def set_webcam_resolution(cap):
    resolutions = [(3840, 2160), (1920, 1080), (1280, 720), (640, 480)]  # Add 4K as the first option
...
class OptionsWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        self.root.geometry("600x450")  # Adjusted size to fit sliders
        self.root.configure(bg="#2E2E2E")
        self.on_close_callback = on_close_callback
        ...
        # White Ball HSV Sliders
        ttk.Label(frame, text="White Ball Detection", font=("Helvetica", 12, "bold"), style="Custom.TLabel").pack(pady=5)
        white_frame = ttk.Frame(frame)
        white_frame.pack(pady=5, fill="x")

        # Lower HSV for White
        self.lower_white_hue = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_white"][0])
        self.lower_white_sat = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_white"][1])
        self.lower_white_val = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_white"][2])
        # Upper HSV for White
        self.upper_white_hue = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_white"][0])
        self.upper_white_sat = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_white"][1])
        self.upper_white_val = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_white"][2])

        # Labels and sliders for Lower HSV (White)
        ttk.Label(white_frame, text="Lower Hue (0-180):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=180, orient="horizontal", variable=self.lower_white_hue, command=self.update_hsv).pack(fill="x")
        self.lower_white_hue_label = ttk.Label(white_frame, text=str(int(self.lower_white_hue.get())), style="Custom.TLabel")
        self.lower_white_hue_label.pack()

        ttk.Label(white_frame, text="Lower Sat (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=255, orient="horizontal", variable=self.lower_white_sat, command=self.update_hsv).pack(fill="x")
        self.lower_white_sat_label = ttk.Label(white_frame, text=str(int(self.lower_white_sat.get())), style="Custom.TLabel")
        self.lower_white_sat_label.pack()

        ttk.Label(white_frame, text="Lower Val (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=255, orient="horizontal", variable=self.lower_white_val, command=self.update_hsv).pack(fill="x")
        self.lower_white_val_label = ttk.Label(white_frame, text=str(int(self.lower_white_val.get())), style="Custom.TLabel")
        self.lower_white_val_label.pack()

        # Labels and sliders for Upper HSV (White)
        ttk.Label(white_frame, text="Upper Hue (0-180):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=180, orient="horizontal", variable=self.upper_white_hue, command=self.update_hsv).pack(fill="x")
        self.upper_white_hue_label = ttk.Label(white_frame, text=str(int(self.upper_white_hue.get())), style="Custom.TLabel")
        self.upper_white_hue_label.pack()

        ttk.Label(white_frame, text="Upper Sat (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=255, orient="horizontal", variable=self.upper_white_sat, command=self.update_hsv).pack(fill="x")
        self.upper_white_sat_label = ttk.Label(white_frame, text=str(int(self.upper_white_sat.get())), style="Custom.TLabel")
        self.upper_white_sat_label.pack()

        ttk.Label(white_frame, text="Upper Val (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(white_frame, from_=0, to=255, orient="horizontal", variable=self.upper_white_val, command=self.update_hsv).pack(fill="x")
        self.upper_white_val_label = ttk.Label(white_frame, text=str(int(self.upper_white_val.get())), style="Custom.TLabel")
        self.upper_white_val_label.pack()

        # Red Ball HSV Sliders
        ttk.Label(frame, text="Red Ball Detection", font=("Helvetica", 12, "bold"), style="Custom.TLabel").pack(pdy=5)
        red_frame = ttk.Frame(frame)
        red_frame.pack(pady=5, fill="x")

        # Lower HSV for Red
        self.lower_red_hue = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_red"][0])
        self.lower_red_sat = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_red"][1])
        self.lower_red_val = tk.DoubleVar(value=BALL_COLOR_RANGE["lower_red"][2])
        # Upper HSV for Red
        self.upper_red_hue = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_red"][0])
        self.upper_red_sat = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_red"][1])
        self.upper_red_val = tk.DoubleVar(value=BALL_COLOR_RANGE["upper_red"][2])

        # Labels and sliders for Lower HSV (Red)
        ttk.Label(red_frame, text="Lower Hue (0-180):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=180, orient="horizontal", variable=self.lower_red_hue, command=self.update_hsv).pack(fill="x")
        self.lower_red_hue_label = ttk.Label(red_frame, text=str(int(self.lower_red_hue.get())), style="Custom.TLabel")
        self.lower_red_hue_label.pack()

        ttk.Label(red_frame, text="Lower Sat (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=255, orient="horizontal", variable=self.lower_red_sat, command=self.update_hsv).pack(fill="x")
        self.lower_red_sat_label = ttk.Label(red_frame, text=str(int(self.lower_red_sat.get())), style="Custom.TLabel")
        self.lower_red_sat_label.pack()

        ttk.Label(red_frame, text="Lower Val (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=255, orient="horizontal", variable=self.lower_red_val, command=self.update_hsv).pack(fill="x")
        self.lower_red_val_label = ttk.Label(red_frame, text=str(int(self.lower_red_val.get())), style="Custom.TLabel")
        self.lower_red_val_label.pack()

        # Labels and sliders for Upper HSV (Red)
        ttk.Label(red_frame, text="Upper Hue (0-180):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=180, orient="horizontal", variable=self.upper_red_hue, command=self.update_hsv).pack(fill="x")
        self.upper_red_hue_label = ttk.Label(red_frame, text=str(int(self.upper_red_hue.get())), style="Custom.TLabel")
        self.upper_red_hue_label.pack()

        ttk.Label(red_frame, text="Upper Sat (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=255, orient="horizontal", variable=self.upper_red_sat, command=self.update_hsv).pack(fill="x")
        self.upper_red_sat_label = ttk.Label(red_frame, text=str(int(self.upper_red_sat.get())), style="Custom.TLabel")
        self.upper_red_sat_label.pack()

        ttk.Label(red_frame, text="Upper Val (0-255):", style="Custom.TLabel").pack(anchor="w")
        ttk.Scale(red_frame, from_=0, to=255, orient="horizontal", variable=self.upper_red_val, command=self.update_hsv).pack(fill="x")
        self.upper_red_val_label = ttk.Label(red_frame, text=str(int(self.upper_red_val.get())), style="Custom.TLabel")
        self.upper_red_val_label.pack()

        # Reset button for HSV sliders
        ttk.Button(frame, text="Reset HSV to Default", command=self.reset_hsv).pack(pady=5)
        ...
        ttk.Checkbutton(frame, text="Disable Sound Effects", variable=self.sound_effects_var, command=self.toggle_sound_effects).pack(pdy=2)
    ...
    def reset_hsv(self):
        global BALL_COLOR_RANGE
        BALL_COLOR_RANGE = DEFAULT_BALL_COLOR_RANGE.copy()
        # Update slider values
        self.lower_white_hue.set(BALL_COLOR_RANGE["lower_white"][0])
        self.lower_white_sat.set(BALL_COLOR_RANGE["lower_white"][1])
        self.lower_white_val.set(BALL_COLOR_RANGE["lower_white"][2])
        self.upper_white_hue.set(BALL_COLOR_RANGE["upper_white"][0])
        self.upper_white_sat.set(BALL_COLOR_RANGE["upper_white"][1])
        self.upper_white_val.set(BALL_COLOR_RANGE["upper_white"][2])

        self.lower_red_hue.set(BALL_COLOR_RANGE["lower_red"][0])
        self.lower_red_sat.set(BALL_COLOR_RANGE["lower_red"][1])
        self.lower_red_val.set(BALL_COLOR_RANGE["lower_red"][2])
        self.upper_red_hue.set(BALL_COLOR_RANGE["upper_red"][0])
        self.upper_red_sat.set(BALL_COLOR_RANGE["upper_red"][1])
        self.upper_red_val.set(BALL_COLOR_RANGE["upper_red"][2])
    ...
    def update_hsv(self, event=None):
        global BALL_COLOR_RANGE
        BALL_COLOR_RANGE["lower_white"] = [int(self.lower_white_hue.get()), int(self.lower_white_sat.get()), int(self.lower_white_val.get())]
        BALL_COLOR_RANGE["upper_white"] = [int(self.upper_white_hue.get()), int(self.upper_white_sat.get()), int(self.upper_white_val.get())]
        BALL_COLOR_RANGE["lower_red"] = [int(self.lower_red_hue.get()), int(self.lower_red_sat.get()), int(self.lower_red_val.get())]
        BALL_COLOR_RANGE["upper_red"] = [int(self.upper_red_hue.get()), int(self.upper_red_sat.get()), int(self.upper_red_val.get())]

        # Update labels with current slider values
        self.lower_white_hue_label.config(text=str(int(self.lower_white_hue.get())))
        self.lower_white_sat_label.config(text=str(int(self.lower_white_sat.get())))
        self.lower_white_val_label.config(text=str(int(self.lower_white_val.get())))
        self.upper_white_hue_label.config(text=str(int(self.upper_white_hue.get())))
        self.upper_white_sat_label.config(text=str(int(self.upper_white_sat.get())))
        self.upper_white_val_label.config(text=str(int(self.upper_white_val.get())))

        self.lower_red_hue_label.config(text=str(int(self.lower_red_hue.get())))
        self.lower_red_sat_label.config(text=str(int(self.lower_red_sat.get())))
        self.lower_red_val_label.config(text=str(int(self.lower_red_val.get())))
        self.upper_red_hue_label.config(text=str(int(self.upper_red_hue.get())))
        self.upper_red_sat_label.config(text=str(int(self.upper_red_sat.get())))
        self.upper_red_val_label.config(text=str(int(self.upper_red_val.get())))

        print(f"Updated BALL_COLOR_RANGE: {BALL_COLOR_RANGE}")
...
class HelpWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        ttk.Button(frame, text="Show Tutorial", command=self.show_tutorial).pack(pdy=5)
        ttk.Button(frame, text="Close", command=self.close).pack(pdy=5)
...
class WhiffleGame:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Whiffle Playfield v{VERSION}")
        ...
        self.current_fps = 0  # Store the current FPS
        ...
        self.fps_label = tk.Label(self.stats_frame, text="FPS: 0", font=("Helvetica", 12), bg="#2E2E2E", fg="white")
        ...
        try:
            ...
            tk.messagebox.showerror("Error", f"Webcam initialization failed: {e}. Please ensure a webcam is connected and try again.")
            self.root.destroy()
            return
        ...
        self.calibration_guide_text = None
    ...
    def new_game_menu(self):
        ...
        tk.Button(frame, text="Classic Mode", command=lambda: self.new_game(True, new_game_window)).pack(pdy=5)
        tk.Button(frame, text="Timed Mode (2m)", command=lambda: self.new_game(False, new_game_window)).pack(pdy=5)
    ...
    def high_score(self):
        ...
        leaderboard = load_high_score()
        if not leaderboard:
            ttk.Label(frame, text="No scores yet!", font=("Helvetica", 12), style="Custom.TLabel").pack(pdy=5)
        else:
            for i, entry in enumerate(leaderboard, 1):
                ttk.Label(frame, text=f"{i}. {entry['initials']}: {entry['score']}", font=("Helvetica", 12), style="Custom.TLabel").pack(pdy=5)
        ttk.Button(frame, text="Close", command=lambda: high_score_window.destroy() or self.resume_frame()).pack(pdy=10)
    ...
    def render_frame(self):
        ...
        if self.frame is not None:
            ...
            # Cache scoring zones as static elements
            if self.calibrating or not self.zone_circles:
                for x_frame, y_frame, r, points in self.point_zones:
                    ...
                    circle_id = self.canvas.create_oval(x_canvas - r_canvas, y_canvas - r_canvas, 
                                                       x_canvas + r_canvas, y_canvas + r_canvas, outline="red", width=2)
                    self.red_zone_circles.append(circle_id)
                    text_id = self.canvas.create_text(x_canvas, y_canvas, text=str(points), fill="white", font=("Helvetica", 10))
                    self.red_zone_texts.append(text_id)

            if self.special_hole and not self.special_hole_circle:
                ...
                self.special_hole_circle = self.canvas.create_oval(x_canvas - r_canvas, y_canvas - r_canvas, 
                                                                  x_canvas + r_canvas, y_canvas + r_canvas, outline="purple", width=2)
                self.special_hole_text = self.canvas.create_text(x_canvas, y_canvas, text="Double!", fill="purple", font=("Helvetica", 10))
            ...
            if self.power_up_zone and self.power_up_zone.is_active():
                ...
                self.power_up_zone_circle = self.canvas.create_oval(x_canvas - r_canvas, y_canvas - r_canvas, 
                                                                   x_canvas + r_canvas, y_canvas + r_canvas, outline="green", width=2)
                self.power_up_zone_text = self.canvas.create_text(x_canvas, y_canvas, text="Power-Up", fill="green", font=("Helvetica", 10))
            ...
            # Add calibration guide overlay if in calibration mode
            if self.calibrating and not self.calibration_guide_text:
                self.calibration_guide_text = self.canvas.create_text(canvas_width // 2, 30, 
                                                                     text=f"Click to define zones ({self.zone_count + (1 if self.special_hole else 0)}/{TOTAL_ZONES}). Press 'Save Zones' when done.",
    ...
    def update_game_logic(self, new_width, new_height, offset_x, offset_y):
        ...
        if self.calibrating:
            ...
            round_score, self.last_red_score_time, scored_positions, power_up_activated, power_up_type = calculate_score(
                tracked_balls, self.point_zones, self.special_hole, self.power_up_zone, self.last_red_score_time, RED_BALL_COOLDOWN, self.power_up
            )
            ...
            for (x, y, r, ball_id, color) in tracked_balls:
                ...
                self.green_ball_circles.append(self.canvas.create_text(x_canvas, y_canvas - r_canvas - 10, 
    ...
    def handle_input(self):
        ...
        if key == ord('q'):
            ...
            if self.calibration_guide_text:
                self.canvas.delete(self.calibration_guide_text)
                self.calibration_guide_text = None
