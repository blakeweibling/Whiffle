...
def _draw_leaderboard_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    if hasattr(game_state, "leaderboard") and game_state.leaderboard: # [cite: 42]
        scores, is_online = game_state.leaderboard.get_top_scores(
            limit=10, mode=game_state.game_mode # [cite: 42]
        )
    else: # [cite: 42]
        logger.warning(
            "game_state.leaderboard not found in _draw_leaderboard_submenu") # [cite: 43]

    status_text = "Online" if is_online else "Local (Offline)" # [cite: 43]
    status_color = UIConstants.GREEN if is_online else UIConstants.YELLOW # [cite: 43]
    cv2.putText(
        menu_frame,
        status_text, # [cite: 43]
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 49]
         item_height), "back_to_main", "Back") # [cite: 49]
    )
    game_state.menu_height = back_y + item_height + 20 # [cite: 49]


# <<< Modified: Added Player Name Editing UI >>> # [cite: 49]
def _draw_players_submenu(menu_frame: np.ndarray, game_state: Any) -> None: # [cite: 50]
    ...
    name_width = menu_frame.shape[1] - 40 - \
        (button_width * 2 + button_spacing * 2) # [cite: 51]
    ...
    if (
        ...
        instruction_text = "Enter Name (A-Z, 0-9), Bksp, Enter=Save, ESC=Cancel" # [cite: 52]
        cv2.putText(
            menu_frame,
    ...
    for i, player in enumerate(game_state.players): # [cite: 53]
        ...
        if ( # [cite: 54]
            ...
            display_name = f"Edit: [{game_state.editing_player_name_input or ''}_]" # [cite: 54]
            name_color = UIConstants.GREEN  # Highlight name being edited # [cite: 54]
        else: # [cite: 55]
            display_name = f"{i+1}. {player.name}" # [cite: 56]

        # Draw Player Name (or input field) - ensure it fits # [cite: 56]
        max_name_len = name_width // 8  # Approx calculation based on font size # [cite: 56]
        cv2.putText(
            menu_frame,
        ...
        select_color = UIConstants.GREEN if is_current else UIConstants.CV2_BLUE # [cite: 60]
        ...
        if (
            ...
            if not is_current:  # Only allow clicking select if not current # [cite: 65]
                game_state.submenu_items.append(
                    (select_rect, f"select_player_{i}", f"Select P{i+1}") # [cite: 65]
                )
    ...
    add_color = (
        UIConstants.CV2_BLUE if len( # [cite: 66]
            game_state.players) < 2 else UIConstants.GREY_BG # [cite: 66]
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 69]
         item_height), "back_to_main", "Back") # [cite: 69]
    )
...
def _draw_achievements_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    unlocked_count = sum(1 for ach in game_state.achievements if ach.unlocked) # [cite: 70]
    ...
    if not game_state.achievements: # [cite: 71]
        ...
        for achievement in game_state.achievements: # [cite: 72]
            text = f"- {achievement.name}: {achievement.description}" # [cite: 72]
            color = UIConstants.GREEN if achievement.unlocked else UIConstants.WHITE # [cite: 73]
            max_len = 55 # [cite: 73]
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 81]
         item_height), "back_to_main", "Back") # [cite: 81]
    )
...
def _draw_help_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 87]
         item_height), "back_to_main", "Back") # [cite: 87]
    )
...
def _draw_faq_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 92]
         item_height), "back_to_main", "Back") # [cite: 93]
    )
...
def _draw_about_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, # [cite: 98]
         item_height), "back_to_main", "Back") # [cite: 98]
    )
