...
from menu import (
    save_zones,
)  # [cite: 28] Keep if clean_exit calls it directly or indirectly
...
# [cite: 29] Keep if clean_exit uses it
...
def clean_exit(
    ...
    if game_state:  # [cite: 29]
        try:  # [cite: 29]
            ...
            if (
                ...
                if hasattr(game_state, "save_score"):  # [cite: 31]
                    game_state.save_score(
                        player_name
                    )  # [cite: 32] save_score handles high score logic now
                else:  # [cite: 32]
                    # [cite: 32]
            ...
            # [cite: 34] Assumes save_zones is still imported
            ...
            try:  # [cite: 34]
                ...
                ):  # [cite: 35] Example if it became a method
                    game_state.save_achievements(
                ...
                logger.exception(
                    f"Error saving achievements: {e}")  # [cite: 37]
    ...
    try:  # [cite: 44]
        ...
        logger.exception(
            f"Unexpected error quitting Pygame: {e}")  # [cite: 45]
    # --- MODIFICATION END ---
    ...
    if cap and hasattr(cap, "isOpened") and cap.isOpened():  # [cite: 40]
        ...
        try:  # [cite: 40]
            ...
            logger.exception(
                f"Unexpected error releasing camera: {e}")  # [cite: 41]
    ...
    try:  # [cite: 42]
        ...
        cv2.waitKey(
            1
        )  # [cite: 43] Add small waitkey needed sometimes for windows to close properly
    ...
    sys.exit(
        "Exiting Whiffle Tracker."
    )  # [cite: 45] Ensure this is the very last thing called
