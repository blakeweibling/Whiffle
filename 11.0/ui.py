...
BALL_COLORS = {
    ...
    "half": (
        ...
        0,
        255,
    ),  # Magenta for half red/half white (Keep or add to UIConstants)
}
...
def _draw_text_with_background(
    ...
    if rect_x1 < rect_x2 and rect_y1 < rect_y2:  # Check if rectangle has valid size
        ...
        if sub_img.size == 0:
            logger.warning(
                f"Empty sub_img for text background at {pos}. Text: '{text}'"
            )
        ...
        try:
            ...
            logger.error(
                f"CV2 error in addWeighted for text '{text}': {e}. ROI shape: {sub_img.shape}, BG shape: {bg_rect.shape}"
            )
    ...
    if hasattr(game_state, "tracked_balls"):
        for ball in game_state.tracked_balls:
            try:
                ...
                ValueError,
                TypeError,
            ):  # Catch potential unpacking errors
                logger.warning(
                    f"Malformed ball data for debug overlay: {ball}")
            except Exception as e:
                logger.error(
                    f"Error drawing debug overlay for ball {ball}: {e}")
...
def show_splash_screen(supabase_url: str, supabase_key: str) -> Optional[GameState]:
    ...
    try:
        ...
        logger.error(f"Failed window create: {e}")
        return None
    ...
    try:  # Initialize GameState
        ...
        if not game_state.camera_available:
            if game_state.static_frame is None:
                logger.error("Static frame None. Exiting.")
                return None
            first_frame = game_state.static_frame.copy()
            logger.info("Using static frame.")
        else:
            ret, first_frame = game_state.cap.read()
            if not ret or first_frame is None:
                logger.error("Failed capture first frame.")
                clean_exit(
                    game_state.cap,
            ...
            logger.info("Captured first frame.")
    ...
    while (
        ...
        # Check for window close at the start of the loop
        try:
            if cv2.getWindowProperty(UIConstants.WINDOW_NAME, cv2.WND_PROP_VISIBLE) < 1:
                logger.info("Splash window closed via red X.")
                clean_exit(
                    game_state.cap,
                    game_state.background_music,
                    game_state.background_music_on,
        ...
        display_frame = (
            np.zeros_like(
                first_frame) if first_frame is not None else splash.copy()
        )
        try:  # Fade logic
            if elapsed < GameConstants.SPLASH_DURATION:
                display_frame = splash
                if game_state.camera_available or game_state.static_frame is not None:
                    cv2.putText(
                        display_frame,
                        "Ready...",
                        (50, UIConstants.WINDOW_HEIGHT - 50),
                ...
                elapsed < GameConstants.SPLASH_DURATION + GameConstants.FADE_DURATION
                ...
                if splash.shape == first_frame.shape:
                    ...
                    logger.warning("Splash/frame mismatch.")
                    display_frame = first_frame
            else:
                display_frame = first_frame if first_frame is not None else splash
            cv2.imshow(UIConstants.WINDOW_NAME, display_frame)
        except Exception as e:
            logger.exception(f"Splash loop error: {e}")
            cv2.putText(
                display_frame,
                "Display Error",
                (10, 30),
