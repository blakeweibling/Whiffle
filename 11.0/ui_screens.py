...
def _draw_game_over_screen(
    ...
    if game_over_splash_cache is not None and game_over_splash_cache != "fallback":
        frame[:, :] = game_over_splash_cache.copy() # Use copy to avoid modifying cache
...
def show_splash_screen(supabase_url: str, supabase_key: str) -> Optional[GameState]:
    ...
    try: # Initialize GameState
        ...
        if not game_state.camera_available:
            if game_state.static_frame is None:
                ...
                cv2.putText(splash, "Error: No Camera/Static Frame", (50, UIConstants.WINDOW_HEIGHT // 2 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, UIConstants.RED, 1)
                cv2.imshow(UIConstants.WINDOW_NAME, splash)
                cv2.waitKey(3000)
                return None
            first_frame = game_state.static_frame.copy()
            first_frame = cv2.resize(first_frame, (UIConstants.WINDOW_WIDTH, UIConstants.WINDOW_HEIGHT)) # Ensure size match
            ...
            if not ret or frame_read is None:
                ...
                # Display error on splash
                cv2.putText(splash, "Error: Camera Read Failed", (50, UIConstants.WINDOW_HEIGHT // 2 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, UIConstants.RED, 1)
                cv2.imshow(UIConstants.WINDOW_NAME, splash)
                cv2.waitKey(3000)
    ...
    if first_frame is None:
         logger.error("First frame is None after init attempts. Cannot proceed with fade.")
         # Display error on splash
         cv2.putText(splash, "Error: Frame Init Failed", (50, UIConstants.WINDOW_HEIGHT // 2 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, UIConstants.RED, 1)
         cv2.imshow(UIConstants.WINDOW_NAME, splash)
         cv2.waitKey(3000)
         clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state)
         return None
    ...
    while (
        ...
        try:
            ...
            logger.info("Window property check failed (cv2.error), assuming window closed.")
            clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state)
        ...
        try: # Fade logic
            if elapsed < GameConstants.SPLASH_DURATION:
                ...
                if splash.shape == first_frame.shape:
                    ...
                    logger.warning("Splash/first_frame shape mismatch during fade. Displaying first_frame.")
            ...
            cv2.putText(display_frame,"Display Loop Error",(10, 30),cv2.FONT_HERSHEY_SIMPLEX,1,UIConstants.RED,2)
            cv2.imshow(UIConstants.WINDOW_NAME, display_frame)
