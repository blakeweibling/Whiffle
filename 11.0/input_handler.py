...
from menu import (
    ...
    load_zones,
    clear_zones,
)  # Import functions called by actions
...
def _process_drawing_event(event: int, x: int, y: int, game_state: Any) -> None:
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if game_state.drawing:
            ...
            logger.debug(
                "Ignoring LBUTTONDOWN for drawing, 's' key not active.")
    ...
    for item_rect, action, label in game_state.submenu_items:
        ...
        if (
            ...
            # Handle string-based actions
                ...
                logger.debug(
                    "Action is string. Checking specific string values...")
                ...
                if game_state.current_state == CurrentGameState.MENU:
                    ...
                    if action == "show_splash":
                        ...
                        if game_state.game_mode != new_mode:
                            ...
                            logger.info(
                                f"Game mode already set to: {new_mode}")
                        ...
                        try:
                            ...
                            if 0 <= index < len(game_state.scoring_zones):
                                ...
                                logger.warning(
                                    f"Invalid zone index for edit: {index}")
                ...
                # --- Game Over State Actions ---
                    ...
                    if action == "new_game_from_gameover":
                        logger.debug(
                            "Action matched: 'new_game_from_gameover'")
                        ...
                        logger.info(
                            "Showing leaderboard from game over screen.")
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    if (
        ...
        logger.info(
            "Click detected during splash, returning to previous state.")
        if game_state.previous_state:
            ...
            logger.warning(
                "Previous state was None when exiting splash, returning to MENU."
            )
