...
def draw_ui(
    ...
    if game_state.current_state not in [CurrentGameState.GAME_OVER]:
        # Draw Score, High Score, Mode Text
    ...
    if game_state.current_state == CurrentGameState.PLAYING:
        ...
        overlay = frame.copy()
        draw_scoring_zones(overlay, game_state.scoring_zones, game_state.special_hole)
        draw_balls(overlay, game_state)
        cv2.addWeighted(overlay, 0.3, frame, 0.7, 0, frame)
        # Draw the menu window itself
        # Ensure width/height have defaults if needed, but DO NOT reset position here
        ...
        # REMOVED: mx, my = ...
        # REMOVED: game_state.menu_pos = (mx, my) # THIS WAS THE PROBLEM

        # Call draw_menu_window which will use the current game_state.menu_pos
        draw_menu_window(frame, game_state)

    # --- NEW: ZONE_EDITING State Drawing ---
