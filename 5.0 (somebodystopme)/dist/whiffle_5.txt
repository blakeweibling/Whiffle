        ...
        if ball_id not in scored_ball_ids:
            ...
            if power_up_zone and power_up_zone.is_active():
                ...
                if distance <= zone_radius:
                    ...
                    power_up_zone.deactivate()  # Deactivate the power-up zone
            ...
            for zone_x, zone_y, zone_radius, points in point_zones:
                ...
                if distance <= zone_radius:
                    ...
                    if power_up and power_up.is_active() and power_up.power_up_type == "Score Multiplier":
                        ...
                        print(f"Score Multiplier applied: {points} to {base_points} points")
                        power_up.deactivate()  # Deactivate after use
...
class OptionsWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        for i, (label, value, min_val, max_val) in enumerate(self.fields):
            ttk.Label(self.frame, text=label, font=("Helvetica", 10), style="Custom.TLabel").pack(anchor="w", padx=5)
            self.slider_vars[label] = tk.DoubleVar(value=value)
            slider = ttk.Scale(self.frame, from_=min_val, to=max_val, orient="horizontal", variable=self.slider_vars[label])
            slider.pack(fill="x", padx=5, pady=2)
            value_label = ttk.Label(self.frame, textvariable=self.slider_vars[label], font=("Helvetica", 10), style="Custom.TLabel")
            value_label.pack(anchor="w", padx=5)

        # Resolution toggle
        ttk.Label(self.frame, text="Resolution", font=("Helvetica", 10), style="Custom.TLabel").pack(pady=2)
        self.resolution_var = tk.StringVar(value="1920x1080")
        ttk.Radiobutton(self.frame, text="1080p (1920x1080)", value="1920x1080", variable=self.resolution_var, style="Custom.TRadiobutton").pack(anchor="w")
        ttk.Radiobutton(self.frame, text="720p (1280x720)", value="1280x720", variable=self.resolution_var, style="Custom.TRadiobutton").pack(anchor="w")
