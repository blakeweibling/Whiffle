...
def start_versus_mode(game_state):
    ...
    show_notification(game_state, f"{game_state.versus_players[0].name}'s Turn", duration=3.0)
    
    logger.info(f"Started versus mode with {game_state.versus_players[0].name} vs {game_state.versus_players[1].name}")
...
def switch_versus_player_turn(game_state):
    ...
    if game_state.data_logger and hasattr(game_state, "current_session_stats"):
        if game_state.current_session_stats is not None:
            game_state.versus_stats[current_player] = game_state.current_session_stats.copy()
...
def show_versus_results(game_state):
    ...
    """
    frame = np.zeros((game_state.current_height, game_state.current_width, 3), dtype=np.uint8)
    ...
    # Draw header
    cv2.putText(frame, "VERSUS MODE RESULTS", 
    ...
    if game_state.versus_scores[0] == game_state.versus_scores[1]:
        cv2.putText(frame, "IT'S A TIE!", 
                   (int(game_state.current_width/2) - 100, 180),
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, UIConstants.YELLOW, 2, cv2.LINE_AA)
    else:
        cv2.putText(frame, f"WINNER: {winner_name}", 
                   (int(game_state.current_width/2) - 150, 180),
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, UIConstants.GREEN, 2, cv2.LINE_AA)
    ...
    for i in range(2):
        ...
        color = UIConstants.GREEN if i == winner_idx and game_state.versus_scores[0] != game_state.versus_scores[1] else UIConstants.WHITE
        
        cv2.putText(frame, f"{player.name}: {score} points", 
    ...
    for i in range(2):
        ...
        if stats:
            cv2.putText(frame, f"{player.name}'s Stats:", 
                       (int(game_state.current_width/2) - 150, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, UIConstants.WHITE, 1, cv2.LINE_AA)
            y_pos += 40
            
            # Display at most 4 stats per player
            stat_count = 0
            for stat_name, stat_value in stats.items():
                if isinstance(stat_value, (int, float)) and stat_name not in ['score', 'time_elapsed'] and stat_count < 4:
                    cv2.putText(frame, f"  {stat_name}: {stat_value}", 
    ...
    # Draw instructions
    cv2.putText(frame, "Press 'M' to return to menu", 
