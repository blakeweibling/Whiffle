...
from math import ceil # Add import

# Imports needed for mouse_callback helpers
from constants import UIConstants, GameConstants, ScoringConstants, MenuConstants # Added MenuConstants
from menu import (
...
# --- Menu/Game Over Click Processing (Modified for Close Button and Zone Edit Actions) ---
def _process_menu_or_gameover_click(x: int, y: int, game_state: GameState) -> bool:
    ...
    main_menu_actions = {item[1] for item in MenuConstants.MAIN_MENU_ITEMS if isinstance(item[1], str)}
    zone_submenu_actions = {item[1] for item in MenuConstants.ZONE_SUBMENU_ITEMS if isinstance(item[1], str)}
    known_submenu_nav_actions = main_menu_actions.union(zone_submenu_actions) - {"resume", "quit", "back_to_main", "back_to_manage_zones", "save_zones", "load_zones", "clear_zones", "add_zone_info"} # Remove non-nav actions
    ...
    for item_rect, action, label in reversed(game_state.submenu_items):
        ...
        if (
            ...
            if isinstance(action, Callable):
                ...
                if game_state.current_state == CurrentGameState.MENU:
                    game_state.menu_cache = None # Invalidate cache after action
                return True
                ...
                if game_state.current_state == CurrentGameState.MENU:
                    ...
                    if action == "show_splash":
                        ...
                        game_state.menu_cache = None # Splash covers menu, redraw needed
                        return True
                        ...
                        save_zones(game_state)
                        game_state.menu_cache = None # Content doesn't change, but feedback shown
                        ...
                        # Go back to playing after mode select/confirm
                        game_state.current_state = CurrentGameState.PLAYING
                        game_state.submenu_active = None
                        reset_editing_states()
                        ...
                        # No need to reset page number here, it's handled by _reset_all_menu_editing_states
                        reset_editing_states() # Reset specific edit modes
                        game_state.submenu_active = "manage_zones"


                    # --- NEW: Handle Pagination Actions ---
                    ...
                    # --- END NEW: Handle Pagination Actions ---
                        ...
                        try:
                            ...
                            if 0 <= index < len(game_state.scoring_zones):
                                if not (
                                    ...
                                    _reset_all_menu_editing_states(game_state) # Reset others first
                    ...
                    # --- Interactive Zone Move ---
                        ...
                        try:
                            ...
                            if 0 <= index < len(game_state.scoring_zones):
                                _reset_all_menu_editing_states(game_state) # Clear points/delete confirms etc.
                        ...
                        try:
                            ...
                            if 0 <= index < len(game_state.scoring_zones):
                                if (
                                    ...
                                    _reset_all_menu_editing_states(game_state) # Reset state after delete
                                else:
                                    # First click: enter confirm mode
                                    _reset_all_menu_editing_states(game_state) # Reset any other editing modes
                                    game_state.editing_zone_index = index
                    ...
                    # --- FIXED: Explicit Submenu Navigation Handling ---
                    ...
                    # --- END FIX ---
                        ...
                        # Log unhandled actions specifically within the MENU state
                        logger.warning(f"Unhandled string action in MENU state: {action}")
                ...
                # Game Over State Actions
                    ...
                    if action == "new_game_from_gameover":
                        ...
                        logger.warning(f"Unhandled string action in GAME_OVER state: {action}")
