...
httpx.Client = lambda *args, **kwargs: httpx.Client(*args, verify=False, **kwargs)
...
supabase: Client = create_client(SUPABASE["URL"], SUPABASE["KEY"], options=ClientOptions())
...
secret_splash_surface = pygame.Surface((400, 200), pygame.SRCALPHA)
secret_splash_surface.fill((200, 200, 200, 200))
secret_text = FONTS["ui"].render("Secret Unlocked!", True, COLORS["BLACK"])
secret_splash_surface.blit(secret_text, (100, 80))
...
class Window:
    def __init__(self, width, height):
        ...
        self.height = height
        self.surface = None
        self.close_button_rect = None  # Initialize as None, set in draw_base
    ...
    def draw_base(self, screen, title):
        ...
        if self.surface is None:
            ...
            self.close_button_rect = pygame.Rect(self.width - 40, 10, 30, 30)  # Reinitialize here
        
        # Clear surface and draw shadow
        self.surface.fill((0, 0, 0, 0))
        pygame.draw.rect(self.surface, (100, 100, 100, 100), (5, 5, self.width, self.height), border_radius=10)  # Shadow
        pygame.draw.rect(self.surface, (200, 200, 200, 200), (0, 0, self.width, self.height), border_radius=10)  # Main background
        
        # Title bar
        pygame.draw.rect(self.surface, COLORS["DARK_GRAY"], (0, 0, self.width, 40), border_top_left_radius=10, border_top_right_radius=10)
        if title:
            title_text = self.font.render(title, True, COLORS["WHITE"])
            self.surface.blit(title_text, (10, 10))
        
        # Circular close button with "X"
        pygame.draw.circle(self.surface, COLORS["RED"], self.close_button_rect.center, 15)
        pygame.draw.line(self.surface, COLORS["WHITE"], (self.width - 45, 15), (self.width - 25, 35), 2)
        pygame.draw.line(self.surface, COLORS["WHITE"], (self.width - 25, 15), (self.width - 45, 35), 2)
    ...
    def handle_base_input(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            # Check circular close button
            center_x, center_y = self.close_button_rect.center
            if ((local_x - center_x) ** 2 + (local_y - center_y) ** 2) ** 0.5 <= 15:
                self.active = False
                return True
...
class Menu:
    ...
    def draw(self, screen):
        ...
        # Gradient background
        for y in range(SETTINGS["MENU_HEIGHT"]):
            color = (64 + y * 2, 64 + y * 2, 64 + y * 2)  # Dark gray to lighter
            pygame.draw.line(self.surface, color, (0, y), (current_width, y))
        ...
        for main_option, sub_options in self.options.items():
            ...
            if main_option == self.hovered_main:
                pygame.draw.rect(self.surface, COLORS["LIGHT_GRAY"], button_rect.inflate(4, 4), border_radius=10)
            pygame.draw.rect(self.surface, COLORS["DARK_GRAY"], button_rect, border_radius=10)
            pygame.draw.rect(self.surface, COLORS["BUTTON_BORDER"], button_rect, 2, border_radius=10)
            ...
            if self.selected == main_option and sub_options:
                ...
                submenu_width = max(200, max(self.font.render(sub_option, True, COLORS["WHITE"]).get_width() + 20 for sub_option in sub_options))
                submenu_height = len(sub_options) * (self.button_height + self.button_padding)
                pygame.draw.rect(self.surface, COLORS["SUBMENU_BG"], (submenu_x, submenu_y, submenu_width, submenu_height), border_radius=5)
                sub_y = submenu_y
                for sub_option in sub_options:
                    sub_text_surface = self.font.render(sub_option, True, COLORS["WHITE"])
                    sub_button_rect = pygame.Rect(submenu_x, sub_y, submenu_width, self.button_height)
                    color = COLORS["YELLOW"] if sub_option == self.hovered_sub else COLORS["WHITE"]
                    if sub_option == self.hovered_sub:
                        pygame.draw.rect(self.surface, COLORS["LIGHT_GRAY"], sub_button_rect.inflate(4, 4), border_radius=5)
                    pygame.draw.rect(self.surface, COLORS["DARK_GRAY"], sub_button_rect, border_radius=5)
    ...
    def handle_input(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN:
                ...
                pygame.display.flip()  # Force redraw
                return True
                ...
                if self.submenu and self.options[self.selected][self.submenu]:
                    ...
                    self.selected = None  # Clear selection after action
                    self.submenu = None
    ...
    def handle_mouse_input(self, event):
        if event.type == pygame.MOUSEMOTION:
            ...
            self.hovered_sub = None
            mouse_pos = event.pos
            # Check for hovering over main menu items
            for main_option, rect in self.main_rects.items():
                if rect.collidepoint(mouse_pos):
                    ...
                    break
            # Check for hovering over submenu items if a menu is selected
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            # Check for clicks on main menu items
            clicked_main = None
            for main_option, rect in self.main_rects.items():
                if rect.collidepoint(mouse_pos):
                    clicked_main = main_option
                    break
            
            if clicked_main:
                # If clicking a new main menu item, clear the previous submenu
                if self.selected != clicked_main:
                    logger.debug(f"Switching main menu: from {self.selected} to {clicked_main}")
                    self.selected = clicked_main
                    self.submenu = None
                else:
                    # Clicking the same item toggles it off
                    logger.debug(f"Toggling off main menu: {clicked_main}")
                    self.selected = None
                    self.submenu = None
                pygame.display.flip()  # Force redraw
                return True
            
            # Check for clicks on submenu items
            if self.selected and self.sub_rects:
                clicked_sub = None
                for sub_option, rect in self.sub_rects.items():
                    if rect.collidepoint(mouse_pos):
                        clicked_sub = sub_option
                        break
                if clicked_sub:
                    logger.debug(f"Clicked sub option: {clicked_sub}, Rect: {rect}, Mouse: {mouse_pos}")
                    self.options[self.selected][clicked_sub]()
                    self.selected = None  # Clear selection after action
                    self.submenu = None
                    logger.debug("Menu deselected after submenu click")
                    pygame.display.flip()  # Force redraw
                    return True
            
            # Clicked outside both main menu and submenu, so close the menu
            if self.selected:
                main_menu_clicked = any(rect.collidepoint(mouse_pos) for rect in self.main_rects.values())
                submenu_clicked = any(rect.collidepoint(mouse_pos) for rect in self.sub_rects.values()) if self.sub_rects else False
                if not main_menu_clicked and not submenu_clicked:
...
def draw_ui():
    ...
    # Balls with icon
    balls_text = FONTS["ui"].render(f"Balls: {game.balls if game.mode == 'Classic' else '∞'}", True, COLORS["GREEN"])
    pygame.draw.circle(status_surface, COLORS["WHITE"], (20, 20), 5)  # Simple ball icon
    status_surface.blit(balls_text, (30, 10))
    
    # Score
    score_text = FONTS["ui"].render(f"Score: {game.score}", True, COLORS["YELLOW"])
    status_surface.blit(score_text, (150, 10))
    
    # Time
    time_text = FONTS["ui"].render(f"Time: {game.time}", True, COLORS["WHITE"])
    status_surface.blit(time_text, (300, 10))
    
    # Power-Up with flash effect
    power_text = FONTS["ui"].render(f"Power-Up: {game.power_up or 'None'}", True, COLORS["RED" if game.power_up else "WHITE"])
    if game.power_up and frame_count % 20 < 10:  # Flash every 20 frames
        pygame.draw.rect(status_surface, COLORS["YELLOW"], (450, 5, power_text.get_width() + 10, power_text.get_height() + 10), 2)
    status_surface.blit(power_text, (450, 10))
...
class AboutWindow(Window):
    ...
    def draw(self, screen):
        if not self.active:
            return
        # Draw content onto self.surface before draw_base() blits it
        if self.surface is None:
            self.surface = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        ...
        for line in self.text:
            ...
            y_offset += 20
        # Now draw the base window with the updated surface
...
class SettingsWindow(Window):
    def __init__(self):
        super().__init__(400, 400)  # Increased height to 400 to fit all sliders
        self.slider_values = {
        ...
        if not self.label_to_key:
            logger.error("label_to_key is empty!")
        logger.info(f"SettingsWindow initialized with slider_values: {self.slider_values}, labels: {list(self.label_to_key.keys())}")
    ...
    def draw(self, screen):
        if not self.active:
            return
        logger.info("Drawing SettingsWindow")
        if self.surface is None:
            self.surface = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        self.surface.fill((0, 0, 0, 0))  # Clear surface
        labels = list(self.label_to_key.keys())
        if not labels:
            logger.error("No labels to render in SettingsWindow!")
            return
        y_offset = 40
        for label in labels:
            key = self.label_to_key[label]
            if key not in self.slider_values:
                logger.warning(f"Key {key} not found in slider_values!")
                continue
            text = self.font.render(f"{label}: {int(self.slider_values[key])}", True, COLORS["BLACK"])
            logger.debug(f"Rendering label: {label}, value: {self.slider_values[key]}, y_offset: {y_offset}")
            self.surface.blit(text, (10, y_offset))
            slider_length = 200
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            for key, (slider_x, slider_y, slider_length) in self.slider_positions.items():
                ...
                if slider_rect.collidepoint(mouse_pos):
                    ...
                    logger.info(f"Started dragging slider: {key} at {mouse_pos}")
                    return True
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
            if self.dragging_slider:
                ...
                self.dragging_slider = None
                logger.info(f"Updated slider {self.dragging_slider} to value {value}")
                return True
        return False
...
def detect_ball_in_hole(image, hole_coords, game_state, frame_count, scale_x=1.0, scale_y=1.0):
    ...
    # Further broadened red detection ranges
    lower_red1 = np.array([0, 5, 30])    # Even lower S and V thresholds, extended H range
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_MOUSEMOVE:
            mouse_callback.last_pos = (x, y)

    if not hasattr(mouse_callback, 'last_pos'):
        mouse_callback.last_pos = (0, 0)

    cv2.namedWindow('Playfield Debug')
    cv2.setMouseCallback('Playfield Debug', mouse_callback)

    debug_frame = image.copy()
    cv2.imshow('Playfield Debug', debug_frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('p'):
        x, y = mouse_callback.last_pos
        y = max(0, min(y, hsv.shape[0] - 1))
        x = max(0, min(x, hsv.shape[1] - 1))
        hsv_at_point = hsv[y, x]
        logger.info(f"HSV at ({x}, {y}): Hue={hsv_at_point[0]}, Saturation={hsv_at_point[1]}, Value={hsv_at_point[2]}")
...
while running:
    try:
        ...
        screen.fill(COLORS["BLACK"])  # Clear the screen every frame
