...
class Replay:
    ...
    def generate_video(self, output_path: Optional[str] = None) -> Optional[str]:
        """
        ...
        if output_path is None:
            ...
            output_path = os.path.join(REPLAY_DIR, "videos", f"{self.replay_id}.mp4")
        ...
        try:
            ...
            fps = 10  # Use 10fps for keyframes to make it smooth enough but faster than real-time
            ...
            for keyframe_file in keyframe_files:
                ...
                if frames_written % 10 == 0:
                    logger.debug(
                        f"Written {frames_written}/{len(keyframe_files)} frames to video"
                    )
    ...
    def extract_highlight_video(
        self, highlight_index: int, output_path: Optional[str] = None
    ) -> Optional[str]:
        """
...
class ReplayManager:
    ...
    def generate_video(
        self, replay_id: str, output_path: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        return replay.generate_video(output_path)

    def extract_highlight(
        self, replay_id: str, highlight_index: int, output_path: Optional[str] = None
    ) -> Optional[str]:
        """
    ...
    def share_replay(
        self, replay_id: str, share_method: str = "local"
    ) -> Optional[str]:
        """
        ...
        try:
            if share_method == "local":
                ...
                return self.generate_video(replay_id)
