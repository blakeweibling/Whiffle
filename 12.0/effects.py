...
class BallTrail:
    ...
    def draw(self, frame: np.ndarray):
        ...
        for i in range(num_points - 1):
            ...
            color = tuple(int(c1 + (c2 - c1) * fade_ratio) for c1, c2 in zip(TRAIL_START_COLOR, TRAIL_END_COLOR))
            thickness = int(TRAIL_START_THICKNESS + (TRAIL_END_THICKNESS - TRAIL_START_THICKNESS) * fade_ratio)
            thickness = max(1, thickness)

            try:
                cv2.line(frame, pt1, pt2, color, thickness, lineType=cv2.LINE_AA)
            except OverflowError: pass
            except Exception as e: print(f"Error drawing trail line: {e}")
...
class Explosion:
    ...
    def __init__(self, center_x: int, center_y: int):
        ...
        for _ in range(EXPLOSION_PARTICLE_COUNT):
            ...
            vy = np.sin(angle) * speed - random.uniform(0, EXPLOSION_GRAVITY * 0.1) # Slight upward nudge
    ...
    def draw(self, frame: np.ndarray):
        ...
        alpha_blend = 0.5 + 0.5 * (1.0 - (elapsed_time / EXPLOSION_DURATION)) # Fade alpha from 1.0 down to 0.5
        ...
        for p in self.particles:
            ...
            try:
                ...
                cv2.circle(overlay, (int(x), int(y)), EXPLOSION_PARTICLE_SIZE, color, -1, lineType=cv2.LINE_AA)
            except OverflowError: pass
            except Exception as e: print(f"Error drawing explosion particle: {e}")
        ...
        try:
            ...
            for p in self.particles:
                 x, y, _, _, color = p
                 try: cv2.circle(frame, (int(x), int(y)), EXPLOSION_PARTICLE_SIZE, color, -1, lineType=cv2.LINE_AA)
