...
def reset_game(game_state: GameState) -> None:
    ...
    if game_state.game_mode == "timed":
        ...
        logger.info(f"Timed mode selected. Timer set to {game_state.game_timer} seconds.")
    else:
        game_state.game_timer = None
    ...
    logger.info(f"Game state reset for player: {game_state.get_current_player().name}, Mode: {game_state.game_mode}")
...
def _draw_menu_content(menu_frame: np.ndarray, game_state: GameState) -> None:
    ...
    if game_state.submenu_active:
        ...
        cv2.putText(
            menu_frame, "Main Menu", (30, 40), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        for label, action_key in MenuConstants.MAIN_MENU_ITEMS:
            ...
            _draw_button(
                menu_frame, item_rect[0], item_rect[1], item_rect[2], item_rect[3],
...
def draw_menu_window(frame: np.ndarray, game_state: GameState) -> None:
    ...
    current_item_actions_tuple = tuple(item[1] if len(item) > 1 else None for item in game_state.submenu_items)
    cache_key_parts = [
        game_state.submenu_active, current_item_actions_tuple,
    ...
    # Cache check...
    if (
        hasattr(game_state, "menu_cache") and game_state.menu_cache is not None
    ...
    if menu_frame is None:
        ...
        try:
            ...
            btn_y2 = pad + size
            line_pad = size // 4
            cv2.line(menu_frame, (btn_x1 + line_pad, btn_y1 + line_pad), (btn_x2 - line_pad, btn_y2 - line_pad),
                     UIConstants.MENU_CLOSE_BUTTON_COLOR, UIConstants.MENU_CLOSE_BUTTON_THICKNESS)
            cv2.line(menu_frame, (btn_x1 + line_pad, btn_y2 - line_pad), (btn_x2 - line_pad, btn_y1 + line_pad),
                     UIConstants.MENU_CLOSE_BUTTON_COLOR, UIConstants.MENU_CLOSE_BUTTON_THICKNESS)
        except Exception as e:
            logger.error(f"Error drawing menu close button: {e}")

        if menu_frame is not None:
            game_state.menu_cache = menu_frame # Cache the newly drawn frame with close button
    ...
    if roi_h > 0 and roi_w > 0 and menu_end_y <= menu_h and menu_end_x <= menu_w:
        try:
            ...
            if roi.shape == menu_frame_slice.shape:
                ...
                cv2.rectangle(frame, (x1c, y1c), (x2c, y2c), UIConstants.WHITE, 2) # Border
            else:
                logger.warning(f"ROI shape {roi.shape} mismatch menu slice {menu_frame_slice.shape}. Fallback draw.")
                cv2.rectangle(frame, (x1c, y1c), (x2c, y2c), (50, 50, 50), -1)
        except Exception as e:
            logger.exception(f"Error blending menu: {e}")
            cv2.rectangle(frame, (x1c, y1c), (x2c, y2c), (50, 0, 0), -1) # Error fallback
