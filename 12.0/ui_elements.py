...
def draw_balls(frame: np.ndarray, game_state: GameState) -> None:
    """
    Draw ball trails (if in Fun Mode) on the frame.
    ...
    # --- Draw Ball Trails (Fun Mode Only) ---
    # --- RE-ENABLED ---
        ...
        if hasattr(game_state, 'active_trails'):
            for trail in game_state.active_trails.values():
                try:
                    ...
                    logger.error(f"Error drawing trail for ball ID {trail.ball_id}: {e}")
        else:
        ...
        for ball in game_state.tracked_balls:
            try:
                if len(ball) >= 6:
                    ...
                    cv2.rectangle(
                        frame, (text_x, text_y - h - 2), (text_x + w, text_y + 2),
                        (0, 0, 0), -1,
                    )
                    cv2.putText(
                        frame, label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX,
                        UIConstants.FONT_SCALE_SMALL, UIConstants.YELLOW, 1, cv2.LINE_AA
                    )
                else:
                    logger.warning(f"Skipping debug draw for malformed ball data (length < 6): {ball}")
            except (IndexError, ValueError, TypeError) as e:
                logger.warning(f"Error processing ball data for debug overlay: {ball}. Error: {e}")
            except Exception as e:
                logger.error(f"Unexpected error drawing debug overlay for ball {ball}: {e}")
    else:
        logger.debug("No tracked_balls attribute found in game_state for debug overlay.")