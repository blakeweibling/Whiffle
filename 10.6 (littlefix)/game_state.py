...
class GameState: #
    def __init__(self, supabase_url: str, supabase_key: str) -> None: #
        ...
        self.next_ball_id: int = 0; self.ball_trails: Dict[int, List[Tuple[int, int]]] = {} #
        ...
        # <<< Added: Flag for special hole bonus >>>
        ...
        self.menu_pos: Tuple[int, int] = (0, 0); self.menu_width: int = 400; self.menu_height: int = 450 #
        ...
        try: #
            ...
            self.score_sound, self.achievement_sound, self.background_music = None, None, None #
        self.game_sounds_on = True; self.background_music_on = True #
    ...
    def _load_initial_state(self): #
        ...
        # ...(load_initial_state remains the same)...
        ...
        try: # Load High Score for current mode #
             if os.path.exists(GameConstants.HIGH_SCORE_FILE): #
                 ...
                 else: self.high_score = 0; logger.warning(f"High score file exists but is empty.") #
             else: self.high_score = 0; logger.info(f"High score file not found.") #
        except (IOError, json.JSONDecodeError) as e: logger.error(f"Failed load high score: {e}"); self.high_score = 0 #
        except Exception as e: logger.exception(f"Unexpected error loading high score: {e}"); self.high_score = 0 #


    # <<< Modified: Save potentially doubled high score >>>
    def _save_high_score(self): #
         ...
         try: #
             if os.path.exists(GameConstants.HIGH_SCORE_FILE): #
                 ...
                 else: logger.warning(f"High score file exists but is empty: {GameConstants.HIGH_SCORE_FILE}"); data = {} #
         except (IOError, json.JSONDecodeError) as e: logger.error(f"Could not read/parse high score file ({GameConstants.HIGH_SCORE_FILE}): {e}. Will overwrite."); data = {} #
         except Exception as e: logger.exception(f"Unexpected error reading high score file: {e}"); data = {} #
         ...
         if self.high_score > current_saved_high: #
              data[self.game_mode]['high_score'] = self.high_score # Save the potentially doubled high score #
              data[self.game_mode]['player'] = self.get_current_player().name # Save current player name with new high score #
              data[self.game_mode]['date'] = time.strftime("%Y-%m-%d"); logger.info(f"Updating high score file for mode '{self.game_mode}' to {self.high_score} by {self.get_current_player().name}") #
         else: #
              logger.debug(f"Current session high score ({self.high_score}) not greater than saved high score ({current_saved_high}) for mode '{self.game_mode}'. No update needed.") #
    ...
    def get_current_player(self) -> Player: #
        ...
        # ...(get_current_player remains the same)...
...
# <<< Modified: Handle score doubling before saving >>>
    def save_score(self, player_name: str, mode: Optional[str] = None) -> None:
        ...
        if score_to_save > 0:
            ...
            # --- START CORRECTION ---
            self.leaderboard.submit_score(player_name, score_to_save, current_mode) # Use submit_score instead of add_score
            # --- END CORRECTION ---
             # Update high score using the potentially doubled score
            if current_mode == self.game_mode and score_to_save > self.high_score:
                 logger.info(f"New high score for mode '{current_mode}': {score_to_save}")
    ...
    def toggle_background_music(self) -> None: #
        ...
        # ...(toggle_background_music remains the same)...
    ...
    def check_achievements(self) -> None: #
        ...
        # ...(check_achievements remains the same)...
    ...
    def update_achievement_notification(self, dt: float) -> None: #
        # ...(update_achievement_notification remains the same)...
    ...
    def show_notification(self, text: str, duration: float = 2.0, is_error: bool = False) -> None: #
        # ...(show_notification remains the same)...
    ...
    def update_scoring(self) -> None: #
        ...
        for ball in self.tracked_balls: #
            ...
            logger.debug(f"Ball {ball_id}: Checking {num_zones} zones. Debug Mode: {self.debug_mode}")
            # --- END INSERTED LOGGING ---

            # Find current zone
            zone,zone_idx = None,-1 #
            for i, z in enumerate(self.scoring_zones): #
                # (Previous inner loop logging removed)
                if is_in_scoring_zone((x,y,r,ball_id), z): # Original line #
                     zone,zone_idx = z, i; break #
            ...
            if zone and stable and ball_id not in self.ball_scored_zones and current_time >= self.scored_cooldown.get(ball_id, 0): #
                ...
                self.scored_cooldown[ball_id] = current_time + GameConstants.SCORE_COOLDOWN_DURATION #

                logger.info(f"Ball {ball_id}({b_type}) scored {points_to_add}pts Zone:{zone_idx}{' (Special Hole)' if is_sp else ''}. Score:{self.score}") #
                ...
                if self.game_mode=="timed" and self.score>=self.win_score and self.current_state!=CurrentGameState.GAME_OVER: #
                    ...
                    self.save_score(self.get_current_player().name) # Save score immediately on win/game over #
                ...
                if self.score > self.high_score: #
                     ...
                     # self.high_score = self.score # Incorrect: High score update happens in save_score now #
