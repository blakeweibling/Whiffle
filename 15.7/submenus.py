...
def _draw_zone_submenu(menu_frame: np.ndarray, game_state: GameState) -> None:
    ...
    for label, action_key in MenuConstants.ZONE_SUBMENU_ITEMS:
        button_label = label
        button_color = UIConstants.CV2_BLUE
        if action_key == "toggle_zone_alignment":
            button_label = (
                "Anchor Alignment: ON"
                if getattr(game_state, "zone_alignment_enabled", False)
                else "Anchor Alignment: OFF"
            )
            button_color = (
                UIConstants.GREEN
                if getattr(game_state, "zone_alignment_enabled", False)
        ...
        elif action_key == "calibrate_anchors":
            ref_count = int(getattr(game_state, "zone_alignment_reference_count", 0) or 0)
            button_label = (
                f"Recalibrate Anchors ({ref_count})"
                if ref_count >= 2
                else "Calibrate Anchors"
            )
        elif action_key == "clear_anchor_calibration":
            button_color = (
                UIConstants.GREY_BG
                if int(getattr(game_state, "zone_alignment_reference_count", 0) or 0) < 1
                else UIConstants.CV2_ORANGE
            )
        _draw_button(
            menu_frame,
            20,
