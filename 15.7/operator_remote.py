...
from anchor_alignment import (
    calibrate_anchor_reference,
    clear_zone_alignment_runtime,
    set_reference_anchor_points,
)
...
REMOTE_ACTION_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "toggle_zone_alignment": {
        "label": "Toggle Anchor Alignment",
    },
    "calibrate_anchors": {
        "label": "Calibrate Anchors",
    },
    "clear_anchor_calibration": {
        "label": "Clear Anchor Calibration",
        "destructive": True,
        "confirm_message": "Clear the saved anchor calibration for this playfield?",
    },
    "toggle_auto_record": {
...
def _build_zone_alignment_snapshot(game_state: Any) -> Dict[str, Any]:
    reference_count = int(getattr(game_state, "zone_alignment_reference_count", 0) or 0)
    detected_count = int(getattr(game_state, "zone_alignment_detected_count", 0) or 0)
    enabled = bool(getattr(game_state, "zone_alignment_enabled", False))
    active = bool(getattr(game_state, "zone_alignment_active", False))
    confidence = float(getattr(game_state, "zone_alignment_confidence", 0.0) or 0.0)
    offset_x = int(getattr(game_state, "zone_alignment_offset_x", 0) or 0)
    offset_y = int(getattr(game_state, "zone_alignment_offset_y", 0) or 0)
    status = str(getattr(game_state, "zone_alignment_status", "Not calibrated") or "Not calibrated")
    last_message = str(getattr(game_state, "zone_alignment_last_message", "") or "")
    calibrated = reference_count >= 2
    return {
        "enabled": enabled,
        "active": active,
        ...
        "offset_x": offset_x,
        "offset_y": offset_y,
        "offset_display": f"dx={offset_x:+d}, dy={offset_y:+d}",
        "status": status,
...
def update_remote_status_snapshot(game_state: Any) -> None:
    ...
    try:
        ...
        alignment_snapshot = _build_zone_alignment_snapshot(game_state)
        ...
        game_state.remote_status_snapshot = {
            ...
            "zone_alignment": alignment_snapshot,
        }
        ...
        toggle_alignment_state = game_state.remote_status_snapshot["action_states"].get("toggle_zone_alignment")
        if toggle_alignment_state is not None and not alignment_snapshot.get("calibrated"):
            toggle_alignment_state["enabled"] = False
            toggle_alignment_state["disabled_reason"] = "Calibrate at least two visible anchors before enabling alignment."
        calibrate_alignment_state = game_state.remote_status_snapshot["action_states"].get("calibrate_anchors")
        if calibrate_alignment_state is not None and not bool(getattr(game_state, "camera_available", True)):
            calibrate_alignment_state["enabled"] = False
            calibrate_alignment_state["disabled_reason"] = "Anchor calibration requires a live camera feed."
    except Exception as exc:
...
def _toggle_zone_alignment_remote(game_state: Any) -> Dict[str, Any]:
    reference_count = int(getattr(game_state, "zone_alignment_reference_count", 0) or 0)
    if not bool(getattr(game_state, "zone_alignment_enabled", False)) and reference_count < 2:
        return {
            "ok": False,
            "message": "Calibrate at least two anchors before enabling alignment.",
        }

    game_state.zone_alignment_enabled = not bool(
        getattr(game_state, "zone_alignment_enabled", False)
    )
    if not game_state.zone_alignment_enabled:
        clear_zone_alignment_runtime(game_state, "Disabled")
    save_settings(game_state)
    label = "enabled" if game_state.zone_alignment_enabled else "disabled"
    show_notification(game_state, f"Anchor alignment {label}", duration=1.5)
    return {"ok": True, "message": f"Anchor alignment {label}."}


def _calibrate_anchors_remote(game_state: Any) -> Dict[str, Any]:
    ok, message = calibrate_anchor_reference(
        game_state, getattr(game_state, "current_frame", None)
    )
    if not ok:
        show_notification(game_state, message, is_error=True, duration=3.0)
        return {"ok": False, "message": message}
    save_settings(game_state)
    show_notification(game_state, message, duration=2.0)
    return {"ok": True, "message": message}


def _clear_anchor_calibration_remote(game_state: Any) -> Dict[str, Any]:
    set_reference_anchor_points(game_state, [])
    game_state.zone_alignment_enabled = False
    clear_zone_alignment_runtime(game_state, "Not calibrated")
    save_settings(game_state)
    show_notification(game_state, "Anchor calibration cleared", duration=1.5)
    return {"ok": True, "message": "Anchor calibration cleared."}
...
def _execute_remote_action(game_state: Any, action_name: str, payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if action_name == "toggle_zone_alignment":
        return _toggle_zone_alignment_remote(game_state)

    if action_name == "calibrate_anchors":
        return _calibrate_anchors_remote(game_state)

    if action_name == "clear_anchor_calibration":
        return _clear_anchor_calibration_remote(game_state)
...
<body>
    ...
    <div class="layout-columns">
      <div class="column">
        ...
        <div class="subgrid" style="margin-top:12px;">
          ...
          <div class="tile">
            <div class="label">Anchor Alignment</div>
            <div id="alignmentStatusValue" class="value" style="font-size:18px;">-</div>
            <div id="alignmentOffsetValue" class="tiny">-</div>
          </div>
        </div>
        </div>
        ...
        <div class="subgrid">
          ...
          <div class="tile">
            ...
            <div id="alignmentSummaryValue" class="tiny">No anchor calibration saved.</div>
            <div class="actions" style="margin-top:12px;">
              <button id="toggleAlignmentButton" data-action="toggle_zone_alignment" onclick="sendAction('toggle_zone_alignment')" class="secondary">Anchor Alignment: OFF</button>
              <button id="calibrateAnchorsButton" data-action="calibrate_anchors" onclick="sendAction('calibrate_anchors')" class="secondary">Calibrate Anchors</button>
              <button id="clearAnchorCalibrationButton" data-action="clear_anchor_calibration" onclick="sendAction('clear_anchor_calibration')" class="secondary">Clear Calibration</button>
            </div>
          </div>
        </div>
      ...
      <div class="column">
        ...
        <div class="grid">
          ...
          <div class="tile"><div class="label">Anchors Detected</div><div id="alignmentDetectedValue" class="value" style="font-size:18px;">-</div></div>
          <div class="tile"><div class="label">Alignment Confidence</div><div id="alignmentConfidenceValue" class="value" style="font-size:18px;">-</div></div>
        </div>
        <div class="subgrid" style="margin-top:12px;">
          <div class="tile">
          ...
          <div class="tile">
            <div class="label">Alignment Detail</div>
            <div id="alignmentDetailValue" class="tiny">-</div>
          </div>
        </div>
        </div>
  ...
  <script>
    ...
    function updateAlignmentUI(data) {{
      const alignment = data.zone_alignment || {{}};
      const status = alignment.status || 'Not calibrated';
      const offsetDisplay = alignment.offset_display || 'dx=+0, dy=+0';
      document.getElementById('alignmentStatusValue').textContent = status;
      document.getElementById('alignmentOffsetValue').textContent = offsetDisplay;
      document.getElementById('alignmentDetectedValue').textContent =
        String(alignment.detected_count ?? 0) + ' / ' + String(alignment.reference_count ?? 0);
      document.getElementById('alignmentConfidenceValue').textContent =
        String(alignment.confidence_percent ?? 0) + '%';
      document.getElementById('alignmentDetailValue').textContent =
        alignment.message || 'Anchor alignment is waiting for calibration.';
      document.getElementById('alignmentSummaryValue').textContent = alignment.calibrated
        ? ('Saved ' + String(alignment.reference_count ?? 0) + ' anchors for ' + (data.playfield || 'this playfield') + '.')
        : 'No anchor calibration saved.';
      document.getElementById('toggleAlignmentButton').textContent =
        'Anchor Alignment: ' + onOff(alignment.enabled);
    }}
