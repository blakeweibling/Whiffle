...
from operator_remote import start_operator_remote, update_remote_status_snapshot
...
class GameState:
    ...
    def __init__(self, supabase_url: str, supabase_key: str) -> None:
        ...
        # Settings-backed startup defaults
        ...
        self.operator_remote_enabled: bool = True
        self.operator_remote_port: int = 8765
        self.operator_remote_pin: str = "1234"
        self.operator_remote_service = None
        ...
        load_settings(self)
        self.current_width, self.current_height = ResolutionConstants.RESOLUTIONS.get(
            self.current_resolution_key,
            ResolutionConstants.RESOLUTIONS[ResolutionConstants.DEFAULT_RESOLUTION],
        )
        ...
        self.effective_scoring_zones: List[Tuple[int, int, int, int, int]] = []
        ...
        self.effective_special_hole: Optional[Tuple[int, int, int, int, int]] = None
        self.drawing_points_input: str = ""
        self.zone_alignment_settings: Dict[str, Any] = {"playfields": {}}
        self.zone_alignment_enabled: bool = False
        self.zone_alignment_offset_x: int = 0
        self.zone_alignment_offset_y: int = 0
        self.zone_alignment_confidence: float = 0.0
        self.zone_alignment_detected_count: int = 0
        self.zone_alignment_reference_count: int = 0
        self.zone_alignment_active: bool = False
        self.zone_alignment_status: str = "Not calibrated"
        self.zone_alignment_last_update_time: float = 0.0
        self.zone_alignment_last_message: str = ""
        self.zone_alignment_observed_points: List[Tuple[int, int]] = []
        self.zone_alignment_last_detection_frame: int = -1
        ...
        if not GameConstants.BACKGROUND_MUSIC_TRACKS:
        ...
        ensure_zone_alignment_state(self)
        update_effective_scoring_zones(self)
        ...
        try:
            update_remote_status_snapshot(self)
            start_operator_remote(self)
        except Exception as remote_error:
            logger.warning(f"Operator remote failed to start: {remote_error}")
    ...
    def set_playfield(self, playfield: str) -> bool:
        ...
        try:
            ...
            try:
                from game_state_utils import save_settings

                save_settings(self)
            except Exception as save_error:
                logger.debug(f"Could not persist playfield selection: {save_error}")
            return True
        except Exception as e:
    ...
    def _scale_scoring_zones(self, old_w: int, old_h: int, new_w: int, new_h: int):
        ...
        scale_reference_anchor_points(self, old_w, old_h, new_w, new_h)
        if self.is_fivestar_playfield():
            self.special_hole = None
        else:
            self.special_hole = set_special_hole(self.scoring_zones)
    ...
    def set_resolution(self, new_resolution_key: str):
        ...
        try:
            ...
            logger.debug(f"Could not persist resolution change: {save_error}")
