...
from anchor_alignment import get_effective_scoring_zones, get_effective_special_hole
...
def _draw_status_badges(frame: np.ndarray, game_state: "GameState") -> None:
    """Draw lightweight status badges for recording and sync state."""
    current_width, _current_height = game_state.get_current_resolution_dimensions()
    badge_entries = []
    snapshot = getattr(game_state, "remote_status_snapshot", {}) or {}

    if getattr(game_state, "replay_recording", False):
        recording_seconds = 0
        replay_manager = getattr(game_state, "replay_manager", None)
        current_replay = getattr(replay_manager, "current_replay", None) if replay_manager else None
    ...
    if not getattr(game_state, "camera_available", True):
        ...
        alignment_enabled = bool(getattr(game_state, "zone_alignment_enabled", False))
        alignment_calibrated = int(getattr(game_state, "zone_alignment_reference_count", 0) or 0) >= 2
        alignment_active = bool(getattr(game_state, "zone_alignment_active", False))
        if alignment_active:
            ...
            badge_entries.append(
                (
                    f"Align dx={offset_x:+d} dy={offset_y:+d}",
                    UIConstants.WHITE,
                    UIConstants.GREEN,
                )
            )
        elif alignment_enabled and alignment_calibrated:
            badge_entries.append(("Align Searching", UIConstants.WHITE, UIConstants.CV2_ORANGE))
        elif alignment_calibrated:
            badge_entries.append(("Align Ready", UIConstants.WHITE, UIConstants.CV2_BLUE))
    ...
    if pending_scores > 0:
        label = f"Score Pending" if pending_scores == 1 else f"{pending_scores} Scores Pending"
        badge_entries.append((label, UIConstants.WHITE, UIConstants.CV2_ORANGE))

    active_remote_sessions = int(snapshot.get("active_remote_sessions", 0) or 0)
    ...
    start_y = 92
    for idx, (text, fg_color, bg_color) in enumerate(badge_entries):
        _optimized_draw_text(
            frame,
            text,
            (start_x, start_y + idx * 28),
...
def draw_ui(frame: np.ndarray, game_state: "GameState") -> None:
    ...
    if game_state.current_state not in [CurrentGameState.GAME_OVER]:
        ...
        _draw_status_badges(frame, game_state)
    if game_state.current_state == CurrentGameState.PLAYING:
        ...
        if game_state.show_scoring_zones:
            draw_scoring_zones(
                frame,
                get_effective_scoring_zones(game_state),
                get_effective_special_hole(game_state),
            )
