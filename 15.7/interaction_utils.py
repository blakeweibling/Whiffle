...
from anchor_alignment import (
    ...
    ensure_zone_alignment_state,
    set_reference_anchor_points,
    update_effective_scoring_zones,
)
...
def _refresh_zone_alignment_after_zone_change(game_state: "GameState") -> None:
    """Rebuild the effective zone set after any base-zone edit."""
    ensure_zone_alignment_state(game_state)
    update_effective_scoring_zones(game_state)


def _sync_special_hole_and_effective_zones(game_state: "GameState") -> None:
    """Keep special-hole state and runtime effective zones in sync with base zones."""
    ...
    _refresh_zone_alignment_after_zone_change(game_state)


def _calibrate_zone_anchors(game_state: "GameState") -> bool:
    """Capture the current marker layout as the anchor calibration for this playfield."""
    current_frame = getattr(game_state, "current_frame", None)
    ok, message = calibrate_anchor_reference(game_state, current_frame)
    if not ok:
...
def _toggle_zone_alignment(game_state: "GameState") -> bool:
    """Enable or disable runtime anchor-based alignment."""
    ensure_zone_alignment_state(game_state)
    reference_points = getattr(game_state, "zone_alignment_reference_count", 0)
    if not getattr(game_state, "zone_alignment_enabled", False) and reference_points < 2:
        show_notification(
            game_state,
            "Calibrate anchors before enabling alignment.",
            is_error=True,
            duration=3.0,
        )
        return False
    ...
    save_settings(game_state)
    game_state.menu_cache = None
    state_label = "enabled" if game_state.zone_alignment_enabled else "disabled"
    show_notification(game_state, f"Anchor alignment {state_label}", duration=1.8)
    return True


def _clear_zone_anchor_calibration(game_state: "GameState") -> bool:
    """Remove saved anchor references for the current playfield."""
    ensure_zone_alignment_state(game_state)
    ...
    save_settings(game_state)
    game_state.menu_cache = None
    show_notification(game_state, "Anchor calibration cleared", duration=1.8)
    return True
        ...
        if event == cv2.EVENT_LBUTTONUP:
            if getattr(game_state, "drag_start_pos", None) and getattr(
                ...
                if not valid:
                    ...
                    _sync_special_hole_and_effective_zones(game_state)
        ...
        if (
            ...
            if isinstance(action, str):
                ...
                if current_state == CurrentGameState.MENU:
                    ...
                    elif action == "calibrate_anchors":
                        return _calibrate_zone_anchors(game_state)

                    elif action == "toggle_zone_alignment":
                        return _toggle_zone_alignment(game_state)

                    elif action == "clear_anchor_calibration":
                        return _clear_zone_anchor_calibration(game_state)
