...
from game_constants import UIConstants, GameConstants, ScoringConstants, PlayerConstants
...
def _handle_input(game_state: Any) -> Optional[int]:
    ...
    if key != -1 and key != 255: # Ignore no-input/-1 and potential 255 spam
                ...
                if not entered_name:
                    ...
                    try:
                        if game_state.players:
                            ...
                            # Start music only after name confirmed and state is PLAYING
                            if (
                                game_state.background_music_on
        ...
        # --- Handle input for MENU state ---
                ...
                if key == 13: # Enter Key - Save Points
                    ...
                    try:
                        ...
                        if not new_points_str:
                            ...
                            # Use constant for max points validation
                        ...
                        # Reset editing state on unexpected error
                        game_state.editing_zone_index = None
                        game_state.editing_zone_mode = None
                        game_state.editing_zone_points_input = None
                        game_state.menu_cache = None # Invalidate cache
                    ...
                    if game_state.editing_zone_points_input is None:
                        game_state.editing_zone_points_input = ""
                    # Limit length based on MAX_POINTS (e.g., 3 digits for 999)
            ...
            if not key_handled_globally: # Check if not handled by specific editing modes
                if key == ord("m"): # Toggle menu OFF (Resume)
                    ...
                    # Handle back from submenus
                        ...
                        game_state.menu_cache = None
                        key_handled_globally = True
                    elif game_state.submenu_active: # Back from any other submenu to main menu
                        game_state.submenu_active = None
                        game_state.menu_cache = None
                        key_handled_globally = True
        ...
        # --- Handle input for PLAYING state ---
            ...
            if game_state.drawing:
                if ord('0') <= key <= ord('9'):
                    ...
                    if len(game_state.drawing_points_input) < 3:
                        ...
                        logger.debug(f"Drawing points input: {game_state.drawing_points_input}")
                    else:
                         game_state.show_notification(f"Max 3 digits for points", duration=1.0)
                    key_handled_in_playing = True
                elif key == 8: # Backspace
                    if game_state.drawing_points_input:
                        game_state.drawing_points_input = game_state.drawing_points_input[:-1]
                        logger.debug(f"Drawing points input (backspace): {game_state.drawing_points_input}")
                    key_handled_in_playing = True
            ...
            if not key_handled_in_playing:
                if key == ord("m"): # Toggle menu ON
                    ...
                    if game_state.drawing:
                        logger.info("Drawing mode enabled. Click and drag to draw zone. Enter digits for points.")
                    ...
                    game_state.show_notification("Game Paused", duration=0) # Persistent
                    key_handled_in_playing = True # Mark as handled
        ...
        # --- *** NEW: Handle input for ZONE_EDITING state *** ---
        elif game_state.current_state == CurrentGameState.ZONE_EDITING:
            if key == 27: # Escape Key - Cancel Edit Action
                logger.info("Escape key (27) detected during ZONE_EDITING.")
                zone_idx = game_state.selected_zone_for_edit
                # Revert the zone to its original state if it was being dragged/resized
                if game_state.original_zone_on_drag_start and zone_idx is not None and 0 <= zone_idx < len(game_state.scoring_zones):
                    logger.info(f"Reverting zone {zone_idx+1} to its original state.")
                    game_state.scoring_zones[zone_idx] = game_state.original_zone_on_drag_start
                    # Update special hole if necessary after revert
                    game_state.special_hole = set_special_hole(game_state.scoring_zones)
                else:
                    logger.debug("No original zone state to revert to, or invalid index.")
                ...
                # Return to previous state (should be MENU)
                previous_state = game_state.previous_state or CurrentGameState.MENU
                game_state.current_state = previous_state
                game_state.previous_state = None # Clear previous state storage
                game_state.show_notification("Zone Edit Cancelled") # Inform user
                logger.info(f"Zone edit cancelled. Returning to state: {previous_state}")
                key_handled_globally = True
            # NOTE: No Enter key handling needed here, mouse up confirms
