...
from constants import UIConstants, ScoringConstants  # Updated to class-based imports
...
FONT_SCALE: float = UIConstants.FONT_SCALE_MEDIUM  # Updated from 0.6 to use UIConstants
...
def _handle_mouse_event(event: int, x: int, y: int, drawing_state: dict) -> None:
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        drawing_state["temp_zone"] = (drawing_state["start"][0], drawing_state["start"][1], 
                                      x - drawing_state["start"][0], y - drawing_state["start"][1])
...
def _draw_existing_zones(overlay: np.ndarray, scoring_zones: List[Tuple[int, int, int, int, int]]) -> None:
    """Draw existing scoring zones onto a static overlay."""
    for zone in scoring_zones:
        x, y, w, h, points = zone
        cv2.rectangle(overlay, (x, y), (x + w, y + h), UIConstants.GREEN, FONT_THICKNESS)  # Updated
        text_x, text_y = _get_text_position(x, y, w, h, overlay.shape[1], overlay.shape[0])
        cv2.putText(overlay, f"{points} pts", (text_x, text_y),
                    cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, UIConstants.GREEN, FONT_THICKNESS)  # Updated

def _zones_overlap(new_zone: Tuple[int, int, int, int], existing_zones: List[Tuple[int, int, int, int, int]]) -> bool:
    """Check if the new zone overlaps with any existing zones."""
    x1, y1, w1, h1 = new_zone
    for x2, y2, w2, h2, _ in existing_zones:
        if not (x1 + w1 < x2 or x2 + w2 < x1 or y1 + h1 < y2 or y2 + h2 < y1):
            logger.warning(f"New zone ({x1}, {y1}, {w1}, {h1}) overlaps with existing zone ({x2}, {y2}, {w2}, {h2})")
            return True
    return False
...
def _run_drawing_loop(
    ...
    while True:
        ...
        temp_frame = cv2.addWeighted(temp_frame, 0.8, overlay, 0.2, 0)  # Composite overlay

        if drawing_state["temp_zone"]:
            x, y, w, h = drawing_state["temp_zone"]
            cv2.rectangle(temp_frame, (x, y), (x + w, y + h), UIConstants.YELLOW, FONT_THICKNESS)  # Updated
            points_display = drawing_state["points"] if drawing_state["points"] else str(DEFAULT_POINTS)
            ...
            cv2.putText(temp_frame, f"{points_display} pts", (text_x, text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, UIConstants.YELLOW, FONT_THICKNESS)  # Updated
        ...
        cv2.putText(temp_frame, instructions, (TEXT_OFFSET_X, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE, UIConstants.WHITE, FONT_THICKNESS)  # Updated

        cv2.imshow(UIConstants.WINDOW_NAME, temp_frame)  # Updated

        key = cv2.waitKey(1) & 0xFF
        if key == 13 and drawing_state["temp_zone"] and not drawing_state["drawing"]:  # Enter
            if _zones_overlap(drawing_state["temp_zone"], scoring_zones):
                logger.warning("Zone overlaps with existing zone, cancelling")
                return None
            points = DEFAULT_POINTS if not drawing_state["points"] else int(drawing_state["points"])
            points = max(1, min(points, MAX_POINTS))
            zone = (drawing_state["temp_zone"][0], drawing_state["temp_zone"][1], 
                    drawing_state["temp_zone"][2], drawing_state["temp_zone"][3], points)
...
def define_scoring_zone(
    ...
    drawing_state = {"drawing": False, "start": (-1, -1), "temp_zone": None, "points": ""}
    cv2.setMouseCallback(UIConstants.WINDOW_NAME, lambda e, x, y, f, p: _handle_mouse_event(e, x, y, drawing_state))  # Updated
...
def draw_scoring_zones(frame: np.ndarray, scoring_zones: List[Tuple[int, int, int, int, int]]) -> None:
    ...
    for zone in scoring_zones:
        x, y, w, h, points = zone
        cv2.rectangle(frame, (x, y), (x + w, y + h), UIConstants.GREEN, FONT_THICKNESS)  # Updated
