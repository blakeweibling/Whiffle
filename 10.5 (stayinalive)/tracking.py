...
def _filter_old_balls(
    ...
    tracked_balls[:] = [ball for ball in tracked_balls 
                       if frame_count - ball[4] < TrackingConstants.MAX_AGE_FRAMES]
    dropped = initial_count - len(tracked_balls)
    if dropped > 0:
        logger.info(f"Dropped {dropped} balls due to age exceeding {TrackingConstants.MAX_AGE_FRAMES} frames")
...
def _match_balls(
    ...
    used_tracked_indices: List[int] = []  # Track which tracked balls have been matched
    ...
    if SCIPY_AVAILABLE:
        ...
        distances, indices = tree.query(new_positions, distance_upper_bound=distance_threshold)

        for i, (dist, idx) in enumerate(zip(distances, indices)):
            if dist != float('inf') and idx < len(tracked_balls) and idx not in used_tracked_indices:
                ...
                tracked_balls[idx] = (new_x, new_y, radius, ball_id, age, ball_type)  # Update position and type
                if debug_mode:
                    logger.debug(f"Matched ball ID {ball_id} at ({new_x}, {new_y}) with distance {dist}, type {ball_type}")
            else:
                if debug_mode:
                    logger.debug(f"No match found for new ball at ({new_balls[i][0]}, {new_balls[i][1]}) within {distance_threshold}, distance={dist}")
    else:
        ...
        for i, (new_x, new_y, radius, ball_type) in enumerate(new_balls):
            ...
            if closest_ball_idx is not None:
                tracked_x, tracked_y, _, ball_id, age, tracked_type = tracked_balls[closest_ball_idx]
                matched_balls.append((new_x, new_y, radius, ball_id, ball_type))
                matched_new_indices.append(i)
                used_tracked_indices.append(closest_ball_idx)
                tracked_balls[closest_ball_idx] = (new_x, new_y, radius, ball_id, age, ball_type)
                if debug_mode:
                    logger.debug(f"Matched ball ID {ball_id} at ({new_x}, {new_y}) with distance {min_dist**0.5}, type {ball_type}")
            else:
                if debug_mode:
                    logger.debug(f"No match found for new ball at ({new_x}, {new_y}) within {distance_threshold}")
...
def _is_scored_recently(
    ...
    for (sx, sy), _ in scored_positions.items():
        ...
        if dist_squared < threshold ** 2:
            logger.debug(f"Position ({new_x}, {new_y}) matches scored position ({sx}, {sy})")
            return True
    return False
...
def _add_new_balls(
    ...
    for i, (new_x, new_y, radius, ball_type) in enumerate(new_balls):
        if i in matched_new_indices:
            continue
        if not _is_scored_recently(new_x, new_y, scored_positions, TrackingConstants.SCORED_DISTANCE_THRESHOLD):
            tracked_balls.append((new_x, new_y, radius, next_ball_id, frame_count, ball_type))
            tracked_detected_balls.append((new_x, new_y, radius, next_ball_id, ball_type))
            if debug_mode:
                logger.debug(f"New ball ID {next_ball_id} at ({new_x}, {new_y}), type {ball_type}")
            next_ball_id += 1
...
def track_balls(
    ...
    if not new_balls:
        ...
        return [(x, y, radius, ball_id, ball_type) 
                for x, y, radius, ball_id, _, ball_type in tracked_balls], next_ball_id

    if not tracked_balls:
        logger.debug("No tracked balls to match against")

    _filter_old_balls(tracked_balls, frame_count)
    matched_balls, matched_new_indices = _match_balls(new_balls, tracked_balls, 
                                                     TrackingConstants.TRACKING_DISTANCE_THRESHOLD,
                                                     debug_mode)
    tracked_detected_balls, next_ball_id = _add_new_balls(new_balls, tracked_balls, matched_new_indices, 
                                                         next_ball_id, scored_positions, frame_count, debug_mode)

    logger.info(f"Tracked {len(tracked_detected_balls)} balls in frame {frame_count}")
    if debug_mode:
        logger.debug(f"Tracked balls: {[(x, y, radius, ball_id, ball_type) for x, y, radius, ball_id, ball_type in tracked_detected_balls]}")
...
class BallTracker:
    def track_balls(
        ...
        new_balls = [(x, y, radius, "white") for x, y, radius in white_balls] + \
                    [(x, y, radius, "red") for x, y, radius in red_balls] + \
                    [(x, y, radius, "half") for x, y, radius in half_balls]

        return track_balls(new_balls, tracked_balls, next_ball_id, frame_count, scored_positions, debug_mode)