    ...
    if zone_idx is None or not (0 <= zone_idx < len(game_state.scoring_zones)):
        ...
        try:
            game_state.current_state = (game_state.previous_state
                                        if game_state.previous_state else
        ...
        if game_state.drag_start_pos and game_state.zone_editing_action:
            ...
            if fw < min_size or fh < min_size:
                show_notification(game_state,
                                  "Zone too small! Reverted.",
                                  is_error=True,
                                  duration=3.0)  # Use helper
                ...
                if _zones_overlap(final_zone[:4], other_zones):
                    ...
                    game_state.special_hole = set_special_hole(
                        game_state.scoring_zones)  # Use helper
            game_state.zone_editing_action = None
            game_state.drag_start_pos = None
            game_state.original_zone_on_drag_start = None
            handled = True
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if game_state.drawing:
            if game_state.temp_zone:
                ...
                if (w > ScoringConstants.MIN_ZONE_SIZE
                    ...
                    try:
                        ...
                        if points_str:
                            show_notification(
                                game_state,
                                "Invalid points. Using default.",
                                is_error=True,
                                duration=3.0,
                            )  # Use helper
                    new_zone = (x1, y1, w, h, points)
                    if not _zones_overlap(new_zone[:4],
                        ...
                        show_notification(
                            game_state,
                            f"Zone Added ({points} pts)")  # Use helper
                    else:
                        show_notification(game_state,
                                          "Zone Overlaps!",
                                          is_error=True)  # Use helper
                else:
                    show_notification(game_state,
                                      "Zone too small",
                                      is_error=True)  # Use helper
...
def _process_menu_or_gameover_click(x: int, y: int,
                                    game_state: GameState) -> bool:
    ...
    if not all(
            ...
            for attr in ["menu_pos", "menu_width", "menu_height"]):
        return False
    ...
    main_menu_actions = {
        item[1]
        for item in MenuConstants.MAIN_MENU_ITEMS if isinstance(item[1], str)
    }
    ...
    known_submenu_nav_actions = main_menu_actions.union(
        zone_submenu_actions) - {
            "resume",
            "quit",
            "back_to_main",
                    ...
                    if game_state.current_state == CurrentGameState.MENU:
                        if action == "toggle_debug_overlay":
                            ...
                            (change_music_track(
                                ...
                                total_tracks,
                            ) if total_tracks > 0 else None)
                            game_state.menu_cache = None  # Use util
                            ...
                            show_notification(
                                game_state, "Press 's', then click and drag")
                            game_state.current_state = CurrentGameState.PLAYING
                            game_state.submenu_active = None
                            _reset_all_menu_editing_states(
                                game_state)  # Use helper
                            ...
                            game_state.menu_cache = None  # Use helper
                        elif action == "load_zones":
                            load_zones(game_state)
                            _reset_all_menu_editing_states(
                                game_state)  # Use helper
                        # --- START CHANGE: Handle set_mode_retro ---
                            ...
                            if (new_mode in valid_modes
                                    and game_state.game_mode != new_mode):
                                save_score(
                                    game_state,
                                    game_state.get_current_player().name,
                        ...
                        # --- END CHANGE ---
                            ...
                            try:  # Keep try block
                                ...
                                if (0 <= index < len(game_state.players)
                                    ...
                                    save_score(game_state,
                                               game_state.get_current_player().name)  # Uses helper
                                ...
                                logger.error(
                                    f"Error processing player selection action '{action}': {e}"
                                )
                            ...
                            try:  # Keep try block
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    if not (game_state.editing_zone_index
                                        ...
                                        game_state.menu_cache = None
                                        logger.info(
                                            f"Selected zone {index+1} for points edit. Value: {game_state.scoring_zones[index][4]}"
                                        )
                                else:
                                    logger.warning(
                                        f"Invalid zone index: {index}")
                                    _reset_all_menu_editing_states(game_state)
                            except Exception as e:  # Add except block back
                                logger.error(
                                    f"Error processing zone points edit action '{action}': {e}"
                                )
                                _reset_all_menu_editing_states(game_state)
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    show_notification(
                                        game_state,
                                        "Click inside zone to move, ESC cancel",
                                        duration=0,
                                    )  # Use helper
                                else:
                                    logger.warning(
                                ...
                                logger.error(
                                    f"Error processing move zone action '{action}': {e}"
                                )
                            # --- End CORRECTED Block ---
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    show_notification(
                                        game_state,
                                        "Click corner handles to resize, ESC cancel",
                                        duration=0,
                                    )  # Use helper
                                else:
                                    logger.warning(
                                ...
                                logger.error(
                                    f"Error processing resize zone action '{action}': {e}"
                                )
                            # --- End CORRECTED Block ---
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.players):
                                    if not (game_state.editing_player_index
                                            == index
                                        ...
                                        game_state.menu_cache = None
                                        logger.info(
                                            f"Selected player {index+1} name edit. Value: '{game_state.players[index].name}'"
                                        )
                                else:
                                    logger.warning(
                                        f"Invalid player index: {index}")
                                ...
                                logger.error(
                                    f"Error processing edit player name action '{action}': {e}"
                                )
                                _reset_all_menu_editing_states(game_state)
                        # --- End CORRECTED Block ---
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    if (game_state.editing_zone_index == index
                                            and game_state.editing_zone_mode
                                            == "confirm_delete"):
                                        ...
                                        show_notification(
                                            game_state,
                                            f"Click Delete again for zone {index+1}",
                                            duration=4.0,
                                        )  # Use helper
                                else:
                                    logger.warning(
                                ...
                                logger.error(
                                    f"Error processing delete zone action '{action}': {e}"
                                )
                                _reset_all_menu_editing_states(game_state)
                            # --- End CORRECTED Block ---
                            ...
                            logger.warning(f"Unhandled MENU action: {action}")
                        ...
                        if action == "new_game_from_gameover":
                            ...
                            game_state.win_condition_met = False  # Use util
                            ...
                            logger.warning(
                                f"Unhandled GAME_OVER action: {action}")
                    return True
    if volume_adjusted:
        return True
    return False
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    if (not click_handled
            ...
            and event == cv2.EVENT_LBUTTONDOWN):
        if (UIConstants.MENU_BUTTON_X <= x <=
                UIConstants.MENU_BUTTON_X + UIConstants.MENU_BUTTON_WIDTH
                and UIConstants.MENU_BUTTON_Y <= y <=
