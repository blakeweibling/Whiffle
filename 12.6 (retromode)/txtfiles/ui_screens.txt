...
def _draw_game_over_screen(frame: np.ndarray, game_state: GameState) -> None:
    ...
    if isinstance(game_over_splash_cache, np.ndarray):
        ...
        title_color = (UIConstants.GREEN
                       if game_state.win_condition_met else UIConstants.RED)
    ...
    try:
        ...
        if not game_state.camera_available:
            if game_state.static_frame is None:
                logger.error("No Camera/Static Frame. Exiting.")
                return None
        ...
        # --- Start CORRECTED Block ---
            ...
            try:
                logger.debug(
                    "Attempting to destroy window after init failure (no game_state)..."
                )
                ...
                logger.error(
                    f"Error destroying window during init failure cleanup: {destroy_err}"
                )
        # --- End CORRECTED Block ---
    ...
    while (time.time() - start_time
        ...
        try:
            if splash.shape != first_frame.shape:
                ...
                elif (elapsed < GameConstants.SPLASH_DURATION +
                      GameConstants.FADE_DURATION):
                    alpha = (max(
                        0.0,
                        1.0 - (elapsed - GameConstants.SPLASH_DURATION) /
                        GameConstants.FADE_DURATION,
            ...
            logger.exception(f"Error during splash fade loop: {e}")
