...
class GameState:
    def __init__(self, supabase_url: str, supabase_key: str) -> None:
        ...
        self.debug_mode: bool = False  # Disabled by default to prevent debug windows
        ...
        # Load scoring zones, passing self as game_state (Fix for TypeError)
    ...
    def update_score(self, frame: np.ndarray, tracked_detected_balls: List[Tuple[int, int, float, int, str]]) -> None:
        ...
        for x, y, radius, ball_id, ball_type in tracked_detected_balls:
            ...
            for zone in self.scoring_zones:
                if is_in_scoring_zone(ball, zone):
                    current_zone = zone
                    # Check if this zone is the special hole
                    if self.special_hole and zone == self.special_hole:
                        self.special_hole_scored = True
                        logger.info(f"Ball ID {ball_id} landed in special hole {self.special_hole}, will double score at end of session")
                    break
