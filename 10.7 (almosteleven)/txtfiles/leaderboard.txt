...
class Leaderboard:
    ...
    def __init__(self, supabase_url: str, supabase_key: str) -> None:
        ...
        self.pending_scores: List[Dict[str, Any]] = []  # Queue for batch updates (Change 4)
    ...
    def _post_supabase(self, data: List[Dict[str, Any]], retries: int = 3, delay: float = 1) -> None:
        """
    ...
    def submit_score(self, player_name: str, score: int, mode: str) -> bool:
        ...
        self.pending_scores.append(score_entry)  # Queue for batch submission (Change 4)
        logger.info(f"Score queued for batch submission: {player_name} - {score} ({mode})")
    ...
    def flush_pending_scores(self, retries: int = 3, delay: float = 1) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully submitted {len(self.pending_scores)} scores to online leaderboard")
            self.pending_scores.clear()
        except requests.RequestException:
            logger.warning(f"Failed to submit {len(self.pending_scores)} scores to online leaderboard, keeping in queue")
