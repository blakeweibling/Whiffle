...
def run_game_loop(game_state: Any) -> None:
    ...
    while True:
        ...
        if _check_window_close(game_state):
            break  # Exit the loop immediately if the window is closed
        ...
        try:
            frame = cv2.resize(frame, (UIConstants.WINDOW_WIDTH, UIConstants.WINDOW_HEIGHT))
        except cv2.error as e:
        ...
        if key is None:
            break  # Exit signal received (e.g., 'q' key or menu quit)
