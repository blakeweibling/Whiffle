...
def calculate_session_stats(session_data: SessionData) -> Dict[str, Any]:
    ...
    if not session_data:
        logger.warning(
            "calculate_session_stats called with None session_data.")
        return {}
    ...
    # Score Rate
    duration_min = (stats["duration_seconds"] /
                    60.0 if stats["duration_seconds"] > 0 else 0)
    stats["score_rate_per_min"] = ((stats["total_score"] /
        ...
        for event in session_data.score_events:
            try:
                ...
                if isinstance(points, (int, float)):
                    ...
                    logger.warning(
                        f"Invalid points type in score event: {event}")
    ...
    sorted_zones = sorted(points_by_zone.items(),
...
if __name__ == "__main__":
    ...
    class MockSessionData:
        ...
        def __init__(self):
            ...
            ) - 125  # Session started 125 seconds ago
    ...
    print(
        f"Duration: {calculated_stats.get('duration_seconds', 0):.1f} seconds")
    print(f"Total Score: {calculated_stats.get('total_score', 0)}")
    print(
        f"Score Rate: {calculated_stats.get('score_rate_per_min', 0):.1f} points/min"
    )
    print(
        f"Points by Ball Type: {calculated_stats.get('points_by_ball_type', {})}"
    )
    print(
        f"Top 3 Zones (Zone ID, Points): {calculated_stats.get('top_3_zones', [])}"
    )
