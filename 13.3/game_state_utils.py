    ...
    return [
        ...
        Achievement(
            ...
            lambda gs: hasattr(gs, "players") and gs.players and hasattr(
                gs.players[gs.current_player_index], "games_played") and gs.
            players[gs.current_player_index].games_played >= 10,
        ),
...
def reset_game(game_state: Any) -> None:
    ...
    # --- Randomize music track --- (Keep this behavior)
    available_tracks = getattr(GameConstants, "BACKGROUND_MUSIC_TRACKS", [])
    if available_tracks:
        num_tracks = len(available_tracks)
        new_track_index = random.randint(0, num_tracks - 1)
        current_track_index = getattr(game_state, "selected_music_track_index",
                                      -1)
        ...
        logger.info(
            f"Resetting music track randomly to index: {new_track_index}")
        # change_music_track handles stopping old, loading new, setting volume/playing
