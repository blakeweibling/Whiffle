# Whiffle Playfield - Removed ROI, Fixed Double-Counting
# Lines 1-700
import cv2
import numpy as np
import pygame
            ...
            for contour in contours:
                ...
                if area > 0.1 * total_pixels and area < total_pixels:  # Reasonable ball size
                    ...
                    if len(approx) > 4:  # Loose check for circularity
                        if pos not in game_state.scored_balls:  # Only count if not previously scored
                            ball_positions.append(pos)
                            points_list.append(points)
                            game_state.scored_balls.add(pos)  # Mark this position as scored
                            game_state.detection_cooldown[pos] = frame_count  # Start cooldown
                        break
...
while running and game.running:
    ...
    # Use raw frame directly (ROI processing removed)
    roi = frame
