...
def _draw_edit_zones_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    if not game_state.scoring_zones:
        ...
        for i_display, zone_data in enumerate(zones_to_display):
            ...
            x_z, y_z, w_z, h_z, points = zone_data # Use zone_data from the paginated list
            ...
            if game_state.selected_zone_for_edit == original_index: # Check original index
            ...
            edit_color = (
                UIConstants.GREEN
                if game_state.editing_zone_index == original_index # Check original index
                and game_state.editing_zone_mode == "edit_points"
                else UIConstants.CV2_BLUE
            )
            _draw_button(
                menu_frame, button_x, y_offset, button_width, item_height, "Pts", edit_color, UIConstants.FONT_SCALE_SMALL
            )
            game_state.submenu_items.append(
                (edit_rect, f"edit_zone_{original_index}", f"Edit Zone {original_index+1} Points")
            )
            ...
            is_moving_this_zone = (
                ...
                and game_state.selected_zone_for_edit == original_index # Check original index
            ...
            _draw_button(
                menu_frame, button_x, y_offset, button_width, item_height, "Move", move_color, UIConstants.FONT_SCALE_SMALL
            )
            game_state.submenu_items.append(
                (move_rect, f"move_zone_{original_index}", f"Move Zone {original_index+1}")
            )
            button_x += button_width + button_spacing
            ...
            _draw_button(
                menu_frame, button_x, y_offset, button_width, item_height, "Resize", resize_color, UIConstants.FONT_SCALE_SMALL
            )
            game_state.submenu_items.append(
                (resize_rect, f"resize_zone_{original_index}", f"Resize Zone {original_index+1}")
            )
            button_x += button_width + button_spacing
            ...
            _draw_button(
                menu_frame, button_x, y_offset, button_width, item_height, "Del", delete_color, UIConstants.FONT_SCALE_SMALL
            )
            game_state.submenu_items.append(
                (delete_rect, f"delete_zone_{original_index}", f"Delete Zone {original_index+1}")
            )
    ...
    if prev_enabled:
        game_state.submenu_items.append(
            ((prev_button_x, page_y_start, page_button_width, page_item_height), "prev_edit_zone_page", "Previous Page")
        )
    ...
    if next_enabled:
        game_state.submenu_items.append(
            ((next_button_x, page_y_start, page_button_width, page_item_height), "next_edit_zone_page", "Next Page")
        )

    y_offset = page_y_start + page_item_height + 5 # Update y_offset after page controls
    ...
    game_state.submenu_items.append(
        ((20, back_y, menu_frame.shape[1] - 40, back_button_height), "back_to_manage_zones", "Back")
    )
    game_state.menu_height = back_y + back_button_height + 20 # Adjust height based on content