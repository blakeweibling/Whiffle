# heatmap_utils.py
"""
Utility functions for generating and managing heatmaps.
"""
...
def generate_heatmap(
    ...
    if not session_data.has_position_data:
    ...
    if isinstance(session_data.ball_position_history, dict):
        for ball_id, history in session_data.ball_position_history.items():  #
            # Extract just x,y from the (x,y,timestamp) tuples
    ...
    if not all_positions:  #
        logger.info("Heatmap generation skipped: No valid ball positions found.")  #
        return None  #
    ...
    try:
        heatmap_blurred = cv2.GaussianBlur(heatmap_raw, HEATMAP_BLUR_KERNEL_SIZE, 0)  #
