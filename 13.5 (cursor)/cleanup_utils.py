...
def clean_exit(
        ...
        game_state: Optional[
            Any] = None,  # Expecting a GameState object or similar
) -> None:
    """
    ...
    if game_state:
        try:
            ...
            if (player_name != "Unknown Player"
                    and current_state != CurrentGameState.
                    GAME_OVER  # Don't re-save if game over already handled it
                    and score_val > 0):
                logger.info(
                    f"Saving score ({score_val}) for {player_name} on exit...")
                try:
                    ...
                    logger.error(
                        f"Error calling save_score during cleanup: {e}")
            ...
            try:
                ach_file = getattr(GameConstants, "ACHIEVEMENTS_FILE",
            ...
            if hasattr(game_state, "data_logger") and game_state.data_logger:
                try:
                    ...
                    game_state.data_logger.end_current_session(
                        final_score, zones)
                    # Note: end_current_session in DataLogger handles saving the history file
                    logger.info(
                        "Current session data finalized and history saved.")
                except Exception as e:
                    logger.error(
            ...
            logger.exception(
                f"Error during game state saving/flushing on exit: {e}")
    ...
    if cap and hasattr(
            cap, "release"):  # Check if cap exists and has release method
        try:
        ...
        logger.warning(
            "Camera object provided but seems invalid (no release method?).")
    else:
