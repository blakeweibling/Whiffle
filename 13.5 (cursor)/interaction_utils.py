...
def _process_menu_or_modal_click(x: int, y: int, game_state: "GameState") -> bool:
                        ...
                        if (
                            ...
                            if new_mode == "retro":
                                ...
                                try:
                                    ...
                                    retro_track_index = available_tracks.index(
                                        target_track
                                    )
                                    logger.info(
                                        f"Explicitly changing music for Retro mode switch to: {retro_track_index} ({target_track})"
                                    )
                                    change_music_track(game_state, retro_track_index)
                                except ValueError:
                                    logger.error(
                                        f"Could not find '{target_track}' for retro mode switch. Music will be randomized by reset_game."
                                    )
                                except Exception as e_music:
                                    logger.error(
                                        f"Error changing music track during retro mode switch: {e_music}"
                                    )
                            reset_game(game_state)
                            show_notification(
                                game_state, f"Mode set to: {new_mode.capitalize()}"
                            )
                            game_state.current_state = CurrentGameState.PLAYING
                            game_state.submenu_active = None
                            _reset_all_menu_editing_states(game_state)
                    # Add rest of menu actions
                        ...
                        if len(getattr(game_state, "players", [])) < 2:
                            ...
                            show_notification(
                                game_state, "Maximum players reached", is_error=True
                            )
                        _reset_all_menu_editing_states(game_state)
                        ...
                        # Handle zone-related actions
                        if action.startswith(
                            ("edit_zone_", "move_zone_", "resize_zone_", "delete_zone_")
                        ):
                            try:
                                if action.startswith("edit_zone_"):
                                    ...
                                    index = int(action.split("_zone_")[-1])
                                    if (
                                        0
                                        <= index
                                        < len(getattr(game_state, "scoring_zones", []))
                                        ...
                                        msg = (
                                            ...
                                            else "Click & drag corner handles. ESC=Cancel."
                                        )
                                        show_notification(game_state, msg, duration=0)
                                ...
                                logger.error(
                                    f"Error processing zone action '{action}': {e}"
                                )
                        # Handle pagination
                        elif action in ("prev_edit_zone_page", "next_edit_zone_page"):
                            if action == "prev_edit_zone_page":
                                if (
                                ...
                                if (
                                    ...
                                    game_state.edit_zones_current_page += 1
                                    game_state.menu_cache = None
                        # Handle leaderboard modes
                        elif action.startswith("leaderboard_"):
                            game_state.leaderboard_mode = action.split("leaderboard_")[
                                1
                            ]
                            game_state.menu_cache = None
                        else:
    ...
    if volume_adjusted:
        return True  # Indicate click was handled by volume adjustment
