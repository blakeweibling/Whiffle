...
def load_background_music(game_state: Any,
    ...
    sound_path_base = getattr(GameConstants, "SOUND_EFFECTS_PATH", "sounds/")
...
def reset_game(game_state: Any) -> None:
    """
    Reset the game state fully, ending the current stats session
    and starting a new one. Preserves music selection and volume levels.
    """
    logger.info("Resetting game state via game_state_utils...")

    # --- >>> ADDED: End current data logging session <<< ---
    if hasattr(game_state, "data_logger") and game_state.data_logger:
        try:
            # Pass the final score and zone definitions before they are cleared
            ...
            logger.error(f"Error ending data logger session during reset: {e}")
    # --- >>> END ADDED <<< ---

    # Store current settings that should persist across resets
    current_game_mode = getattr(game_state, "game_mode", "classic")
    # Persist volume/music settings by NOT resetting them here. load_settings handles loading.
    ...
    attrs_to_clear = [
        "tracked_balls",
        "scored_balls",  # legacy?
        "scored_positions",  # legacy?
        "balls_in_zone",  # legacy?
        "ball_scored_zones",
        "ball_states",
        "previous_ball_states",
        "ball_positions_history",  # Short term history for rest check
        "ball_zone_history",  # Short term history for stability check
        "zone_cooldown",
        "active_trails",
        "active_explosions",
        "submenu_items",  # Clear clickable menu items
    ]
    for attr_name in attrs_to_clear:
        if hasattr(game_state, attr_name):
            target = getattr(game_state, attr_name)
            try:
                ...
                logger.error(f"Error clearing attribute '{attr_name}': {e}")
    ...
    game_state.current_session_stats = None  # Clear calculated stats display

    # Reset menu/editing state
    _reset_all_menu_editing_states(game_state)  # Use internal helper

    # Reset player score (assumes player object persists or is handled)
    if (hasattr(game_state, "players") and game_state.players
            and hasattr(game_state, "current_player_index") and
            0 <= game_state.current_player_index < len(game_state.players)):
        try:
            current_player = game_state.players[
                game_state.current_player_index]
            if hasattr(current_player, "reset_score"):
                current_player.reset_score()
            # Optionally increment games played here if desired
            # if hasattr(current_player, 'increment_games_played'):
            #     current_player.increment_games_played()
        except Exception as e:
            logger.error(
                f"Error resetting score for player {game_state.current_player_index}: {e}"
            )

    # Reset timer based on the (potentially just changed) game mode
    if current_game_mode == "timed":
    ...
    # Reload initial state (zones, high score for the current mode)
    # Volume/Music settings loaded earlier via load_settings() should persist
    load_initial_state(game_state)  # Reloads zones, high score

    # --- Randomize/Set music track --- # --- >>> MODIFIED SECTION START <<< ---
    available_tracks = getattr(GameConstants, "BACKGROUND_MUSIC_TRACKS", [])
    if available_tracks:
        num_tracks = len(available_tracks)
        new_track_index = -1 # Default to invalid index

        # Check if the game mode being reset *to* is retro
        if current_game_mode == "retro":
            try:
                target_track = "background_music4.mp3"
                new_track_index = available_tracks.index(target_track)
                logger.info(f"Resetting music track specifically for Retro mode to index: {new_track_index} ({target_track})")
            except ValueError:
                logger.error(f"Could not find '{target_track}' in BACKGROUND_MUSIC_TRACKS. Will randomize instead.")
                new_track_index = -1 # Signal to randomize below
            except Exception as e_find:
                 logger.error(f"Error finding retro track index: {e_find}. Will randomize.")
                 new_track_index = -1

        # If not retro mode, or retro track not found, randomize
        if new_track_index == -1:
            new_track_index = random.randint(0, num_tracks - 1)
            current_track_index = getattr(game_state, "selected_music_track_index", -1)
            # Avoid repeating if possible
            if num_tracks > 1 and new_track_index == current_track_index:
                new_track_index = (new_track_index + 1) % num_tracks
            logger.info(f"Resetting music track randomly to index: {new_track_index}")

        # Change to the determined track (retro-specific or random)
        change_music_track(game_state, new_track_index)
    else:
        logger.warning("Cannot randomize/set music on reset: No tracks defined.")
    # --- >>> MODIFIED SECTION END <<< ---

    # Ensure volume is applied correctly after potential changes/reloads
    set_volume(game_state)

    # --- >>> ADDED: Start new data logging session <<< ---
    if hasattr(game_state, "data_logger") and game_state.data_logger:
        try:
            player_name = "Unknown"
            if hasattr(game_state, "get_current_player"):
                ...
                if player and hasattr(player, "name"):
                    player_name = player.name
            game_state.data_logger.start_new_session(player_name,
                                                     current_game_mode)
        except Exception as e:
            logger.error(
                f"Error starting new data logger session during reset: {e}")
    # --- >>> END ADDED <<< ---
