...
def _update_ball_trails(game_state: Any) -> None:
    ...
    for ball in game_state.tracked_balls:
        try:
            ...
            logger.warning(f"Malformed ball data trail: {ball}")
        except Exception as e:
            logger.error(f"Error trail ball {ball}: {e}")
...
def _render_frame(frame: np.ndarray, game_state: Any) -> None:
    ...
    try:
        ...
        logger.warning(f"imshow error: {e}")
...
def _check_window_close(game_state: Any) -> bool:
    ...
    try:
        ...
        logger.info("SystemExit in _check_window_close.")
        raise
    except Exception as e:
...
def run_game_loop(game_state: Any) -> None:
    ...
    while True:  # Use internal break/return for exit
        ...
        # ADD THIS LOG:
        logger.debug(f"State *after* _handle_input: {game_state.current_state}")
        # -------------
        ...
        if frame is None:
            logger.debug("Capture None frame, break.")
            break
        ...
        # --- Update Game State ---
        # Note: This block structure assumes SHOWING_SPLASH is handled by returning early in draw_ui
        if game_state.current_state == CurrentGameState.PLAYING:
            ...
            # This covers MENU and any other future states that need these updates
            # but excludes SHOWING_SPLASH which should ideally do nothing here.
            _update_ball_trails(game_state)
            _update_game_state(game_state, dt)

        # ADD THIS LOG:
        logger.debug(
            f"State *after* _update_game_state block: {game_state.current_state}"
        )
        # -------------

        # --- Render Frame ---
        logger.debug(
            f"State *immediately before* calling _render_frame (draw_ui): {game_state.current_state}"
        )
        _render_frame(frame_resized, game_state)  # This calls draw_ui
