...
def _process_frame(frame: np.ndarray, game_state: Any) -> None:
    ...
    except AttributeError as e: logger.exception(f"AttrErr detect: {e}.")
...
def _update_ball_trails(game_state: Any) -> None:
    ...
    if not hasattr(game_state, 'ball_trails') or not hasattr(game_state, 'tracked_balls'):
        ...
        try:
            ...
            if len(ball) >= 6:
                ...
                if len(game_state.ball_trails[ball_id]) > GameConstants.BALL_TRAIL_LENGTH:
                 ...
                 logger.warning(f"Skipping trail update for malformed ball data (length < 6): {ball}")

        except (IndexError, ValueError, TypeError) as e:
             logger.warning(f"Error processing ball data for trail update: {ball}. Error: {e}")
    ...
    if ( game_state.current_state == CurrentGameState.PLAYING and game_state.game_mode == "timed" and game_state.game_timer is not None ):
        ...
        if game_state.game_timer <= 0:
            ...
            if game_state.current_state != CurrentGameState.GAME_OVER:
                ...
                if hasattr(game_state, 'save_score') and hasattr(game_state, 'get_current_player'):
...
def run_game_loop(game_state: Any) -> None:
    ...
    while True: # Use internal break/return/SystemExit for exit
        ...
        try:
            if cv2.getWindowProperty(UIConstants.WINDOW_NAME, cv2.WND_PROP_VISIBLE) < 1:
                 logger.info("Window closed via red X (WND_PROP_VISIBLE < 1). Breaking loop.")
                 # No need to call clean_exit here, the finally block will handle it
                 break
        except cv2.error as e:
             logger.warning(f"cv2.error checking WND_PROP_VISIBLE (window likely closed): {e}. Breaking loop.")
             break
        ...
        try:
            ...
            logger.error(f"Frame resize failed: {e}. Frame shape: {frame.shape}. Skipping frame.")
        ...
        if current_state == CurrentGameState.PLAYING:
            ...
            # Update trails visually, but don't advance game logic timers/scoring
             ...
             _update_game_state(game_state, dt) # Still call this to handle notifications/achievements popups
