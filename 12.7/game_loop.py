...
def _process_frame(frame: np.ndarray, game_state: GameState) -> None:
    ...
    try:
        ...
        logger.exception(f"Tracking error: {e}")  # Keep generic exception
...
def _render_frame(draw_canvas: np.ndarray, game_state: GameState) -> None:
    ...
    try:
        ...
        # Ignore error if window not found (likely closed between check and imshow)
...
def run_game_loop(game_state: GameState) -> None:
    """The main game loop."""
    # (Code mostly unchanged, except for adding pixelation logic)
    _initialize_display()
    last_time = time.time()
    ...
    try:
        while True:
            ...
            # --- START CHANGE: Add Pixelation for Retro Mode ---
            ...
            if game_state.current_state == CurrentGameState.PLAYING:
                ...
                # Update scoring AFTER processing frame
                update_scoring(game_state)

            # Render the final canvas (pixelated or not) with UI
