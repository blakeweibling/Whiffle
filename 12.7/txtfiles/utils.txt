    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if game_state.drag_start_pos and game_state.zone_editing_action:
            ...
            if fw < min_size or fh < min_size:
                show_notification(
                    game_state, "Zone too small! Reverted.", is_error=True, duration=3.0
                )  # Use helper
            ...
            if game_state.temp_zone:
                ...
                if (
                    ...
                    if not _zones_overlap(new_zone[:4], game_state.scoring_zones):
                        ...
                        show_notification(
                            game_state, "Zone Overlaps!", is_error=True
                        )  # Use helper
                else:
                    show_notification(
                        game_state, "Zone too small", is_error=True
                    )  # Use helper
            ...
            if isinstance(action, str):
                if action == "adjust_sound_volume":
                    ...
                    if action == "quit":
                        try:
                            save_score(
                                game_state, game_state.get_current_player().name
                            )  # Use helper
                    ...
                    if game_state.current_state == CurrentGameState.MENU:
                        if action == "toggle_debug_overlay":
                            ...
                            game_state.current_state = CurrentGameState.PLAYING
                            game_state.submenu_active = None
                            _reset_all_menu_editing_states(game_state)  # Use helper
                        ...
                        # --- END CHANGE ---
                            ...
                            if len(game_state.players) < 2:
                                ...
                                show_notification(
                                    game_state, "Player Added"
                                )  # Use helper
                            else:
                                show_notification(
                                    game_state, "Maximum 2 players", is_error=True
                                )  # Use helper
                            _reset_all_menu_editing_states(game_state)
                        ...
                        if action == "new_game_from_gameover":
                            ...
                            logger.warning(f"Unhandled GAME_OVER action: {action}")
                    return True
    if volume_adjusted:
        return True
    return False
