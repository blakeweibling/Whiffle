...
class ScoringZones:
    def __init__(self):
        ...
        self.load_zones()
        print(f"Initialized zones: {self.zones}")
    ...
    def mouse_callback(self, event, x, y, flags, frame, scale_x, scale_y):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            self.start_point = (x / scale_x, y / scale_y)  # Store unscaled coordinates
            self.pending_zone = None
        elif event == cv2.EVENT_LBUTTONUP:
            if self.start_point:
                self.end_point = (x / scale_x, y / scale_y)  # Store unscaled coordinates
    ...
    def calibrate_zones(self, frame, current_width, current_height):
        ...
        while self.calibrating:
            ...
            box_coords = ((mode_x, mode_y + 5), (mode_x + mode_size[0], mode_y - size[1] - 5))
            cv2.rectangle(calib_frame, box_coords[0], box_coords[1], (128, 128, 128), -1)
    ...
    def check_scores(self, balls, current_width, current_height):
        ...
        for ball in balls:
            ...
            scaled_x = x * scale  # Scale ball coordinates to match display
            scaled_y = y * scale
            print(f"Checking ball at ({scaled_x:.1f}, {scaled_y:.1f}) with ball_id {ball_id}, type {ball_type}")
            in_zone = False
            ...
            for zone_idx, zone in enumerate(self.zones):
                ...
                if in_zone:
                    ...
                    if ball_id not in self.scored_balls or zone_idx not in self.scored_balls.get(ball_id, set()):
                        zone_key = "_".join(map(str, [int(z * scale) if i < 4 else z for i, z in enumerate(zone)]))
                        adjusted_points = points * multiplier
                        if zone_key not in self.zone_scores or self.zone_scores[zone_key] < adjusted_points:
                            print(f"Scoring {adjusted_points} for ball_id {ball_id} in zone {zone}")
                            ...
                            self.zone_scores[zone_key] = self.zone_scores.get(zone_key, 0) + adjusted_points
                    break
        ...
        active_zones = {"_".join(map(str, [int(z * scale) if i < 4 else z for i, z in enumerate(zone)])) for zone in self.zones}
        self.zone_scores = {k: v for k, v in self.zone_scores.items() if k in active_zones}

        print(f"Total score: {total_score}, Scored balls: {self.scored_balls}, Zone scores: {self.zone_scores}")
        return total_score
