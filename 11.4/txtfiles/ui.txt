...
def _draw_player_name_input(frame: np.ndarray, game_state: GameState):
    ...
    # Draw a semi-transparent overlay first to dim background
    overlay = frame.copy()
    cv2.rectangle(
...
def draw_ui(
    ...
    # --- Draw elements common to PLAYING and MENU states (but not GAME_OVER) ---
    if game_state.current_state != CurrentGameState.GAME_OVER:
        ...
        if (
            ...
            timer_text = f"Time: {int(max(0, game_state.game_timer))}" # Ensure non-negative display
        ...
        if game_state.current_state == CurrentGameState.PLAYING:
            ...
            if game_state.drawing and game_state.temp_zone:
                ...
                # --- Draw points input text near the rectangle ---
                show_cursor = int(time.time() * 2) % 2 == 0
                cursor = "_" if show_cursor else " "
                points_display_str = game_state.drawing_points_input or "..." # Show '...' if empty
                points_text = f"{points_display_str}{cursor} pts"

                # Position text near the bottom-right corner of the temp zone
                text_x = x1 + w + 5  # Offset slightly right
                text_y = y1 + h - 5  # Offset slightly up from bottom

                # Basic boundary check for text position
                (ptw, pth), _ = cv2.getTextSize(points_text, cv2.FONT_HERSHEY_SIMPLEX, UIConstants.FONT_SCALE_SMALL, 1)
                if text_x + ptw > frame.shape[1]: # If text goes off right edge
                     text_x = x1 + w - ptw - 5 # Move it left of the corner
                if text_y < pth: # If text goes off top edge (unlikely here but good practice)
                     text_y = y1 + pth + 5 # Move below top corner
                if text_y > frame.shape[0] - 5: # If text goes off bottom edge
                     text_y = y1 + h - pth - 5 # Move it further up from bottom corner
                ...
                # --- End points input text drawing ---

            # Draw menu button only when not drawing a zone
            if not game_state.drawing:
            ...
            if game_state.achievement_notification and game_state.achievement_notification_timer > 0:
                ...
                (tw_ach, th_ach), _ = cv2.getTextSize(
                    notif_text,
                    cv2.FONT_HERSHEY_SIMPLEX,
                    UIConstants.FONT_SCALE_LARGE, # Make achievement popups large
                    UIConstants.FONT_THICKNESS,
                )
                nx_ach, ny_ach = (
                    UIConstants.WINDOW_WIDTH - tw_ach
                ) // 2, UIConstants.WINDOW_HEIGHT - 50 # Position near bottom center
                _draw_text_with_background(
                    frame,
                    notif_text,
                    (nx_ach, ny_ach),
                    UIConstants.FONT_SCALE_LARGE,
                    UIConstants.GREEN, # Achievement color
                    UIConstants.BLACK,
                    thickness=UIConstants.FONT_THICKNESS,
                    alpha=0.7,
                )
            ...
            _draw_text_with_background(
                ...
                UIConstants.YELLOW,
                UIConstants.BLACK,
                thickness=3, # Bold thickness for pause text
            )
            ...
            # Draw balls and zones dimly in background? Optional.
            overlay = frame.copy()
            draw_scoring_zones(
            ...
            # Ensure width/height are set before calculating position
        ...
        # Draw general notifications (like zone added/deleted, errors) - Draw last
    ...
    # Draw general debug text if debug mode is on
    if (
        game_state.current_state != CurrentGameState.GETTING_PLAYER_NAME
        ...
        debug_text = f"FPS:{fps:.1f}|State:{state_text}|{drawing_active_text}|Overlay(b):{overlay_status}|Tracked:{tracked_count}"
