...
class BallTracker:
        ...
        for ball in self.balls:
            ...
            if 0 <= scaled_x <= current_width and 0 <= scaled_y <= current_height:
                if ball_type == "half":
                    ...
                    cv2.circle(frame, center, int(scaled_radius), (0, 0, 0), 1)
                else:
    ...
    def update_physics(self, current_width, current_height, delta_time):
        """Update the physics of tracked balls using the actual frame time."""
        scale = min(current_width / self.settings.base_frame_width, current_height / self.settings.base_frame_height)
        ...
        for ball in self.balls:
            ...
            vy += scaled_gravity * delta_time * 10
            x += vx * delta_time
            y += vy * delta_time
