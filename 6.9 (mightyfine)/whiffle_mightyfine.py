...
while running:
    try:
        ...
        for (x, y, radius, points, is_oblong, rect) in game.hole_positions:
            scaled_radius = int(radius * (target_width / INITIAL_WIDTH))
            scaled_x = int(x * (target_width / INITIAL_WIDTH))
            scaled_y = int(y * (target_height / INITIAL_HEIGHT))
            ...
            if is_oblong and rect:
                x1, y1, x2, y2 = rect
                scaled_x1 = int(x1 * (target_width / INITIAL_WIDTH))
                scaled_y1 = int(y1 * (target_height / INITIAL_HEIGHT))
                scaled_x2 = int(x2 * (target_width / INITIAL_WIDTH))
                scaled_y2 = int(y2 * (target_height / INITIAL_HEIGHT))
        ...
        screen.fill(BLACK)
        screen.blit(scaled_surface, ((current_width - target_width) // 2, MENU_HEIGHT))
        ...
        for pos in game.detected_positions:
            if any(pos == game.confirming_balls[ball_id]["hole_pos"] for ball_id in game.confirming_balls
                ...
                pygame.draw.circle(screen, YELLOW, pos, int(20 * (target_width / INITIAL_WIDTH)), 2)
            else:
                pygame.draw.circle(screen, RED, pos, int(20 * (target_width / INITIAL_WIDTH)), 2)
            for (x, y, _, _, is_oblong, rect) in game.hole_positions:
                if is_oblong and rect and pos == (x, y) and pos in game.scored_balls:
                    ...
                    scaled_x2 = int(x2 * (target_width / INITIAL_WIDTH))
                    scaled_y2 = int(y2 * (target_height / INITIAL_HEIGHT))
                    cv2.rectangle(frame_to_display, (scaled_x1, scaled_y1), (scaled_x2, scaled_y2), (0, 0, 255), 2)

        # Draw about and settings windows if active
