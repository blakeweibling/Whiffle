...
class BallDetector:
    def __init__(self):
        ...
        self.model = YOLO('F:/Whiffle/9.6 (pickupsticks)/runs/train/whiffle_exp/weights/best.pt')  # Update with your model path
    ...
    def _is_position_excluded(self, x: int, y: int, excluded_positions: List[Tuple[int, int]]) -> bool:
        ...
        """
        return any(np.sqrt((x - ex) ** 2 + (y - ey) ** 2) < DetectionConstants.EXCLUSION_DISTANCE
    ...
    def _is_in_scoring_zone(self, x: float, y: float, radius: float, scoring_zones: List[Tuple[int, int, int, int, int]]) -> bool:
        ...
        for zone in scoring_zones:
            ...
            # We can also consider the radius to ensure the ball is fully or mostly inside
    ...
    def detect_all_balls(
        ...
        for result in results:
            ...
            for box, score, cls in zip(boxes, scores, classes):
                ...
                # Generate a unique ball ID based on position and class
                ball_id = f"{int(cls)}_{int(x_center)}_{int(y_center)}"

                # Infer the state based on position relative to scoring zones
                state = "in_hole" if self._is_in_scoring_zone(x_center, y_center, radius, scoring_zones) else "on_playfield"

                # Update game_state with ball state for scoring
                game_state.ball_states[ball_id] = state

                if debug_mode:
                    logger.debug(f"Detected {self.class_names[int(cls)]} ball ID {ball_id} at ({x_center}, {y_center}) with radius={radius}, state={state}")

                if self.class_names[int(cls)] == 'white':
                    white_balls.append((int(x_center), int(y_center), radius))
                elif self.class_names[int(cls)] == 'red':
                    red_balls.append((int(x_center), int(y_center), radius))
                elif self.class_names[int(cls)] == 'half':
