# Whiffle Playfield - Fixed Rendering Order and Reduced Debug Logging
# Lines 1-700
import cv2
import numpy as np
import pygame
...
while running and game.running:
    ...
    if frame_to_display is None or frame_to_display.size == 0:
        print("Warning: Frame is None or invalid after ROI, using black screen as fallback.")
...
while running and game.running:
    ...
    frame_to_display = roi if roi is not None else frame
    if frame_to_display is None or frame_to_display.size == 0:
        print("Warning: Frame is None or invalid, using black screen as fallback.")
        frame_to_display = np.zeros((HEIGHT, WIDTH, 3), dtype=np.uint8)
    frame_rgb = cv2.cvtColor(frame_to_display, cv2.COLOR_BGR2RGB)
