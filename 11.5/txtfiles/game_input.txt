...
def _handle_input(game_state: Any) -> Optional[int]:
    ...
    if key != -1 and key != 255: # Ignore no-input/-1 and potential 255 spam
        ...
        # Use 8 spaces for this level
            ...
            # Use 12 spaces for this level
            if not key_handled_globally: # Changed from key_handled
                # Use 16 spaces for this level
        ...
        # Use 8 spaces for this level
            ...
            # --- NEW: Handle input during zone drawing ---
            if game_state.drawing:
                if ord('0') <= key <= ord('9'):
                    ...
                    key_handled_in_playing = True
                # Consider adding 'c' or ESC to cancel drawing here?
                # elif key == ord('c') or key == 27:
                #     logger.info("Drawing cancelled via key press.")
            ...
            if not key_handled_in_playing:
                ...
                if key == ord("m"): # Toggle menu ON
                    ...
                    # Only toggle if not already drawing (or handle toggle off?)
                    # Current logic toggles on/off
