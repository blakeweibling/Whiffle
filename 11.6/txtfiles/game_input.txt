...
def _handle_input(game_state: Any) -> Optional[int]:
    ...
    key_handled_globally = (
        False  # Flag to check if input was processed overall [cite: 3]
    )
    ...
    if key != -1 and key != 255:  # Ignore no-input/-1 and potential 255 spam [cite: 5]
        ...
        if (
            ...
            if key == 13:  # Enter Key [cite: 6]
                logger.debug(
                    "Enter key (13) detected during initial name input."
                )  # [cite: 6]
                ...
                if not entered_name:  # [cite: 6]
                    ...
                    try:  # [cite: 8]
                        if game_state.players:  # [cite: 8]
                            ...
                            logger.info(
                                f"Player 1 name set to: '{entered_name}'"
                            )  # [cite: 9]
                ...
                logger.debug(
                    "Escape key (27) detected during initial name input."
                )  # [cite: 18]
                ...
                logger.debug(
                    "Backspace key (8) detected during initial name input."
                )  # [cite: 22]
                ...
                if char in PlayerConstants.ALLOWED_PLAYER_NAME_CHARS:  # [cite: 25]
                    ...
                    logger.debug(
                        f"Character '{char}' not allowed for player name."
                    )  # [cite: 29]
        ...
        # --- Handle input for MENU state ---
            ...
            if (  # [cite: 31]
                ...
                if key == 13:  # Enter Key - Save Name [cite: 32]
                    ...
                    new_name = (
                        game_state.editing_player_name_input.strip()
                    )  # [cite: 33]
                    if not new_name:  # [cite: 34]
                        game_state.show_notification(
                            "Player name cannot be empty!",
                            is_error=True,
                            duration=2.0,  # [cite: 34]
                        )
                    ...
                    game_state.show_notification(
                        "Name edit cancelled", duration=1.5
                    )  # [cite: 44]
            ...
            # Zone Points Editing Logic [cite: 53]
                ...
                if key == 13:  # Enter Key - Save Points [cite: 55]
                    ...
                    try:  # [cite: 56]
                        new_points_str = (
                            game_state.editing_zone_points_input.strip()
                        )  # [cite: 57]
                        if not new_points_str:  # [cite: 57]
                            ...
                            if not (
                                1 <= new_points <= ScoringConstants.MAX_POINTS
                            ):  # [cite: 59]
                                ...
                                0 <= zone_idx < len(game_state.scoring_zones)
                            ):  # [cite: 61]
                    ...
                    game_state.show_notification(
                        "Points edit cancelled", duration=1.5
                    )  # [cite: 78]
                    ...
                    logger.debug(
                        f"Numeric key {chr(key)} detected during points edit."
                    )  # [cite: 82]
                    ...
                    if (
                        len(game_state.editing_zone_points_input) < max_digits
                    ):  # [cite: 84]
            ...
            ):  # Check if not handled by specific editing modes [cite: 88]
                if key == ord("m"):  # Toggle menu OFF (Resume) [cite: 88]
                    logger.debug(
                        "Menu key ('m') pressed in menu, resuming game."
                    )  # [cite: 88]
                    ...
                    logger.debug(
                        "Backspace key (8) detected for menu navigation."
                    )  # [cite: 91]
                    ...
                    if game_state.editing_zone_mode:  # [cite: 92]
                        ...
                        logger.debug(
                            "Cancelled zone editing via backspace."
                        )  # [cite: 93]
                        ...
                        logger.debug(
                            "Cancelled player editing via backspace."
                        )  # [cite: 95]
                    ...
                    ):  # Back from any other submenu to main menu [cite: 97]
                    ...
                    logger.debug(
                        "Escape key (27) detected in menu, resuming game."
                    )  # [cite: 100]
        ...
        # --- Handle input for PLAYING state ---
            ...
            key_handled_in_playing = (
                False  # Use local flag for PLAYING state [cite: 103]
            )
            ...
            if not key_handled_in_playing:  # [cite: 110]
                if key == ord("m"):  # Toggle menu ON [cite: 110]
                    ...
                    logger.info(
                        "Escape key (27) pressed while playing, exiting."
                    )  # [cite: 122]
                    clean_exit(  # [cite: 122]
        ...
        # --- Handle input for PAUSED state ---
            ...
            if key == ord("p"):  # Resume game [cite: 125]
                ...
                logger.info(
                    "Escape key (27) pressed while paused, exiting."
                )  # [cite: 126]
                clean_exit(  # [cite: 127]
                    game_state.cap,
                    game_state.background_music,
        ...
        # --- Handle input for GAME_OVER state ---
            ...
            if key == ord("n"):  # Start New Game [cite: 128]
                logger.info(
                    "'n' key pressed on game over screen. Starting new game."
                )  # [cite: 129]
                ...
                logger.info(
                    "'l' key pressed on game over screen. Showing leaderboard."
                )  # [cite: 131]
                ...
                logger.info(
                    "Escape key (27) pressed on game over screen, exiting."
                )  # [cite: 132]
                clean_exit(  # [cite: 132]
