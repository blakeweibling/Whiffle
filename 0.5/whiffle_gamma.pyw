...
def setup_point_zones_in_main(frame, window_name):
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            # Prompt for points immediately after clicking
            prompt = "Points: "
            points_str = get_text_input_near_click(window_name, frame, prompt, x, y)
            try:
                points = int(points_str)
                point_zones.append((x, y, ZONE_RADIUS, points))
                # Update frame with the assigned points
                cv2.putText(frame, str(points), (x - 10, y), cv2.FONT_HERSHEY_SIMPLEX, 
                            0.5, (255, 255, 255), 2)
            except ValueError:
                clicked_points.pop()  # Remove invalid point
