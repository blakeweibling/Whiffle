...
# Network timeout (seconds) for YouTube API calls and the resumable upload
# HTTP transport. Without this, httplib2 can block forever on slow/lost
# connections, hanging the background upload thread.
...
def _build_youtube_http() -> httplib2.Http:
    ...
    return httplib2.Http(timeout=YOUTUBE_HTTP_TIMEOUT_SECONDS)
...
def upload_video_to_youtube(
    ...
    try:
        # Build the YouTube API service on a bounded-timeout HTTP transport so
        # uploads cannot hang the uploader thread indefinitely. The oauth2client
        # credentials authorise the http in-place by wrapping its request().
        authed_http = credentials.authorize(_build_youtube_http())
