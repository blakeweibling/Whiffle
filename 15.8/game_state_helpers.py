...
def save_high_score(game_state: Any):
    ...
    if not atomic_write_json(high_score_file, data, indent=4):
        logger.error(f"Save high score file failed: {high_score_file}")
...
def save_zones(game_state: Any, zones_file_path: Optional[str] = None) -> None:
    """Save the current scoring zones to a JSON file (atomic)."""
    zones_file = zones_file_path or getattr(
        game_state, "zones_file_path", GameConstants.ZONES_FILE
    )
    if atomic_write_json(zones_file, game_state.scoring_zones, indent=4):
        logger.info(f"Scoring zones saved to {zones_file}")
        show_notification(game_state, "Zones Saved")
    else:
        logger.error(f"Error saving scoring zones to {zones_file}")
        show_notification(game_state, "Error Saving Zones", is_error=True)
    ...
    if os.path.exists(zones_file_path):
        try:
            ...
            if isinstance(loaded_data, list) and all(
                ...
                logger.debug(f"Scoring zones loaded from {zones_file_path}")
