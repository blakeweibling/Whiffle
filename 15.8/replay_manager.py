...
class Replay:
    ...
    def save_to_file(self, base_path: Optional[str] = None) -> str:
        ...
        # Create a thumbnail from the first keyframe BEFORE deleting temp files
            ...
            if sorted_keyframes:
                ...
                try:
                    ...
                    if os.path.exists(first_keyframe) and os.access(
                        ...
                        # Fallback: try the already-copied keyframe in the replay dir
                        fallback_frame = os.path.basename(first_keyframe).split("_")[1]
                        fallback_path = os.path.join(keyframes_dir, fallback_frame)
                        if os.path.exists(fallback_path):
                            shutil.copy2(fallback_path, thumbnail_path)
                            logger.info(f"Created thumbnail from copied keyframe for replay {self.replay_id}")
                        else:
                            logger.warning(f"First keyframe not accessible: {first_keyframe}")
                except Exception as e:
                    logger.warning(
        ...
        # Clean up temp keyframes after thumbnail creation
        for temp_keyframe_path in temp_keyframe_files:
    ...
    def generate_video(
        ...
        try:
            ...
            if format == "GIF":
                ...
                # Create primary video writer. If it fails to open, try a
                # fallback codec. Always release whichever writer we end up
                # opening (via try/finally below).
                video_writer = cv2.VideoWriter(
                    output_path, fourcc, fps, (width, height), True  # isColor
                )

                if not video_writer.isOpened():
                    # Release the failed writer before replacing it so we
                    # don't orphan a handle.
                    try:
    ...
    def extract_highlight_video(
        ...
        try:
            ...
            first_frame = cv2.imread(first_frame_path)
            if first_frame is None:
                logger.error(f"Failed to read first keyframe: {first_frame_path}")
                return None
            height, width = first_frame.shape[:2]
            ...
            if not video_writer.isOpened():
                logger.error(f"Failed to open highlight video writer at {output_path}")
                try:
                    video_writer.release()
                except Exception:
                    pass
            ...
            for frame_count in keyframe_counts:
                ...
                if os.path.exists(keyframe_path):
                    ...
                    if img is None:
                        logger.warning(f"Failed to read keyframe: {keyframe_path}")
                        continue
...
class ReplayManager:
    ...
    def _load_replays(self) -> None:
        ...
        logger.debug(f"Loaded metadata for {len(self.replays)} replays")
