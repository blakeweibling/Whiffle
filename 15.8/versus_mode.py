...
def _full_reset_for_versus(game_state, player_name: str) -> None:
    """Perform a full game reset for ``player_name`` in versus mode.

    The module-level ``reset_game`` in ``game_state_utils`` clears tracked
    balls, zone cooldowns, ball/zone histories and everything else that
    the shallow ``GameState.reset_game`` method leaves behind. It reads
    ``game_mode`` and ``current_player_index`` off the instance, so we set
    those first.
    """
    # Select the player by name (falling back to the existing index).
    players = getattr(game_state, "players", []) or []
    target_index = getattr(game_state, "current_player_index", 0)
    for idx, player in enumerate(players):
        if getattr(player, "name", None) == player_name:
            target_index = idx
            break
    game_state.current_player_index = target_index
    game_state.game_mode = "versus"
...
def start_versus_mode(game_state):
    ...
    _full_reset_for_versus(game_state, game_state.versus_players[0].name)
...
def switch_versus_player_turn(game_state):
    ...
    _full_reset_for_versus(game_state, next_player.name)
