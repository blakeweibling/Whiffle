...
from operator_remote import process_remote_actions, update_remote_status_snapshot
...
def _process_frame(frame: np.ndarray, game_state: GameState) -> None:
    ...
    try:
        ...
        if frame is not None and hasattr(frame, "shape") and len(frame.shape) >= 2:
            try:
                hsv_for_detect = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            except cv2.error:
                hsv_for_detect = None
...
def run_game_loop(game_state: GameState) -> None:
    ...
    try:
        while True:
            ...
            # Keep an unmodified copy for detection so retro effects don't
            # degrade ball-detection accuracy.
            detection_frame = draw_canvas

            # Apply retro mode effects if enabled
            if game_state.game_mode == "retro":
                detection_frame = draw_canvas.copy()
            ...
            if game_state.current_state == CurrentGameState.PLAYING:
                ...
                if run_detection_tracking:
                    _process_frame(detection_frame, game_state)
