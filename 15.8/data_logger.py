...
class DataLogger:
                ...
                with open(STATS_HISTORY_FILE, "r") as f:
                    ...
                    if isinstance(data, list):
                        ...
                        if len(data) > MAX_HISTORY_ENTRIES:
                            ...
                            if not atomic_write_json(STATS_HISTORY_FILE, data, indent=2):
                                logger.warning("Could not write trimmed history to disk")
                        logger.debug(f"Loaded {len(data)} historical session records.")
                        return data
                    else:
                        logger.warning(
    ...
    def _save_historical_stats(self):
        """Saves the current historical session data to the JSON file (atomic)."""
        if atomic_write_json(STATS_HISTORY_FILE, self.historical_stats, indent=2):
            logger.info(
                f"Saved {len(self.historical_stats)} historical session records to '{STATS_HISTORY_FILE}'."
            )
        else:
            logger.error(
                f"Failed to save historical stats to '{STATS_HISTORY_FILE}'."
            )
...
if __name__ == "__main__":
    ...
    data_logger.log_score_event(zone_id=0, points=100, ball_type="silver")
    time.sleep(0.1)
    data_logger.log_ball_positions([(100, 200, 10.0, 1, 5, "silver")])
    data_logger.log_score_event(zone_id=1, points=200, ball_type="gold")
    time.sleep(0.1)
    data_logger.log_ball_positions(
        [(110, 210, 10.0, 1, 6, "silver"), (500, 400, 12.0, 2, 1, "gold")]
    )
