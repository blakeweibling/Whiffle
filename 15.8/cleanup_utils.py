...
from operator_remote import stop_operator_remote
...
def clean_exit(
    ...
    if game_state:
        try:
            stop_operator_remote(game_state)
            ...
            # Persist XP data on exit (no longer cleared between sessions)
            logger.info("Saving player XP data on exit...")
            try:
                from xp_system import xp_system
                xp_system.save_player_data()
                logger.info("Player XP data saved successfully.")
            except Exception as e:
                logger.error(f"Error saving XP data on exit: {e}")
