...
class Leaderboard:
    ...
    def _save_local_scores(self) -> None:
        """Save local scores to the JSON file atomically."""
        for mode in VALID_MODES:
            if mode not in self.local_scores:
                self.local_scores[mode] = []
        if atomic_write_json(self.local_file, self.local_scores, indent=4):
            logger.info(f"Saved local leaderboard to {self.local_file}")
        else:
            logger.error(f"Error saving local leaderboard to {self.local_file}")
