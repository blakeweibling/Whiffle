...
def update_scoring(game_state: Any) -> None:
    ...
    for ball in tracked_balls_list:
        ...
        if zone and stable:  # Must be in a zone and stable
            ...
            # Record the ball's scoring position so the tracker can suppress
            # fresh YOLO IDs reappearing at the same spot ("ghost balls").
            # This was previously dead code because nothing populated the dict.
            scored_positions = getattr(game_state, "scored_positions", None)
            if isinstance(scored_positions, dict):
                scored_positions[(int(x), int(y))] = ball_id
                # Bound growth: keep only the most recent ~32 scored positions.
                # Dict preserves insertion order (Python 3.7+), so popping the
                # first key removes the oldest entry.
                max_scored_positions = 32
                while len(scored_positions) > max_scored_positions:
                    try:
                        oldest = next(iter(scored_positions))
                        del scored_positions[oldest]
