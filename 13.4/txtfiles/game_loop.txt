...
# (Loop logic unchanged from previous correction)
def run_game_loop(game_state: GameState) -> None:
    ...
    try:
        while True:
            ...
            # Retro Pixelation and Sepia Filter
            if game_state.game_mode == "retro":
                try:
                    # Pixelation
                    h, w = draw_canvas.shape[:2]; factor = GameConstants.RETRO_PIXEL_FACTOR; small_h, small_w = max(1, h // factor), max(1, w // factor)
                    ...
                    # Sepia Filter
                    if len(draw_canvas.shape) == 2:
                         draw_canvas = cv2.cvtColor(draw_canvas, cv2.COLOR_GRAY2BGR)
                    elif draw_canvas.shape[2] == 4:
                         draw_canvas = cv2.cvtColor(draw_canvas, cv2.COLOR_BGRA2BGR)
                    ...
                    sepia_canvas = cv2.transform(draw_canvas, sepia_kernel)
                    sepia_canvas = np.clip(sepia_canvas, 0, 255).astype(np.uint8)
                    draw_canvas = sepia_canvas

                except Exception as retro_filter_error: logger.error(f"Error during retro pixelation or sepia filter: {retro_filter_error}")
