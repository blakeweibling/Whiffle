...
# (Content of _process_zone_editing_event remains unchanged, as zone coordinates are scaled in game_state)
...
# (Content of _process_drawing_event remains unchanged, as it adds new zones which will be scaled if resolution changes later)
...
def _process_menu_or_modal_click(x: int, y: int, game_state: "GameState") -> bool:
    ...
    for item_data in reversed(submenu_items_list):
        ...
        if current_state != CurrentGameState.CONFIRM_QUIT: item_rect_abs = (item_x + menu_x, item_y + menu_y, item_w, item_h); click_x_to_check, click_y_to_check = x, y #
        ...
        if (abs_item_x <= click_x_to_check < abs_item_x + abs_item_w and abs_item_y <= click_y_to_check < abs_item_y + abs_item_h):
            ...
            if isinstance(action, str): #
                # Volume slider checks (remain unchanged)
                if current_state == CurrentGameState.MENU: #
                ...
                if current_state == CurrentGameState.MENU: #
                    if action == "quit": game_state.previous_state_before_quit_confirm = (CurrentGameState.MENU); game_state.current_state = CurrentGameState.CONFIRM_QUIT; _reset_all_menu_editing_states(game_state) #
                    ...
                    elif action.startswith("set_mode_"): # --- >>> MODIFIED SECTION START <<< ---
                         ...
                         if new_mode in valid_modes and getattr(game_state, "game_mode", "classic") != new_mode:
                              try: save_score(game_state, game_state.get_current_player().name, mode=game_state.game_mode) # Save score of previous mode
                              except Exception as e: logger.error(f"Error saving score before mode change: {e}") # Log error if save fails

                              game_state.game_mode = new_mode; # Set the new game mode first

                              # Explicitly set retro music BEFORE reset_game is called
                              if new_mode == "retro":
                                  ...
                                  try:
                                      ...
                                      change_music_track(game_state, retro_track_index) # Call the utility function
                                  except ValueError:
                                      logger.error(f"Could not find '{target_track}' for retro mode switch. Music will be randomized by reset_game.")
                                  except Exception as e_music:
                                       logger.error(f"Error changing music track during retro mode switch: {e_music}")

                              # Reset game state (reset_game now handles music based on the new mode, including retro)
                              reset_game(game_state);
                              show_notification(game_state, f"Mode set to: {new_mode.capitalize()}");
                              game_state.current_state = CurrentGameState.PLAYING; # Switch back to playing state
                              game_state.submenu_active = None; # Close menu
                              _reset_all_menu_editing_states(game_state) # Reset menu editing flags
                    # --- >>> MODIFIED SECTION END <<< ---
                    elif action.startswith("select_player_"): # Player select logic...
                         try:
                    ...
                    elif action == "add_player": # Add player logic...
                    ...
                    elif action.startswith("edit_zone_"): # Edit zone points logic...
                         try:
                    ...
                    elif action.startswith("move_zone_"): # Move zone logic...
                         try:
                    ...
                    elif action.startswith("resize_zone_"): # Resize zone logic...
                         try:
                    ...
                    elif action.startswith("edit_player_name_"): # Edit player name logic...
                         try:
                    ...
                    elif action.startswith("delete_zone_"): # Delete zone logic...
                         try:
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    # Get current dimensions for relative calculations if needed
    ...
    # 5. [NEW] Resolution Button Click (If Playing)
    # 6. Main Menu Button Click (If Playing)
    ...
    if (not click_handled and game_state.current_state in [CurrentGameState.MENU, CurrentGameState.PAUSED] and event == cv2.EVENT_LBUTTONDOWN): #
        # Calculate heatmap button position dynamically (must match ui.py)
        try: # Wrap calculation in try-except
            ...
            panel_width = 350; stats_content_height = 230; button_height = 35; panel_padding_bottom = 15 #
            ...
            panel_y = menu_y #
            if panel_x + panel_width > current_width - padding: panel_x = menu_x - panel_width - padding # Use current_width
            if panel_x < padding: panel_x = (current_width - panel_width) // 2; panel_y = menu_y + menu_h + padding; panel_height = total_content_height + 40 # Use current_width
            ...
            hm_button_width = panel_width - (2 * text_x_offset) #
            heatmap_button_rect = (button_x_pos, button_y_pos, hm_button_width, button_height) #
            ...
            if bx <= x < bx + bw and by <= y < by + bh: #
                ...
                try: display_heatmap_modal(game_state, mouse_callback, game_state) #
                except Exception as e: logger.exception(f"Error occurred when trying to display heatmap: {e}"); show_notification(game_state,"Error displaying heatmap", is_error=True) #
                click_handled = True
                return # Click handled
        except Exception as e_calc:
            logger.error(f"Error calculating heatmap button position: {e_calc}")


    # --- [ADD] 5. Resolution Button Click ---
    if (not click_handled
            and game_state.current_state == CurrentGameState.PLAYING
            and not getattr(game_state, "drawing", False) # Check not drawing
            and event == cv2.EVENT_LBUTTONDOWN):

        # Define res_button_rect using constants (ensure these are defined in constants.txt)
        # Use absolute constants for now as defined in modified constants.txt
        res_button_rect = (
        ...
        rbx, rby, rbw, rbh = res_button_rect

        if rbx <= x < rbx + rbw and rby <= y < rby + rbh:
            logger.info("Resolution toggle button clicked.")
            game_state.click_feedback_state = (res_button_rect, time.time()) # Visual feedback

            current_res_key = game_state.current_resolution_key
            # Determine the target key by finding the *other* key in the RESOLUTIONS dict
            ...
            if len(available_keys) == 2: # Specific logic for 2 resolutions
                 ...
                 try:
                      ...
                      target_res_key = available_keys[next_index]
                 except ValueError:
                      logger.error(f"Current resolution key '{current_res_key}' not found in available keys.")
                      target_res_key = ResolutionConstants.DEFAULT_RESOLUTION # Fallback

            if target_res_key:
                try:
                    # Call the method in game_state to handle the change
                    game_state.set_resolution(target_res_key)
                except Exception as e:
                     logger.exception(f"Error setting resolution to {target_res_key}: {e}")
                     show_notification(game_state, "Error changing resolution!", is_error=True)

            click_handled = True
            return # Exit callback after handling click
    # --- [END ADD] ---


    # 6. Main "Menu" Button Click
    if (not click_handled
            and game_state.current_state == CurrentGameState.PLAYING # Only clickable when playing
            and not getattr(game_state, "drawing", False) # Check not drawing
            and event == cv2.EVENT_LBUTTONDOWN):
        # Use constants for menu button position/size
        menu_button_rect = (
