...
class ScoringZones:
    ...
    def check_scores(self, balls, current_width, current_height):
        ...
        if self.debug:
            print(f"Scoring with current resolution {current_width}x{current_height}, scale_x={scale_x:.4f}, scale_y={scale_y:.4f}")
        
        for ball in balls:
            x, y, _, _, ball_type, _, ball_id, _ = ball
            # Apply scaling to ball position
            scaled_x = x * scale_x
            scaled_y = y * scale_y
            ...
            for zone in possible_zones:
                if len(zone) == 4:  # Circle zone
                    ...
                    # Scale zone position and radius
                    scaled_zx = zx * scale_x
                    scaled_zy = zy * scale_y
                    scaled_radius = radius * scale_x  # Use scale_x for radius to maintain aspect
                    ...
                    if distance <= scaled_radius:
                        ...
                        if adjusted_points > best_score:
                            ...
                            if self.debug:
                                print(f"Ball {ball_id} in circle zone {best_zone_idx}: distance={distance:.1f}, scaled_radius={scaled_radius:.1f}")
                else:  # Rectangle zone
                    zx, zy, zw, zh, points = zone
                    # Scale zone position and size
                    scaled_zx = zx * scale_x
                    scaled_zy = zy * scale_y
                    ...
                    if scaled_zx <= scaled_x <= scaled_zx + scaled_zw and scaled_zy <= scaled_y <= scaled_zy + scaled_zh:
                        ...
                        if adjusted_points > best_score:
                            ...
                            best_zone_idx = self.zones.index(zone)
                            if self.debug:
                                print(f"Ball {ball_id} in rectangle zone {best_zone_idx}: bounds=({scaled_zx:.1f}, {scaled_zy:.1f}) to ({scaled_zx + scaled_zw:.1f}, {scaled_zy + scaled_zh:.1f})")
