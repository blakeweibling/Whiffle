...
def save_labeled_data(frame, labels, filename="labeled_data.pkl"):
    ...
    if os.path.exists(filename):
        try:
            ...
            if not isinstance(existing_data, list):
                print(f"Warning: {filename} contains invalid data. Starting fresh.")
                existing_data = []
        except Exception as e:
            print(f"Error loading existing data from {filename}: {e}. Starting fresh.")
            existing_data = []
    ...
    try:
        ...
        print(f"Saved {len(new_data)} new labeled patches to {filename}. Total entries: {len(combined_data)}")
    except Exception as e:
