...
class Game:
    ...
    def initialize_camera(self):
        ...
        if not self.cap.isOpened():
            ...
            if not self.cap.isOpened():
                ...
                # Redirect stderr to suppress libpng warnings during image loading
                with contextlib.redirect_stderr(io.StringIO()):
    ...
    def show_splash(self, frame):
        ...
        # Redirect stderr during the entire splash screen display to catch any rendering-related warnings
        with contextlib.redirect_stderr(io.StringIO()):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.is_splash_active and self.splash_img is not None:
                # Resize splash_img to match the current frame dimensions
            ...
            if self.initials_input:
                ...
                # Handle mouse events for InitialsInput
            ...
            if self.full_processing and not self.menu.is_menu_active():
                ...
                filtered_balls = [
                    ...
                    if (ball[4] == "white" and self.menu_settings.config.white_ball_detection) or
                       ...
                       (ball[4] == "half" and getattr(self.menu_settings.config, 'white_ball_detection', True))
                ]
                if filtered_balls:
            ...
            # Draw timer text during gameplay
...
if __name__ == "__main__":
    # Redirect stderr during program startup to catch any remaining libpng warnings
    with contextlib.redirect_stderr(io.StringIO()):
        try:
            pygame.init()
            pygame.font.init()
