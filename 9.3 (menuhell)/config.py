...
class GameConfig(BaseModel):
    ...
    def validate_areas(cls, v, values):
        """Ensure detection_area_max is greater than detection_area_min."""
        if "detection_area_min" in values and v <= values["detection_area_min"]:
            raise ValueError("detection_area_max must be greater than detection_area_min")
        return v
...
def load_config(filename="config.json"):
    ...
    if os.path.exists(filename):
        ...
        print(f"Configuration file {filename} not found. Creating with default settings.")
        save_config(config, filename)
