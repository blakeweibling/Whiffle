...
def _is_position_excluded(x: int, y: int, excluded_positions: List[Tuple[int, int]]) -> bool:
    ...
    Args:
        ...
        excluded_positions: List of (x, y) positions to exclude.
...
def detect_balls(
    ...
    Args:
        frame: Input BGR frame from the video capture.
        frame_count: Current frame number for tracking small balls.
        potential_small_balls: Dictionary tracking potential small balls across frames.
        hsv_lower: Lower HSV bound for the color to detect.
        hsv_upper: Upper HSV bound for the color to detect.
        hsv_lower2: Optional second lower HSV bound (e.g., for red).
        hsv_upper2: Optional second upper HSV bound (e.g., for red).
        excluded_positions: List of (x, y) positions to exclude from detection.
        debug_mode: If True, log debug information.
        hsv_frame: Optional pre-converted HSV frame to avoid redundant conversion.

    Returns:
        List of tuples (x, y, radius) representing detected balls.
    """
...
def detect_white_balls(
    ...
    Returns:
        List of tuples (x, y, radius) representing detected white balls.
    """
    return detect_balls(
...
def detect_red_balls(
    ...
    Returns:
        List of tuples (x, y, radius) representing detected red balls.
    """
    return detect_balls(
        frame, frame_count, potential_small_balls,
