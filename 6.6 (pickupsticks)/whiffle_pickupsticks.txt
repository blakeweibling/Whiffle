        ...
        with open(CALIBRATION_FILE, 'r') as f:
            try:
                ...
                for hole in data["holes"]:
                    if len(hole) == 4:  # Old format: (x, y, radius, points)
                        x, y, radius, points = hole
                        holes.append((x, y, radius, points, False, None))
                    else:  # New format: (x, y, radius, points, is_oblong, rect)
    ...
    while calibrating:
        ...
        if input_active:
            if key == 13:  # Enter
                ...
                print("Is this an oblong hole? (y/n)")
                is_oblong_input = cv2.waitKey(0) & 0xFF
                is_oblong = is_oblong_input == ord('y')
                if is_oblong:
                    print("Click top-left and bottom-right corners of the oblong area.")
                    cv2.setMouseCallback('Calibrate Holes', lambda event, x, y, flags, param: None)  # Disable previous callback
                    rect_points = []
                    def rect_callback(event, x, y, flags, param):
                        ...
                        if event == cv2.EVENT_LBUTTONDOWN and len(rect_points) < 2:
                            ...
                            if len(rect_points) == 2:
                                cv2.setMouseCallback('Calibrate Holes', mouse_callback)  # Re-enable original callback
                    cv2.setMouseCallback('Calibrate Holes', rect_callback)
                    while len(rect_points) < 2:
                        cv2.waitKey(1)
                    x1, y1 = rect_points[0]
                    x2, y2 = rect_points[1]
                ...
                print(f"Added hole at {current_pos} with {points} points, Oblong: {is_oblong}")
                input_active = False
                current_input = ""
                current_pos = None
...
def detect_ball_in_hole(image, hole_coords, game_state, frame_count):
    ...
    for contour in contours:
        ...
        if 0.1 * np.pi * 20 * 20 < area < np.pi * 20 * 20:
