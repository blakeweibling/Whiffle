...
class WhiffleGame:
    ...
    def save_file(self, window):
        ...
        if filename:
            ...
            print(f"Saved zones to {filename}")  # Debug
        window.destroy()
        self.paused = False
        self.update_frame()
    ...
    def load_file(self, window):
        ...
        if filename:
            ...
            if loaded_zones:
                ...
                self.zone_count = len(self.point_zones)
                print(f"Loaded zones from {filename}, count: {self.zone_count}")  # Debug
                self.update_frame()
        window.destroy()
        self.paused = False
        self.update_frame()
    ...
    def queue_save_zones(self):
        if self.calibrating and self.point_zones:
            ...
            self.save_triggered = True  # Queue the save action
    ...
    def canvas_click(self, event):
        ...
        if self.frame is not None and canvas_width > 0 and canvas_height > 0:
            ...
            x_frame = x
            y_frame = y
            print(f"Warning: Invalid canvas or frame dimensions, using raw coordinates ({x_frame}, {y_frame})")
        ...
        # Use custom dialog for point input
        self.paused = True
        dialog = CustomDialog(self.root, "Points", f"Points for zone at ({x_frame}, {y_frame}):")
        self.current_entry = dialog.show()
        print(f"Dialog returned: {self.current_entry}")  # Debug dialog result
        if self.current_entry is not None and self.current_entry.strip():  # Check for valid input
            try:
                ...
                print("Invalid points value, removing circle")
        ...
        if self.zone_count >= TOTAL_ZONES:
            print("Reached 21 zones, queuing save and transition")
            self.save_triggered = True  # Queue the save action
    ...
    def update_frame(self):
        ...
        if self.save_triggered and self.calibrating:
            if self.point_zones:
                ...
                print("Zones saved, transitioning to scoring mode")
