...
from menu_utils import _draw_button # Keep if used for splash/game over
...
def _draw_game_over_screen(frame: np.ndarray, game_state: GameState) -> None:
    ...
    if game_over_splash_cache is None:
        ...
        if os.path.exists(splash_path):
            try:
                ...
                if splash is not None: splash_resized=cv2.resize(splash, (frame.shape[1], frame.shape[0])); game_over_splash_cache = splash_resized
                else: game_over_splash_cache = "fallback"
            except Exception as e: logger.error(f"Error loading game_over.png: {e}"); game_over_splash_cache = "fallback"
        else: game_over_splash_cache = "fallback"
    if isinstance(game_over_splash_cache, np.ndarray): frame[:, :] = game_over_splash_cache.copy()
        ...
        title_text="You Win!" if game_state.win_condition_met else "Game Over!"; title_color=UIConstants.GREEN if game_state.win_condition_met else UIConstants.RED
        (tw,th),_=cv2.getTextSize(title_text,cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_XLARGE,UIConstants.FONT_THICKNESS+1); title_x=(frame.shape[1]-tw)//2; title_y=frame.shape[0]//3
        ...
        score_text=f"Final Score: {game_state.score}"; (sw,sh),_=cv2.getTextSize(score_text,cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_LARGE,UIConstants.FONT_THICKNESS); score_x=(frame.shape[1]-sw)//2; score_y=title_y+th+30
    ...
    button_width,button_height,button_y,button_spacing=(200,50,frame.shape[0]-90,60); total_button_width=button_width*2+button_spacing; start_x=(frame.shape[1]-total_button_width)//2
    new_game_x=start_x; new_game_rect=(new_game_x,button_y,button_width,button_height); _draw_button(frame,new_game_x,button_y,button_width,button_height,"New Game (N)",UIConstants.CV2_BLUE)
    action_new_game="new_game_from_gameover"; leaderboard_x=new_game_x+button_width+button_spacing; leaderboard_rect=(leaderboard_x,button_y,button_width,button_height)
    _draw_button(frame,leaderboard_x,button_y,button_width,button_height,"Leaderboard (L)",UIConstants.CV2_BLUE); action_leaderboard="show_leaderboard_from_gameover"
    ...
    if hasattr(game_state,"menu_pos"): game_state.menu_pos,game_state.menu_width,game_state.menu_height=((0,0),frame.shape[1],frame.shape[0])
...
def display_modal_splash(
    ...
    """
    # (Code unchanged)
    logger.info("Displaying modal splash screen..."); splash_path=GameConstants.SPLASH_SCREEN_FILE; splash_image=None
    try:
        if os.path.exists(splash_path):
            splash=cv2.imread(splash_path)
            if splash is not None and splash.size>0: splash_image=cv2.resize(splash,(UIConstants.WINDOW_WIDTH,UIConstants.WINDOW_HEIGHT),interpolation=cv2.INTER_AREA)
    ...
    display_frame=np.zeros((UIConstants.WINDOW_HEIGHT,UIConstants.WINDOW_WIDTH,3),dtype=np.uint8); display_frame[:,:]=UIConstants.BLACK
    ...
    else: message="Splash Image Unavailable. Click or press any key..."; text_color=UIConstants.RED; cv2.putText(display_frame,"Error: Splash Not Found",(50,UIConstants.WINDOW_HEIGHT//2),cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_LARGE,UIConstants.RED,2)
    (tw,th),_=cv2.getTextSize(message,cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_MEDIUM,UIConstants.FONT_THICKNESS); cv2.putText(display_frame,message,((UIConstants.WINDOW_WIDTH-tw)//2,UIConstants.WINDOW_HEIGHT-50),cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_MEDIUM,text_color,UIConstants.FONT_THICKNESS)
    ...
    while True:
        ...
        except Exception as e: logger.error(f"Error during modal splash loop: {e}"); break
...
def show_splash_screen(supabase_url: str, supabase_key: str) -> Optional[GameState]:
    ...
    # (Code mostly unchanged, except for the corrected 'else' block)
    logger.info("Showing splash screen...")
    splash_path=GameConstants.SPLASH_SCREEN_FILE; splash=cv2.imread(splash_path)
    if splash is None:
        ...
        try: cv2.namedWindow(UIConstants.WINDOW_NAME,cv2.WINDOW_NORMAL); cv2.imshow(UIConstants.WINDOW_NAME,splash); cv2.waitKey(3000)
    ...
    try:
        ...
        if not game_state.camera_available:
            ...
            first_frame=game_state.static_frame.copy(); first_frame=cv2.resize(first_frame,(UIConstants.WINDOW_WIDTH,UIConstants.WINDOW_HEIGHT))
        else:
            ret, frame_read=game_state.cap.read()
            if not ret or frame_read is None: logger.error("Failed camera capture."); clean_exit(game_state.cap,game_state.background_music,game_state.background_music_on,game_state); return None
        ...
        # Simplified error display on splash screen
        ...
        cv2.imshow(UIConstants.WINDOW_NAME, splash); cv2.waitKey(5000)
        ...
        # --- Start CORRECTED Block ---
            ...
            try:
                logger.debug("Attempting to destroy window after init failure (no game_state)...")
                cv2.destroyWindow(UIConstants.WINDOW_NAME)
                cv2.waitKey(1) # Short wait might help window close
            except Exception as destroy_err: # Catch any error during destroy
                logger.error(f"Error destroying window during init failure cleanup: {destroy_err}")
        # --- End CORRECTED Block ---
        return None # Ensure return None follows the corrected else block
    ...
    while(time.time()-start_time < GameConstants.SPLASH_DURATION+GameConstants.FADE_DURATION):
        ...
        except cv2.error: logger.info("Window check failed, assuming closed."); clean_exit(game_state.cap,game_state.background_music,game_state.background_music_on,game_state); return None
        ...
        try:
            if splash.shape!=first_frame.shape:
                ...
                if elapsed<GameConstants.SPLASH_DURATION: display_frame=splash.copy(); cv2.putText(display_frame,"Ready...",(50,UIConstants.WINDOW_HEIGHT-50),cv2.FONT_HERSHEY_SIMPLEX,UIConstants.FONT_SCALE_MEDIUM,UIConstants.YELLOW,UIConstants.FONT_THICKNESS)
                elif elapsed<GameConstants.SPLASH_DURATION+GameConstants.FADE_DURATION: alpha=max(0.0,1.0-(elapsed-GameConstants.SPLASH_DURATION)/GameConstants.FADE_DURATION) if GameConstants.FADE_DURATION>0 else 0.0; display_frame=cv2.addWeighted(splash,alpha,first_frame,1.0-alpha,0)
