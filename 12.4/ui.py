...
def draw_ui(frame: np.ndarray, game_state: GameState) -> None:
    ...
    if game_state.current_state == CurrentGameState.PLAYING:
        ...
        if not game_state.drawing: # Only draw menu button if not drawing zone
        ...
        if (
            ...
            cv2.rectangle(
                frame, (zx, zy), (zx + zw, zy + zh), UIConstants.ZONE_EDIT_SELECTED_COLOR, 3
            )
            # Draw handles
    ...
    if game_state.current_state != CurrentGameState.GETTING_PLAYER_NAME:
        ...
        if game_state.notification_text and game_state.notification_timer > 0:
            ...
            _draw_text_with_background(
                frame, game_state.notification_text, (nx_not, ny_not),
                UIConstants.FONT_SCALE_MEDIUM, color, UIConstants.BLACK,
                thickness=UIConstants.FONT_THICKNESS, alpha=0.7,
            )
            notification_drawn = True

        # Draw achievement higher up only if regular notification is NOT showing or timed out
        if (
            game_state.achievement_notification and
            game_state.achievement_notification_timer > 0
        ):
            ach_text = game_state.achievement_notification
            (tw_ach, th_ach), _ = cv2.getTextSize(
                ach_text, cv2.FONT_HERSHEY_SIMPLEX,
                UIConstants.FONT_SCALE_LARGE, UIConstants.FONT_THICKNESS,
            )
            # Position achievement higher if regular notification is present, otherwise lower
            ach_y_offset = 80 if notification_drawn and game_state.notification_timer > 0 else 30
            nx_ach = (UIConstants.WINDOW_WIDTH - tw_ach) // 2
            ny_ach = UIConstants.WINDOW_HEIGHT - ach_y_offset
            _draw_text_with_background(
                frame, ach_text, (nx_ach, ny_ach),
                UIConstants.FONT_SCALE_LARGE, UIConstants.GREEN, UIConstants.BLACK,
                thickness=UIConstants.FONT_THICKNESS, alpha=0.7,
            )
    ...
    if (
        game_state.current_state != CurrentGameState.GETTING_PLAYER_NAME and
        hasattr(game_state, "show_debug_overlay") and game_state.show_debug_overlay
    ):
