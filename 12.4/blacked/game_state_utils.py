...
def initialize_achievements() -> List[Achievement]:
    ...
    return [
        ...
        Achievement(
            "Marathon",
            "Play 10 games",
            lambda gs: hasattr(gs.get_current_player(), "games_played") and gs.
            get_current_player().games_played >= 10,
        ),
    ]
    ...
    try:
        if os.path.exists(achievements_file) and os.path.getsize(
            ...
            for achievement in game_state.achievements:
                if achievement.name in data and data.get(achievement.name,
                                                         {}).get("unlocked"):
                    achievement.unlocked = True
                    loaded_count += 1
            logger.info(
                f"Loaded {loaded_count} achievements from {achievements_file}."
            )
        else:
            ...
            for key in hsv_ranges.keys():
                if (key in loaded_data and isinstance(loaded_data[key], list)
                    ...
                    if lower.shape == (3, ) and upper.shape == (3, ):
                        ...
                        logger.warning(
                            f"Invalid HSV shape for '{key}'. Using default.")
                else:
                    logger.warning(
    ...
    try:
        if os.path.exists(settings_file) and os.path.getsize(
            ...
            game_state.current_sound_volume = (
                ...
                if isinstance(loaded_sound_vol,
                              (int, float)) and 0.0 <= loaded_sound_vol <= 1.0
            ...
            game_state.current_music_volume = (
                ...
                if isinstance(loaded_music_vol,
                              (int, float)) and 0.0 <= loaded_music_vol <= 1.0
            ...
            if "background_music_on" in settings_data and isinstance(
                ...
                game_state.background_music_on = settings_data[
                    "background_music_on"]
...
def change_music_track(game_state: Any, new_index: int):
    ...
    if not (0 <= new_index < num_tracks):
        return
    if (new_index == game_state.selected_music_track_index
...
def set_volume(game_state: Any):
    ...
    music_vol = (game_state.current_music_volume
                 if game_state.background_music_on else 0.0)
    try:
        ...
        if game_state.background_music:
            ...
            is_playing = any(ch.get_sound() == game_state.background_music
    ...
    logger.debug(
        f"Volumes applied: Sounds={sound_vol:.2f}, Music={music_vol:.2f}")
...
def load_initial_state(game_state: Any):
    ...
    game_state.special_hole = set_special_hole(
        game_state.scoring_zones)  # Use helper imported at top
    ...
    try:
        ...
        if os.path.exists(high_score_file) and os.path.getsize(
            ...
            with open(high_score_file, "r") as f:
                data = json.load(f)
            game_state.high_score = data.get(game_state.game_mode,
                                             {}).get("high_score", 0)
            logger.info(
    ...
    for ach in game_state.achievements:
        if not ach.unlocked and ach.check(game_state):
            ...
            show_notification(game_state,
                              f"Unlocked: {ach.name}",
                              duration=5.0)  # Use helper
            play_sound(game_state, game_state.achievement_sound)  # Use helper
            newly_unlocked = True
    if newly_unlocked:
        save_achievements(
            game_state,
            GameConstants.ACHIEVEMENTS_FILE)  # save_achievements is local
...
def update_timers_and_state(game_state: Any, dt: float) -> None:
    ...
    if (game_state.current_state == CurrentGameState.PLAYING
        ...
        if (game_state.game_timer > 0 and game_state.game_timer <= 10.0
        ...
        if game_state.game_mode == "fun":
            if hasattr(game_state, "active_explosions"):
                ...
                game_state.active_explosions = [
                    exp for exp in game_state.active_explosions
                    if exp.is_active()
                ]
...
def reset_game(game_state: Any) -> None:
    ...
    # Reset player score
    if (hasattr(game_state, "players") and game_state.players and
