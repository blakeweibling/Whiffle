...
from constants import GameConstants, UIConstants, ScoringConstants # Added ScoringConstants
...
def is_ball_at_rest(
    ...
    if len(history) < history_len_needed: return False
    relevant_history = history[-history_len_needed:]; start_pos=np.array(relevant_history[0])
...
def is_ball_zone_stable(
    ...
    # (Code unchanged)
    if ball_id not in ball_zone_history: ball_zone_history[ball_id] = []
    ...
    if len(ball_zone_history[ball_id]) > stability_frames_needed: ball_zone_history[ball_id].pop(0)
    if len(ball_zone_history[ball_id]) < stability_frames_needed: return False
    if all(zone_id == current_zone_id for zone_id in ball_zone_history[ball_id]): return current_zone_id is not None
    else: return False
...
def show_notification(
    ...
    # (Code unchanged)
    game_state.notification_text = text; game_state.notification_timer = duration
    game_state.notification_color = UIConstants.RED if is_error else UIConstants.GREEN
    log_level = logging.WARNING if is_error else logging.INFO; logger.log(log_level, f"Notify: {text}")

def save_high_score(game_state: Any):
    """Saves high score data for all modes based on game_state."""
    # (Code unchanged)
    data={}; high_score_file=GameConstants.HIGH_SCORE_FILE
    try:
        if os.path.exists(high_score_file) and os.path.getsize(high_score_file)>0:
            with open(high_score_file,"r") as f: data=json.load(f)
        else: data={mode:{} for mode in ["classic","timed","fun","practice","survival"]}
        if game_state.game_mode not in data: data[game_state.game_mode] = {}
    except Exception as e: logger.error(f"Read high score fail: {e}"); data={mode:{} for mode in ["classic","timed","fun","practice","survival"]}
    player_name="Unknown"
    try: player=game_state.get_current_player(); player_name=player.name if player and hasattr(player,'name') else "Unknown"
    except Exception as e: logger.error(f"Error getting player name for high score: {e}")
    current_saved_high = data.get(game_state.game_mode,{}).get("high_score",0)
    if game_state.score > current_saved_high:
        data[game_state.game_mode]["high_score"]=game_state.score; data[game_state.game_mode]["player"]=player_name
        data[game_state.game_mode]["date"]=time.strftime("%Y-%m-%d %H:%M:%S")
        logger.info(f"Updating high score for '{game_state.game_mode}' to {game_state.score} by {player_name}")
        game_state.high_score = game_state.score # Update attribute too
    try:
        with open(high_score_file,"w") as f: json.dump(data,f,indent=4)
    except IOError as e: logger.error(f"Save high score file failed: {e}")
...
def save_score(game_state: Any, player_name: str, mode: Optional[str] = None) -> None:
    ...
    # (Code unchanged)
    final_score=game_state.score; doubled=False
    if game_state.special_hole_hit_this_session: final_score*=2; doubled=True
    score_to_save=final_score; current_mode=mode or game_state.game_mode
    if score_to_save > 0:
        logger.info(f"Saving score {player_name}: {score_to_save} Mode:{current_mode}{' (D)' if doubled else ''}")
        if hasattr(game_state,"leaderboard") and game_state.leaderboard:
            try: game_state.leaderboard.submit_score(player_name, score_to_save, current_mode)
            except Exception as e: logger.error(f"Leaderboard submit error: {e}")
        else: logger.error("Leaderboard missing.")
        if current_mode==game_state.game_mode and score_to_save>game_state.high_score:
        ...
        else: save_high_score(game_state) # Save anyway to ensure file consistency
    else: logger.info(f"Score {score_to_save}, not saving.")
...
def set_special_hole(
    ...
    # (Code unchanged)
    if not scoring_zones: return None
    try:
        valid_zones = [z for z in scoring_zones if isinstance(z, (list, tuple)) and len(z) >= 1]
        if not valid_zones: return None
        special_hole = min(valid_zones, key=lambda zone: zone[0])
        return special_hole
    except Exception as e: logger.error(f"Error finding special hole: {e}"); return None
...
def save_zones(game_state: Any) -> None:
    ...
    try:
        ...
        show_notification(game_state, "Error Saving Zones", is_error=True) # Uses helper
    ...
    if os.path.exists(zones_file_path):
        try:
            if os.path.getsize(zones_file_path) == 0:
                ...
                show_notification(game_state, "Zones File Empty, Cleared Zones") # Uses helper
                return

            with open(zones_file_path, "r") as f: loaded_data = json.load(f)
            if isinstance(loaded_data, list) and all(isinstance(z,(list,tuple)) and len(z)==5 and all(isinstance(v,(int,float)) for v in z) for z in loaded_data):
                ...
                game_state.special_hole = set_special_hole(game_state.scoring_zones) # Calls helper above
            else:
                logger.error(f"Invalid format in {zones_file_path}.")
                show_notification(game_state, "Invalid Zone File Format", is_error=True) # Uses helper
                game_state.scoring_zones = []; game_state.special_hole = None
        except Exception as e:
             logger.exception(f"Error loading/processing {zones_file_path}: {e}")
             show_notification(game_state, "Error Loading Zones", is_error=True) # Uses helper
             game_state.scoring_zones = []; game_state.special_hole = None
    else:
...
def clear_zones(game_state: Any) -> None:
    ...
    zones_file_path = GameConstants.ZONES_FILE
    if os.path.exists(zones_file_path):
        try: os.remove(zones_file_path); logger.info(f"Removed zones file: {zones_file_path}")
        except OSError as e: logger.error(f"Failed remove zones file {zones_file_path}: {e}"); show_notification(game_state, "Error Removing Zone File", is_error=True) # Uses helper
