...
def clean_exit(
    ...
    if game_state:
        try:
            ...
            if hasattr(game_state, "get_current_player"):
                ...
                else: logger.warning("Could not get valid player name during cleanup.")
            else: logger.warning("game_state missing get_current_player during cleanup.")

            # Save score if game wasn't over and score exists
            if ( player_name != "Unknown Player" and
                 hasattr(game_state, "current_state") and
                 game_state.current_state != CurrentGameState.GAME_OVER and # Use imported enum
                 hasattr(game_state, "score") and game_state.score > 0 ):
                logger.info(f"Saving score for {player_name} on exit...")
                save_score(game_state, player_name) # Use helper
            elif player_name == "Unknown Player": logger.warning("Cannot save score on exit: player name unknown.")
            else: logger.info("Score not saved on exit (game over or score is 0).")
            ...
            if hasattr(game_state, "leaderboard") and game_state.leaderboard:
                ...
                else: logger.warning("Leaderboard object has no flush_pending_scores method.")
            else: logger.debug("No leaderboard found in game_state to flush.")

            # Save zones using the utility function from helpers
            logger.info("Saving zones...")
            try: save_zones(game_state) # Use helper
            except Exception as e: logger.exception(f"Error calling save_zones during cleanup: {e}")

            # Save achievements using the utility function from utils
            logger.info("Saving achievements...")
            try: save_achievements(game_state, GameConstants.ACHIEVEMENTS_FILE) # Use util
            except Exception as e: logger.exception(f"Error calling save_achievements during cleanup: {e}")

        except Exception as e: logger.exception(f"Error during game state save/flush on exit: {e}")
    ...
    except Exception as e: logger.exception(f"Unexpected error quitting Pygame: {e}")

    # Release camera
    if cap and hasattr(cap, 'isOpened') and cap.isOpened():
        logger.debug("Attempting to release camera...")
        try: cap.release(); logger.debug("Released camera.")
        except Exception as e: logger.exception(f"Unexpected error releasing camera: {e}")
    elif cap: logger.debug("Camera was not opened, no need to release.")
    else: logger.debug("No camera object (cap) to release.")

    # Destroy all OpenCV windows
    logger.debug("Attempting to destroy OpenCV windows...")
    try: cv2.destroyAllWindows(); cv2.waitKey(1); logger.debug("cv2.destroyAllWindows() called.")
    except Exception as e: logger.debug(f"Ignoring OpenCV error destroying windows: {e}")
