...
class GameState:
    def __init__(self):
        ...
        # Added for Timed Challenges
        self.mode = "Classic"  # "Classic" or "Timed"
        self.time_left = 60  # Seconds for Timed Mode

    def reset(self):
        self.score = 0
        self.balls = 10 if self.mode == "Classic" else float('inf')  # Infinite balls in Timed Mode
        self.power_up = None
        self.power_up_duration = 0
        ...
        print(f"Game reset: Mode = {self.mode}, Score = 0, Balls = {self.balls}, Power-Up = None")
...
game = GameState()
particles = []  # Added for Particle Effects

# --- Particle Effects: New Particle Class ---
class Particle:
...
class Menu:
    def __init__(self):
        ...
        self.options = {
            ...
            "Settings": {
                ...
                "Toggle Mode (Classic/Timed)": lambda: self.toggle_mode(),  # Added for Timed Challenges
            },
            "View": {
            ...
            "Leaderboard": {
                ...
                "Toggle View (All/Daily)": lambda: leaderboard_window.toggle_view(),  # Added for Daily Leaderboards
            }
        }
    ...
    # --- Timed Challenges: New Toggle Method ---
...
class SettingsWindow:
    ...
    def draw(self, screen):
        ...
        self.window_surface.fill((200, 200, 200, 200))
        pygame.draw.rect(self.window_surface, RED, self.close_button_rect)
        labels = ["HSV Lower (H)", "HSV Lower (S)", "HSV Lower (V)", "HSV Upper (H)", "HSV Upper (S)", "HSV Upper (V)", "Volume"]
        y_offset = 40
        for label in labels:
...
class LeaderboardWindow:
    def __init__(self):
        ...
        self.close_button_rect = None
        self.scores = []
        self.view = "All-Time"  # Added for Daily Leaderboards
        self.load_scores()
    ...
    def load_scores(self):
        try:
            ...
            print(f"Loaded {self.view} scores from Supabase:", self.scores)
        except Exception as e:
            print(f"Error fetching {self.view} scores from Supabase: {e}")
            try:
    ...
    def save_score(self, initials, score, date, mode="Classic"):
        try:
            ...
            print(f"Score saved to Supabase: {initials} - {score} (Mode: {mode})")
    ...
    # --- Daily Leaderboards: New Toggle Method ---
    def toggle_view(self):
        self.view = "Daily" if self.view == "All-Time" else "All-Time"
        self.load_scores()
        print(f"Switched leaderboard view to {self.view}")
    ...
    def draw(self, screen):
        ...
        self.window_surface.fill((200, 200, 200, 200))
        pygame.draw.rect(self.window_surface, RED, self.close_button_rect)
        title = self.font.render(f"Leaderboard ({self.view})", True, BLACK)
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                self.active = False
                return True
            elif event.key == pygame.K_v:  # 'V' to toggle view
                self.toggle_view()
                return True
        return False
                        ...
                        if ball_id in game_state.confirming_balls:
                            ...
                            if dist < 10:
                                ...
                                if frames + 1 >= CONFIRMATION_FRAMES:
                                    ...
                                    # --- Particle Effects: Spawn Particles ---
                                    for _ in range(10):
                                        color = YELLOW if final_points > points else GREEN
                                        particles.append(Particle(hole_pos[0], hole_pos[1], color))
...
def draw_ui():
    ...
    score_text = font.render(f"Balls: {game.balls if game.mode == 'Classic' else '∞'}  Score: {game.score}  Time: {game.time}  Power-Up: {game.power_up or 'None'}", True, WHITE)
    ...
    try:
        ...
        if game.just_reset:
            ...
            for pos, points, is_red in zip(ball_positions, points_list, is_red_list):
                game.score += points
                if game.mode == "Classic":  # Only reduce balls in Classic Mode
                    game.balls -= 1
        ...
        for pos in game.detected_positions:
            ...
            if any(pos == game.confirming_balls[ball_id]["hole_pos"] for ball_id in game.confirming_balls if game.confirming_balls[ball_id]["frames"] < CONFIRMATION_FRAMES):
                pygame.draw.circle(screen, YELLOW, adjusted_pos, int(20 * scale_x), 2)
            else:
                pygame.draw.circle(screen, BLUE if any(pos == game.confirming_balls[ball_id]["hole_pos"] for ball_id in game.confirming_balls if cv2.mean(roi[max(0, int(pos[1]/scale_y)-2):min(roi.shape[0], int(pos[1]/scale_y)+3), max(0, int(pos[0]/scale_x)-2):min(roi.shape[1], int(pos[0]/scale_x)+3)])[2] > 100) else RED, adjusted_pos, int(20 * scale_x), 2)
        ...
        if frame_count % 30 == 0:
            if game.mode == "Timed":
                ...
                if game.time_left <= 0:
                    print("Time's up! Saving score...")
                    initials = get_initials(screen, font)
                    if initials:
                        score = game.score
                        date = time.strftime("%Y-%m-%dT%H:%M:%S")
                        leaderboard_window.save_score(initials, score, date, mode="Timed")
            ...
            if event.type == QUIT:
                ...
                if not handled:
                    if about_window.active:
                        ...
                        leaderboard_window.handle_input(event)
        # --- Timed Challenges: Classic Mode Game Over ---
        if game.mode == "Classic" and game.balls <= 0:
            ...
            if initials:
                score = game.score
                date = time.strftime("%Y-%m-%dT%H:%M:%S")
                leaderboard_window.save_score(initials, score, date, mode="Classic")
                leaderboard_window.load_scores()
            game.reset()
    except Exception as e:
