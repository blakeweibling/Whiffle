# Whiffle Playfield - Added Persistence Check for Ball Detection
# Lines 1-700
import cv2
import numpy as np
import pygame
...
class GameState:
    def __init__(self):
        ...
        self.confirming_positions = {}  # Track positions being confirmed (position: frames detected)
    ...
    def reset(self):
        ...
        self.confirming_positions.clear()
...
def detect_ball_in_hole(image, hole_coords, game_state, frame_count):
    ...
    CONFIRMATION_FRAMES = 5  # Number of consecutive frames required to confirm a ball in a hole
    ...
    for (x, y, radius, points) in hole_coords:
        ...
        # Check if this position is on cooldown (after scoring)
        if pos in game_state.detection_cooldown:
            last_detected_frame = game_state.detection_cooldown[pos]
            if frame_count - last_detected_frame < COOLDOWN_FRAMES:
                continue  # Skip this hole, it's on cooldown

        # Check if a ball is detected in this frame
        if white_pixels > 0.2 * total_pixels:
            ...
            for contour in contours:
                ...
                if area > 0.1 * total_pixels and area < total_pixels:
            ...
            if ball_detected:
                # Increment the confirmation counter for this position
                if pos in game_state.confirming_positions:
                    game_state.confirming_positions[pos] += 1
                else:
                    game_state.confirming_positions[pos] = 1

                # Add to detected positions for visual feedback (yellow circle while confirming)
                if pos not in game_state.detected_positions:
                    game_state.detected_positions.append(pos)

                # Check if the ball has been detected for enough frames to confirm
                if game_state.confirming_positions[pos] >= CONFIRMATION_FRAMES:
                    if pos not in game_state.scored_balls:
                        ball_positions.append(pos)
                        points_list.append(points)
                        game_state.scored_balls.add(pos)
                        game_state.detection_cooldown[pos] = frame_count
                        print(f"Ball confirmed and scored at {pos}, Points: {points}")
            else:
                # No valid contours found, reset the counter
                if pos in game_state.confirming_positions:
                    del game_state.confirming_positions[pos]
                if pos in game_state.detected_positions:
                    game_state.detected_positions.remove(pos)
        else:
            # No ball detected in this frame, reset the counter
            if pos in game_state.confirming_positions:
                del game_state.confirming_positions[pos]
            if pos in game_state.detected_positions:
                game_state.detected_positions.remove(pos)
...
while running and game.running:
    ...
    for pos in game.detected_positions:
        if pos in game.confirming_positions and game.confirming_positions[pos] < 5:
