...
from game_state import MenuState  # Add this import to access MenuState Enum
...
def _update_score(frame: np.ndarray, game_state: Any, tracked_detected_balls: List[Tuple[int, int, float, int]]) -> None:
    for x, y, radius, ball_id in tracked_detected_balls:
        ...
        if (current_zone and
            ...
            current_player.add_score(current_zone[4])  # Add points to the current player
...
def _handle_input(game_state: Any) -> int:
    ...
    if key == ord('q'):
        ...
        game_state.menu_active = False  # Ensure menu is fully closed to prevent state inconsistency
...
def run_game_loop(game_state: Any) -> None:
    ...
    while True:
        frame = _capture_frame(game_state.cap, game_state)  # Updated to pass game_state
        if frame is None:
        ...
        game_state.check_achievements()  # Check for unlocked achievements
        game_state.update_achievement_notification(1.0 / GameConstants.FRAME_RATE)  # Update notification timer
