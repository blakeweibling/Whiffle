...
from utils import clean_exit, mouse_callback  # Added mouse_callback import
from constants import UIConstants  # Updated to import UIConstants class
...
def main() -> None:
    ...
    cv2.setMouseCallback(UIConstants.WINDOW_NAME, mouse_callback, game_state)  # Updated

    print("Press 'q' to quit, 's' to start drawing a scoring zone, 'd' to toggle debug mode")

    ret, frame = game_state.cap.read()
    if ret:
        cv2.imshow(UIConstants.WINDOW_NAME, frame)  # Updated
        cv2.waitKey(100)
    ...
    try:
        ...
        game_state.save_score(game_state.get_current_player().name)  # Updated to save current player's score
