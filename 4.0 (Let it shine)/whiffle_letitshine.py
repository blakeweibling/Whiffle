...
SUPABASE_URL = "https://jtkbujumrobglftzokcs.supabase.co"  # Your provided URL
SUPABASE_API_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp0a2J1anVtcm9iZ2xmdHpva2NzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDIwMTM4NzcsImV4cCI6MjA1NzU4OTg3N30.OibLuqr3X922SUSBL8yGxDw8uwuTjivH97-2wNhJDqs"  # Your provided anon key
...
def load_high_score():
    ...
    try:
        ...
        print(f"Fetching leaderboard with headers: {headers}")  # Debug headers
        ...
        print(f"Fetched leaderboard: {leaderboard}")  # Debug fetched data
        return leaderboard
...
def save_high_score(initials="N/A", new_score=None, leaderboard=None):
    ...
    print(f"Attempting to save high score with initials: {initials}, score: {new_score}")  # Debug attempt
    # Save to Supabase
    try:
        headers = {
            "apikey": SUPABASE_API_KEY,
        ...
        print(f"Sending request with headers: {headers}")  # Debug headers
        data = {
            "initials": initials if initials and initials.strip() else "N/A",  # Ensure initials is not empty
            "score": new_score,
            "created_at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00", time.gmtime())
        }
        print(f"Sending data: {data}")  # Debug data
        ...
        print(f"Error uploading score to Supabase: {e.response.text if hasattr(e.response, 'text') else str(e)}")  # Detailed error
...
class CustomDialog:
    def __init__(self, parent, title, prompt, show_special_option=False):
        ...
        self.root.grab_set()
        self.root.protocol("WM_DELETE_WINDOW", self.cancel)  # Handle close button
        self.result = None
        ...
        self.entry.focus_set()  # Ensure the entry has focus immediately
        ...
        self.root.bind("<Return>", self.on_return)  # Bind Enter key to ok
        self.root.bind("<Escape>", self.on_escape)  # Bind Escape key to cancel

    def on_return(self, event=None):
        print("Enter key pressed")  # Debug
        self.ok()

    def on_escape(self, event=None):
        print("Escape key pressed")  # Debug
        self.cancel()

    def ok(self):
        self.result = self.entry.get().strip()  # Strip whitespace
        self.is_special = getattr(self, 'special_var', tk.BooleanVar(value=False)).get()
        if not self.result:
            self.result = "N/A"  # Default to N/A if empty
        print(f"Dialog OK clicked, result: '{self.result}', is_special: {self.is_special}")  # Debug
        self.root.grab_release()
        self.root.destroy()

    def cancel(self):
        print("Dialog cancelled")  # Debug
        self.result = "N/A"  # Set to N/A on cancel to avoid None
        self.is_special = False
        self.root.grab_release()
        self.root.destroy()
    ...
    def show(self):
        ...
        print(f"Dialog show returning: result: '{self.result}', is_special: {self.is_special}")  # Debug
...
class WhiffleGame:
    ...
    def canvas_click(self, event):
        ...
        # Show special hole option after 20 zones (TOTAL_ZONES - 1)
        show_special_option = not self.special_hole_defined and self.zone_count >= TOTAL_ZONES - 1
        print(f"Calibration click: zone_count={self.zone_count}, special_hole_defined={self.special_hole_defined}, show_special_option={show_special_option}")  # Debug
        self.paused = True
        ...
        if points is not None and points.strip():
            try:
                ...
                if is_special and not self.special_hole_defined and self.zone_count >= TOTAL_ZONES - 1:
                    ...
                    print(f"Special hole defined at ({x_frame}, {y_frame}) with points {points}")  # Debug
                    ...
                    self.zone_count += 1
                    print(f"Added regular zone {self.zone_count} at ({x_frame}, {y_frame}) with points {points}")  # Debug
                    circle_id = self.canvas.create_oval(x - radius, y - radius, x + radius, y + radius, 
                ...
                if len(self.point_zones) < self.zone_count:
                    self.zone_count -= 1  # Adjust zone_count on invalid input
        self.paused = False
    ...
    def update_frame(self):
        ...
        if self.calibrating:
            ...
            if current_score > high_score and not self.new_high_score_prompted:
                ...
                while True:  # Loop until valid input or explicit cancel
                    dialog = CustomDialog(self.root, "New High Score!", "Enter your initials (3 letters):")
                    initials, _ = dialog.show()
                    print(f"Received initials from dialog: '{initials}'")  # Debug initials
                    if initials and len(initials) > 0 and initials.strip():
                        ...
                        break
                    elif initials == "N/A":  # Explicit cancel with N/A
