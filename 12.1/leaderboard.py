...
class Leaderboard:
        ...
        try:
            with open(self.local_file, "r", encoding="utf-8") as f:
                ...
                for mode in VALID_MODES:
                    if mode not in data:
                        logger.warning(f"Mode '{mode}' missing in {self.local_file}, initializing empty list.")
                        data[mode] = []
                    elif not isinstance(data[mode], list):
                         logger.warning(f"Data for mode '{mode}' in {self.local_file} is not a list, resetting.")
                         data[mode] = []
                return data
                # --- END UPDATE ---
    ...
    def submit_score(self, player_name: str, score: int, mode: str) -> bool:
        ...
        if not isinstance(mode, str) or mode not in VALID_MODES:
            logger.warning(f"Invalid mode '{mode}' for score submission, defaulting to 'classic'")
            mode = "classic"
        # --- END UPDATE ---
    ...
    def get_top_scores(
        ...
        # --- UPDATED: Include 'survival' in mode validation ---
        if not isinstance(mode, str) or mode not in VALID_MODES:
            logger.warning(f"Invalid or unknown mode '{mode}' for get_top_scores, defaulting to 'classic'")
            mode = "classic"
        # --- END UPDATE ---
