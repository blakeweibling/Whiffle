...
def _draw_leaderboard_submenu(menu_frame: np.ndarray, game_state: "GameState") -> None:
    ...
    mode_button_y = y_offset + 5
    mode_button_height = 30
    mode_button_width = (menu_frame.shape[1] - 40 - 10) // 2
    classic_x = 20
    timed_x = classic_x + mode_button_width + 10
...
def _draw_help_submenu(menu_frame: np.ndarray, game_state: "GameState") -> None:
    ...
    game_state.submenu_items.clear()
    help_text = [
        "Controls:", "  'm' / Click Menu Btn : Toggle Menu", "  's' : Start/Stop drawing zone (while playing)",
        "  'd' : Toggle General Debug Logs", "  'b' : Toggle Visual Debug Overlay", "  'p' : Pause/Resume Game (while playing)",
        "  'q' / ESC: Quit Game", "  BACKSPACE: Go back in menu / Close menu", "", "Gameplay:",
        "  - Define scoring zones via Menu > Manage Zones.", "  - Edit zones via Menu > Manage Zones > Edit Zones.",
        "       - Use Pts/Move/Resize/Del buttons.", "      - When Move/Resize active, click zone/handle in main window.",
        "      - Press ESC to cancel Move/Resize.", "  - Score points by getting balls into zones.",
        "  - Leftmost zone is the 'Special Hole'.", "  - Change modes in Menu > Game Mode.", "  - Edit player names in Menu > Players."
    ]
...
def _draw_faq_submenu(menu_frame: np.ndarray, game_state: "GameState") -> None:
    ...
    game_state.submenu_items.clear()
    faq_text = [
        "Q: Why aren't balls being detected?", "A: Check lighting.", "   Use Debug Overlay ('b') to see tracked balls.",
        "   Check YOLO model confidence if needed.", "", "Q: How do I define scoring zones accurately?",
        "A: Use 's' key, click and drag.", "   Use Edit Zones menu to delete/modify points.",
        "   Use Move/Resize buttons for interactive edit.", "", "Q: What is the 'Special Hole'?",
        "A: The leftmost zone, potentially for bonus points", "   or specific achievements.", "",
        "Q: Why is the Edit Zones list cut off?", "A: Pagination has been added. Use Prev/Next buttons."
    ]
...
def _draw_settings_submenu(menu_frame: np.ndarray, game_state: "GameState") -> None:
    ...
    # --- ADDED Debug log at function start ---
    ...
    for item in settings_toggles:
        ...
        if item.action == "toggle_background_music":
            logger.debug(f"Drawing Music Track selector because item action is 'toggle_background_music'") # Confirmation Log
            # Draw Label
    ...
    item_height = 35
    _draw_button(
        menu_frame, 20, back_y, menu_frame.shape[1] - 40, item_height,
