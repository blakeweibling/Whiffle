        ...
        if game_state.current_state == CurrentGameState.GETTING_PLAYER_NAME:
            if key == 13:  # Enter
                ...
                if char in PlayerConstants.ALLOWED_PLAYER_NAME_CHARS:
                    if (len(game_state.current_player_name_input)
            ...
            if (game_state.submenu_active == "players"
                ...
                if key == 13:  # Enter
                    ...
                    char = chr(key)
                    if char in PlayerConstants.ALLOWED_PLAYER_NAME_CHARS:
                        if (len(game_state.editing_player_name_input)
                                < PlayerConstants.MAX_PLAYER_NAME_LENGTH):
            ...
            if not key_handled_globally:
                if key == ord("m"):
                    ...
                    game_state.menu_cache = None
                    key_handled_globally = True  # Resume
                elif key == 8:  # Backspace
            ...
            if key == ord("n"):
                ...
                key_handled_globally = True  # Use reset_game from utils
            elif key == ord("l"):
                game_state.current_state = CurrentGameState.MENU
            ...
            if key == 27:  # Esc cancels editing
                ...
                game_state.menu_cache = None
                key_handled_globally = True  # Use helper
        # Global Toggles...
        if not key_handled_globally:
            if key == ord("d"):
