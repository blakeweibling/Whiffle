...
class WhiffleGame:
    def __init__(self, root):
        ...
        self.root.configure(bg="#2E2E2E")  # Ensure dark theme
        self.root.resizable(True, True)
        ...
        self.canvas = tk.Canvas(self.root, bg="#2E2E2E", highlightthickness=2, highlightbackground="#2196F3")  # Add border
        ...
        self.input_text = ""
        self.paused = False
        self.zone_circles = []  # List to store circle IDs on canvas

        # Bind 's' key for saving zones
        self.root.bind('<s>', self.save_zones)
    ...
    def load_file(self, window):
        ...
        if filename:
            ...
            if loaded_zones:
                ...
                self.update_frame()  # Refresh canvas with loaded zones
        window.destroy()
    ...
    def canvas_click(self, event):
        ...
        # Draw blue circle on canvas
        radius = int(CALIBRATION_VISUAL_RADIUS * (canvas_width / frame_width))
        ...
        if self.current_entry:
            try:
                ...
                self.canvas.create_text(x, y + radius + 10, text=str(points), fill="white", font=("Helvetica", 10))
            except ValueError:
                self.clicked_points.pop()
                self.canvas.delete(self.zone_circles.pop())  # Remove circle if input is invalid
        self.paused = False
