...
def reset_game(game_state: Any) -> None:
    ...
    # Update the special hole after clearing zones
    game_state._set_special_hole()
    if game_state.debug_mode:
        logger.info("Game reset: All state variables cleared, menu closed, special hole updated")

def save_zones(scoring_zones: List[Tuple[int, int, int, int, int]], game_state: Any) -> bool:
    """
    Cache scoring zones in memory instead of writing to disk immediately.

    Args:
        scoring_zones: List of scoring zones to save.
        game_state: The current game state to store the cache.

    Returns:
        bool: True (indicating the cache was updated).
    """
    game_state.scoring_zones_cache = scoring_zones  # Cache in memory (Change 4)
    logger.info("Scoring zones cached in memory")
    return True

def flush_scoring_zones(game_state: Any) -> None:
    """
    Write cached scoring zones to disk.

    Args:
        game_state: The current game state containing the cached scoring zones.
    """
    if hasattr(game_state, 'scoring_zones_cache'):
        try:
            with open(UIConstants.SCORING_ZONES_FILE, "w", encoding='utf-8') as f:
                json.dump(game_state.scoring_zones_cache, f)
...
def load_zones(scoring_zones: List[Tuple[int, int, int, int, int]], game_state: Any) -> List[Tuple[int, int, int, int, int]]:
    """
    Load scoring zones from cache if available, otherwise from a JSON file if it exists.

    Args:
        scoring_zones: Current list of scoring zones to extend.
        game_state: The current game state to check for cached zones.

    Returns:
        Updated list of scoring zones.
    """
    if hasattr(game_state, 'scoring_zones_cache'):
        scoring_zones.extend(game_state.scoring_zones_cache)
        logger.info("Scoring zones loaded from cache")
    elif os.path.exists(UIConstants.SCORING_ZONES_FILE):
...
def clear_zones(scoring_zones: List[Tuple[int, int, int, int, int]], game_state: Any) -> List[Tuple[int, int, int, int, int]]:
    """
    Clear saved scoring zones and reset the current zones.

    Args:
        scoring_zones: Current list of scoring zones to clear.
        game_state: The current game state to clear the cache.
    ...
    scoring_zones.clear()
    if hasattr(game_state, 'scoring_zones_cache'):
        game_state.scoring_zones_cache = []  # Clear the cache (Change 4)
...
def _draw_menu_content(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    from submenus import (
        ...
        _draw_game_mode_submenu,  # Added import for Game Mode submenu
        _draw_help_submenu,
        _draw_faq_submenu,
        _draw_about_submenu,
    ...
    elif game_state.submenu_active == "Game Mode":
        _draw_game_mode_submenu(menu_frame, game_state)
...
def draw_menu_window(frame: np.ndarray, game_state: Any) -> None:
    ...
    if not game_state.menu_active:
        return
    # Cache the menu frame and only redraw when the submenu changes (Change 6)
    if not hasattr(game_state, 'menu_frame_cache') or game_state.submenu_active != game_state.last_submenu_active:
        menu_frame = _create_menu_base(game_state)
        _draw_menu_content(menu_frame, game_state)
        game_state.menu_frame_cache = menu_frame
        game_state.last_submenu_active = game_state.submenu_active
        logger.debug("Menu frame cached due to submenu change or first draw")
    _overlay_menu(frame, game_state.menu_frame_cache, game_state)