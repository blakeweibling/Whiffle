...
def _process_zone_editing_event(
    ...
    if zone_idx is None or not (0 <= zone_idx < len(game_state.scoring_zones)):
        logger.warning(
            "Zone editing event processed with invalid selected_zone_for_edit."
        )
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if game_state.drag_start_pos and game_state.zone_editing_action:
            ...
            if fw < min_size or fh < min_size:
                show_notification(
                    game_state,
                    f"Zone too small! Min size {min_size}. Reverted.",
                    is_error=True,
                    duration=3.0,
                )
        ...
        if game_state.drawing:
            ...
            if game_state.temp_zone:
                ...
                if (
                    ...
                    show_notification(
                        game_state,
                        f"Zone too small (Min: {ScoringConstants.MIN_ZONE_SIZE}x{ScoringConstants.MIN_ZONE_SIZE})",
                        is_error=True,
                    )
...
def _process_menu_or_modal_click(x: int, y: int, game_state: GameState) -> bool:
    ...
    # Modified name to reflect it handles more than just menus now
    if game_state.current_state not in [
        CurrentGameState.MENU,
        CurrentGameState.GAME_OVER,
        CurrentGameState.CONFIRM_QUIT, # Added CONFIRM_QUIT state
    ]:
        return False

    if not all(
        hasattr(game_state, attr)
        for attr in ["menu_pos", "menu_width", "menu_height", "submenu_items"]
    ):
        logger.warning("UI attributes missing in game_state for click processing.")
        return False

    # For Menu/Game Over, clicks are relative to menu pos.
    # For CONFIRM_QUIT, ui.py sets menu_pos to (0,0) and dimensions to full screen,
    # so click coords (x,y) can be used directly against the absolute button coords
    # stored in submenu_items.
    menu_x, menu_y = game_state.menu_pos
    relative_x = x - menu_x
    relative_y = y - menu_y

    # Check if click is outside the general active area (only for MENU)
    # CONFIRM_QUIT and GAME_OVER take the whole screen conceptually for clicks
    if game_state.current_state == CurrentGameState.MENU:
        ...
        # Menu Close Button Check
        pad = UIConstants.MENU_CLOSE_BUTTON_PADDING
        size = UIConstants.MENU_CLOSE_BUTTON_SIZE
        # Coordinates relative to menu top-left
        close_btn_rel_x1 = menu_w - pad - size
        close_btn_rel_y1 = pad
        close_btn_rel_x2 = menu_w - pad
        close_btn_rel_y2 = pad + size
        ...
        if is_outside_menu:
            return False # Click outside menu area and not on close button
    ...
    for item_data in reversed(game_state.submenu_items):
        ...
        click_x_to_check = x # Use absolute screen X for CONFIRM_QUIT
        click_y_to_check = y # Use absolute screen Y for CONFIRM_QUIT
        if game_state.current_state != CurrentGameState.CONFIRM_QUIT:
            click_x_to_check = relative_x # Use relative X otherwise
            click_y_to_check = relative_y # Use relative Y otherwise

        # Check click bounds
        if (
            item_x <= click_x_to_check < item_x + item_w
            and item_y <= click_y_to_check < item_y + item_h
        ):
            logger.debug(f"Click detected on item with action: {action}")

            if isinstance(action, str):
                # Settings Sliders (only in MENU state)
                if game_state.current_state == CurrentGameState.MENU:
                    if action == "adjust_sound_volume":
                        ...
                        # break # Slider handled, stop checking this click
                ...
                # --- Quit Confirmation Buttons (only in CONFIRM_QUIT state) ---
                if game_state.current_state == CurrentGameState.CONFIRM_QUIT:
                    if action == "confirm_quit_yes":
                        ...
                        # clean_exit will terminate the program, so no further action needed.
                        # If it didn't exit, we'd need to signal loop exit.
                        return True # Click handled (exit imminent)
                        ...
                        game_state.current_state = getattr(
                            ...
                            CurrentGameState.PLAYING, # Default fallback
                        )
                        game_state.previous_state_before_quit_confirm = None # Clear the temp state
                        game_state.submenu_items = [] # Clear the Yes/No buttons
                        return True # Click handled

                # --- Other Button Actions (if not volume adjusted and not quit confirm buttons) ---
                elif not volume_adjusted:
                    if action == "quit": # Menu quit action
                    ...
                    if game_state.current_state == CurrentGameState.MENU:
                        # (Existing MENU action handling logic remains here...)
                        if action == "toggle_game_sounds":
                            ...
                            if total_tracks > 0:
                                change_music_track(game_state, (game_state.selected_music_track_index + 1) % total_tracks)
                                game_state.menu_cache = None
                            ...
                            show_notification(game_state, "Press 's', then click and drag to draw zone")
                            game_state.current_state = CurrentGameState.PLAYING
                            game_state.submenu_active = None
                            _reset_all_menu_editing_states(game_state)
                            ...
                            try:
                                ...
                                else: logger.warning(f"Invalid zone index {index} for edit points action '{action}'")
                                _reset_all_menu_editing_states(game_state)
                            except (ValueError, IndexError) as e:
                            ...
                            try:
                                ...
                                else: logger.warning(f"Invalid player index {index} for edit name action '{action}'")
                                _reset_all_menu_editing_states(game_state)
                            except (ValueError, IndexError) as e:
                                logger.error(f"Error parsing player index from action '{action}': {e}")
                                _reset_all_menu_editing_states(game_state)
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    if (game_state.editing_zone_index == index and game_state.editing_zone_mode == "confirm_delete"):
                                        ...
                                        show_notification(game_state, f"Zone {index+1} Deleted")
                                        _reset_all_menu_editing_states(game_state)
                                    else:
                                        _reset_all_menu_editing_states(game_state)
                                        game_state.editing_zone_index = index
                                        ...
                                        game_state.menu_cache = None
                                        show_notification(game_state, f"Click Delete again for zone {index+1} to confirm", duration=4.0)
                                else: logger.warning(f"Invalid zone index {index} for delete action '{action}'")
                                _reset_all_menu_editing_states(game_state)
                            except (ValueError, IndexError) as e:
                                logger.error(f"Error parsing zone index from action '{action}': {e}")
                                _reset_all_menu_editing_states(game_state)
                    ...
                    # If any button action was processed, return True
                    return True

            # If the loop continues, it means the click was on an item
            # but wasn't a string action (or was a slider).
            # If volume was adjusted, we should stop processing this click event.
            if volume_adjusted:
                 return True # Volume slider handled the click
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    # 3. Handle clicks on menu, game over, or CONFIRM_QUIT buttons (LBUTTONDOWN only)
    if (
        not click_handled
        and game_state.current_state in [
            CurrentGameState.MENU,
            CurrentGameState.GAME_OVER,
            CurrentGameState.CONFIRM_QUIT, # Added state check
        ]
        and event == cv2.EVENT_LBUTTONDOWN
    ):
        # Pass click to the unified handler function
        click_handled = _process_menu_or_modal_click(x, y, game_state)
        if click_handled:
            return # Event handled by menu/modal click logic
