...
def ensure_supabase_bucket_exists(supabase_url: str, supabase_key: str, bucket_name: str) -> bool:
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if any(bucket["name"] == bucket_name for bucket in buckets):
                logger.info(f"Bucket '{bucket_name}' already exists in Supabase Storage")
                return True
        ...
        if response.status_code == 200 or response.status_code == 201:
            logger.info(f"Successfully created bucket '{bucket_name}' in Supabase Storage")
            return True
        else:
...
def save_screenshot_locally(screenshot: Any, player_name: str, score: int, game_mode: str) -> Optional[str]:
    """
...
def upload_screenshot_to_supabase(
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            logger.error(f"Failed to upload screenshot to Supabase: {response.status_code} - {response.text}")
            return None
    except Exception as e:
