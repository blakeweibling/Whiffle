...
def get_text_input(window_name, frame, prompt, x, y):
    """Graphical text input for File menu."""
    input_text = ""
    while True:
...
def options_menu():
    """Display the Options menu in a new window."""
    ...
    frame = np.zeros((400, 400, 3), dtype=np.uint8)  # Black background

    def mouse_callback(event, x, y, flags, param):
        nonlocal selected_field, editing, saved
        if event == cv2.EVENT_LBUTTONDOWN:
            print(f"Mouse click at ({x}, {y}) in Options")  # Debug
            for i in range(len(fields)):
                field_y = 50 + i * 40
                if field_y - 15 <= y <= field_y + 15 and 50 <= x <= 350:
                    selected_field = i
                    editing = True
                    print(f"Selected field: {fields[i][0]}")  # Debug
                    break
            save_y = 300
            if save_y - 15 <= y <= save_y + 15 and 150 <= x <= 250:
                try:
                    ...
                    print("Options saved")  # Debug
                    saved = True
                except ValueError:
                    print("Invalid input in options")  # Debug
                cv2.destroyWindow(window_name)
            close_y = 350
            if close_y - 15 <= y <= close_y + 15 and 150 <= x <= 250:
                cv2.destroyWindow(window_name)
    ...
    while cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) >= 1:
        display_frame = frame.copy()
        cv2.rectangle(display_frame, (0, 0), (400, 400), (50, 50, 50), -1)  # Gray background
        
        for i, (label, _, min_val, max_val) in enumerate(fields):
            y_pos = 50 + i * 40
            color = (0, 255, 255) if i == selected_field and editing else (255, 255, 255)
            cv2.putText(display_frame, f"{label}: {values[i]}", (50, y_pos), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        
        save_y = 300
        cv2.rectangle(display_frame, (150, save_y - 15), (250, save_y + 15), (0, 255, 0), -1)
        cv2.putText(display_frame, "Save", (175, save_y + 5), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
        
        close_y = 350
        cv2.rectangle(display_frame, (150, close_y - 15), (250, close_y + 15), (0, 0, 255), -1)
        cv2.putText(display_frame, "Close", (165, close_y + 5), 
...
def help_menu():
    """Display the Help menu in a new window."""
    help_lines = [
    ...
    window_name = "Help"
    cv2.namedWindow(window_name)
    frame = np.zeros((300, 400, 3), dtype=np.uint8)  # Black background
    ...
    while cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) >= 1:
        display_frame = frame.copy()
        cv2.rectangle(display_frame, (0, 0), (400, 300), (50, 50, 50), -1)  # Gray background
        
        for i, line in enumerate(help_lines):
            y_pos = 50 + i * 40
            cv2.putText(display_frame, line, (50, y_pos), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
def set_webcam_resolution(cap):
    ...
    for width, height in resolutions:
        ...
        print(f"Trying resolution: {width}x{height}, got {actual_width}x{actual_height}")
        ...
        print(f"Failed to read frame at {width}x{height}, trying next resolution")
    print("Failed to set a working resolution.")
    return None, None
