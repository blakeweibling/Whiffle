    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            return {
                ...
                "power_ups_enabled": data.get("power_ups_enabled", True)  # New config for power-ups
            }
    return {"sound_effects_enabled": True, "tutorial_shown": False, "power_ups_enabled": True}

def save_config(sound_effects_enabled, tutorial_shown, power_ups_enabled):
    data = {
...
def calculate_score(balls, point_zones, special_hole=None):
    global scored_ball_ids, current_score  # Declare global at the top
    round_score = 0
    scored_positions = []
...
class OptionsWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        self.power_up_var = tk.BooleanVar(value=self.game.power_ups_enabled)  # New power-up toggle
        ...
        button_frame.grid(row=2, column=2, padx=5, pady=10)
    ...
    def save(self):
        ...
        try:
            ...
            save_config(not self.sound_effects_var.get(), self.game.tutorial_shown, not self.power_up_var.get())
            ...
            self.game.power_ups_enabled = self.power_up_var.get()  # Update power-up setting
            print("Options saved")
        except ValueError:
...
class WhiffleGame:
    def __init__(self, root):
        ...
        self.power_ups_enabled = True  # New attribute for power-up toggle
        ...
        self.power_ups_enabled = config["power_ups_enabled"]  # Load power-up setting
        ...
        if not self.tutorial_shown:
            ...
            save_config(self.sound_effects_enabled, self.tutorial_shown, self.power_ups_enabled)
    ...
    def new_game(self, classic=True, window=None):
        if window:
            window.destroy()
        global current_score, scored_ball_ids  # Declare global here
        current_score = 0
    ...
    def update_frame(self):
        global scored_ball_ids  # Declare global at the top to fix SyntaxError
        if self.paused:
        ...
        if self.calibrating:
            ...
            if self.power_ups_enabled:  # Only spawn if power-ups are enabled
            ...
            round_score, scored_positions = calculate_score(tracked_balls, self.point_zones, self.special_hole)
            ...
            power_up_type = None
            if self.power_ups_enabled and self.power_up_zone and self.power_up_zone.is_active():  # Only activate if enabled
        ...
        if self.power_ups_enabled and self.power_up_zone and self.power_up_zone.is_active():  # Only draw if enabled
        ...
        if key == ord('q'):
            ...
            self.zone_count = 0
            scored_ball_ids.clear()  # Removed redundant 'global scored_ball_ids'
            self.tracker = CentroidTracker(max_disappeared=5)
            self.power_up_zone = None
            self.power_up = None
            self.last_power_up_spawn = 0
