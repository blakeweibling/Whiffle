...
def options_menu(window_name, frame):
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            print(f"Mouse click at ({x}, {y})")  # Debug
...
def help_menu(window_name, frame):
    """Display the Help menu in the center with a semi-transparent background and close button."""
    help_lines = [
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            close_y = frame.shape[0] // 2 + 90
            if close_y - 15 <= y <= close_y + 15 and frame.shape[1] // 2 - 50 <= x <= frame.shape[1] // 2 + 50:
                param[0] = False
    ...
    while exit_flag[0]:
        display_frame = frame.copy()
        overlay = display_frame.copy()
        cv2.rectangle(overlay, (frame.shape[1] // 2 - 200, frame.shape[0] // 2 - 100), 
                      (frame.shape[1] // 2 + 200, frame.shape[0] // 2 + 120), (50, 50, 50), -1)
        cv2.addWeighted(overlay, 0.8, display_frame, 0.2, 0, display_frame)
        ...
        for i, line in enumerate(help_lines):
            y_pos = frame.shape[0] // 2 - 60 + i * 30
            cv2.putText(display_frame, line, (frame.shape[1] // 2 - 150, y_pos), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Draw Close button
        close_y = frame.shape[0] // 2 + 90
        cv2.rectangle(display_frame, (frame.shape[1] // 2 - 50, close_y - 15), 
                      (frame.shape[1] // 2 + 50, close_y + 15), (0, 0, 255), -1)
        cv2.putText(display_frame, "Close", (frame.shape[1] // 2 - 25, close_y + 5), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
