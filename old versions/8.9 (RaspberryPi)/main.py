...
class Game:
    def __init__(self):
        ...
        self.sound_manager = SoundManager(self.settings)  # Keep sound enabled
    ...
    def initialize_camera(self):
        print("Opening camera with V4L2 backend for Raspberry Pi")
        ...
        try:
            ...
            # Remove FPS limit to allow maximum camera frame rate
            # self.cap.set(cv2.CAP_PROP_FPS, 15)  # Commented out
            ...
            print(f"Set base resolution to {self.current_width}x{self.current_height}, FPS: {self.cap.get(cv2.CAP_PROP_FPS)}")
        ...
        while True:
            try:
                ...
                if self.initials_input:
                    ...
                    key = cv2.pollKey() & 0xFF if cv2.getWindowProperty("Game", cv2.WND_PROP_VISIBLE) >= 0 else -1
                ...
                # Remove the delay and use non-blocking key check
                key = cv2.pollKey() & 0xFF if cv2.getWindowProperty("Game", cv2.WND_PROP_VISIBLE) >= 0 else -1
