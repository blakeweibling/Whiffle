...
class ZoneCalibrator:
    def __init__(self, scoring_zones):
        ...
        self.animator = ZoneAnimator()  # Keep animations
    ...
    def calibrate_zones(self, frame, current_width, current_height):
        ...
        while self.calibrating:
            ...
            # if current_time - last_update < 0.1:  # 0.1 seconds = 10 FPS
            #     time.sleep(0.01)
            #     continue
            ...
            key = cv2.pollKey() & 0xFF if cv2.getWindowProperty("Calibration", cv2.WND_PROP_VISIBLE) >= 0 else -1
            if key == ord('q'):
                self.zones.clear()
