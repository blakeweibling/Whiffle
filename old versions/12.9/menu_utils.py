...
def _draw_button(
    ...
    Highlights the button if its rectangle matches game_state.clicked_button_rect.
    """
    try:
        # <<< ADDED LOGIC to check for click state >>>
        button_rect = (x, y, w, h)
        is_clicked = (
            hasattr(game_state, 'clicked_button_rect') and
            game_state.clicked_button_rect == button_rect
        )
