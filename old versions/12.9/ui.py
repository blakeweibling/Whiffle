...
def draw_ui(frame: np.ndarray, game_state: "GameState") -> None:
    ...
    if game_state.current_state not in [CurrentGameState.GAME_OVER]: # Show score etc even during CONFIRM_QUIT
        ...
        if (game_state.game_mode in ["timed", "survival"] and game_state.game_timer is not None and game_state.current_state in [CurrentGameState.PLAYING, CurrentGameState.PAUSED, CurrentGameState.ZONE_EDITING, CurrentGameState.CONFIRM_QUIT]):
    ...
    if game_state.current_state == CurrentGameState.PLAYING:
        draw_scoring_zones(frame, game_state.scoring_zones, game_state.special_hole) # Pass game_state
    ...
    # State: Paused
        ...
        draw_scoring_zones(frame, game_state.scoring_zones, game_state.special_hole, game_state)
    ...
    # State: Zone Editing (Interactive Move/Resize)
        ...
        if (game_state.selected_zone_for_edit is not None and 0 <= game_state.selected_zone_for_edit < len(game_state.scoring_zones)):
            ...
            try: game_state.current_state = (game_state.previous_state or CurrentGameState.MENU)
            except Exception: game_state.current_state = CurrentGameState.MENU
    ...
    # State: Confirm Quit
        ...
        _draw_button(
            frame, yes_button_x, button_y, button_width, button_height, "Yes (Y)", UIConstants.GREEN,
            # <<< ADDED game_state >>>
        ...
        _draw_button(
            frame, no_button_x, button_y, button_width, button_height, "No (N)", UIConstants.RED,
            # <<< ADDED game_state >>>
            game_state=game_state,
            font_scale=UIConstants.FONT_SCALE_MEDIUM
        )
    ...
    if game_state.game_mode in ["fun", "retro"]:
        if hasattr(game_state, "active_explosions") and isinstance(game_state.active_explosions, list):
            for explosion in list(game_state.active_explosions):
                try:
                    if explosion.is_active(): explosion.draw(frame)
