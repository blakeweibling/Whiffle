        ...
        for main_option, sub_options in self.options.items():
            ...
            if self.selected == main_option and sub_options:
                ...
                for sub_option in sub_options:
                    ...
                    max_submenu_width = max(max_submenu_width, sub_text_surface.get_width() + 20)
                submenu_width = max(200, max_submenu_width, button_rect.width + 20)
...
while running:
    try:
        ...
        # Render secret splash screen if active
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if secret_splash_active:
                    # Ignore key events while secret splash is active
                    continue
                if about_window.active:
                    ...
                    # Track key presses for secret code
                ...
                if secret_splash_active and event.type == pygame.MOUSEBUTTONDOWN:
                    # Dismiss secret splash on click
                    secret_splash_active = False
                    continue
