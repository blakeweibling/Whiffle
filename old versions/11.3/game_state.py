...
class GameState:
    def __init__(self, supabase_url: str, supabase_key: str) -> None:
        ...
        if self.background_music_on and self.background_music:
            ...
            # Don't start music until name is entered
    ...
    def update_scoring(self) -> None:
        ...
        for ball in self.tracked_balls:
            ...
            for i, z in enumerate(self.scoring_zones):
                if is_in_scoring_zone(
                    (x, y, r, ball_id), z
                ):  # Original uses (x,y,r,ball_id) - Keep this signature
                    zone, zone_idx = z, i
                    break
            ...
            if zone and stable:
                ...
                if current_time < self.zone_cooldown.get(zone_idx, 0):
                    if self.debug_mode:
                        logger.debug(
                            f"Zone {zone_idx} is on cooldown. Skipping score check for ball {ball_id}."
                        )
                ...
                if self.ball_scored_zones.get(ball_id) == zone_idx:
                    if self.debug_mode:
                        logger.debug(
                            f"Ball {ball_id} already scored in zone {zone_idx}. Skipping."
                        )
            ...
            # Clear scored status if ball leaves zone or becomes unstable
                ...
                last_scored_zone = self.ball_scored_zones.pop(
                    ball_id, None
                )  # Remove from dict and get the zone
