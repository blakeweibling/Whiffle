...
def run_game_loop(game_state: GameState) -> None:
    ...
    try:
        while True:
            ...
            # Retro Pixelation (uses draw_canvas)
            if game_state.game_mode == "retro":
                try:
                     ...
                     temp = cv2.resize(draw_canvas, (small_w, small_h), interpolation=cv2.INTER_LINEAR); draw_canvas = cv2.resize(temp, (w, h), interpolation=cv2.INTER_NEAREST)
                except Exception as pixel_error: logger.error(f"Error during retro pixelation: {pixel_error}")
