...
def _process_menu_or_modal_click(x: int, y: int, game_state: "GameState") -> bool:
    ...
    if current_state not in [CurrentGameState.MENU, CurrentGameState.CONFIRM_QUIT]:
         return False
    # --- END CHANGE ---
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    # 3. Menu/Modal Clicks (Menu, Confirm Quit) - EXCLUDES Game Over
    # 4. Game Over Screen Clicks (NEW)
    # 5. Heatmap Button Click (If Menu/Paused)
    # 6. Resolution Button Click (If Playing)
    # 7. Main Menu Button Click (If Playing)
    ...
    # --- START CHANGE: Modified Menu/Modal Click Handling ---
    # 3. Menu Items / Confirm Quit Dialog Buttons
    # Handle clicks only in MENU or CONFIRM_QUIT state
    if (not click_handled and game_state.current_state in [CurrentGameState.MENU, CurrentGameState.CONFIRM_QUIT] and event == cv2.EVENT_LBUTTONDOWN):
    ...
    # --- START CHANGE: Added Game Over Screen Click Handling ---
    # 4. Game Over Screen Buttons
    if (not click_handled and game_state.current_state == CurrentGameState.GAME_OVER and event == cv2.EVENT_LBUTTONDOWN):
        game_over_buttons = getattr(game_state, "game_over_buttons", {})
        new_game_rect = game_over_buttons.get("new_game")
        leaderboard_rect = game_over_buttons.get("leaderboard")

        if new_game_rect and isinstance(new_game_rect, tuple) and len(new_game_rect) == 4:
            ng_x, ng_y, ng_w, ng_h = new_game_rect
            if ng_x <= x < ng_x + ng_w and ng_y <= y < ng_y + ng_h:
                logger.info("Game Over 'New Game' button clicked.")
                game_state.click_feedback_state = (new_game_rect, time.time())
                reset_game(game_state) # Reset score, timer, session etc.
        ...
        if leaderboard_rect and isinstance(leaderboard_rect, tuple) and len(leaderboard_rect) == 4:
            lb_x, lb_y, lb_w, lb_h = leaderboard_rect
            if lb_x <= x < lb_x + lb_w and lb_y <= y < lb_y + lb_h:
                logger.info("Game Over 'Leaderboard' button clicked.")
                game_state.click_feedback_state = (leaderboard_rect, time.time())
                game_state.current_state = CurrentGameState.MENU # Go to menu state
                game_state.submenu_active = "leaderboard" # Directly show leaderboard
                game_state.menu_cache = None # Clear menu cache
                game_state.win_condition_met = False # Reset win flag
                _reset_all_menu_editing_states(game_state) # Ensure clean menu state
                click_handled = True
                return # Exit callback
    # --- END CHANGE ---


    # 5. "Show Heatmap" Button Click
    if (not click_handled and game_state.current_state in [CurrentGameState.MENU, CurrentGameState.PAUSED] and event == cv2.EVENT_LBUTTONDOWN):
        ...
        try:
            ...
            if panel_x < padding: panel_x = (current_width - panel_width) // 2; panel_y = menu_y + menu_h + padding; panel_height = total_content_height + 40
    ...
    if (not click_handled
        ...
        # Define res_button_rect using constants
        res_button_rect = (
