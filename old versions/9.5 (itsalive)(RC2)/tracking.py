...
def _match_balls(
    ...
    Returns:
        ...
        (x, y, radius, ball_id) tuples for matched balls, and matched_new_indices is a list
    ...
    try:
        ...
        new_positions = np.array([(x, y) for x, y, _ in new_balls])
        tracked_positions = np.array([(x, y) for x, y, _, _, _ in tracked_balls])
        ...
        for i, (dist, idx) in enumerate(zip(distances, indices)):
            if dist != float('inf') and idx < len(tracked_balls):  # Valid match
                new_x, new_y, radius = new_balls[i]
                _, _, _, ball_id, age = tracked_balls[idx]
                matched_balls.append((new_x, new_y, radius, ball_id))
                matched_new_indices.append(i)
                tracked_balls[idx] = (new_x, new_y, radius, ball_id, age)  # Update position
                logger.debug(f"Matched ball ID {ball_id} at ({new_x}, {new_y})")
            elif dist == float('inf'):
                logger.warning(f"No match found for new ball at ({new_balls[i][0]}, {new_balls[i][1]}) within {distance_threshold}")
        ...
        for i, (new_x, new_y, radius) in enumerate(new_balls):
            ...
            for j, (x, y, _, ball_id, age) in enumerate(tracked_balls):
            ...
            if closest_ball_idx is not None:
                _, _, _, ball_id, age = tracked_balls[closest_ball_idx]
                matched_balls.append((new_x, new_y, radius, ball_id))
                matched_new_indices.append(i)
                tracked_balls[closest_ball_idx] = (new_x, new_y, radius, ball_id, age)
                logger.debug(f"Matched ball ID {ball_id} at ({new_x}, {new_y})")
            else:
                logger.warning(f"No match found for new ball at ({new_x}, {new_y}) within {distance_threshold}")
...
def _add_new_balls(
    ...
    Returns:
        List of (x, y, radius, ball_id) tuples for all tracked balls in the current frame.
    """
    tracked_detected_balls = [(x, y, radius, ball_id) for x, y, radius, ball_id, _ in tracked_balls]
...
def track_balls(
    ...
    if not new_balls:
        ...
        return [(x, y, radius, ball_id) for x, y, radius, ball_id, _ in tracked_balls], next_ball_id
    ...
    if debug_mode:
        logger.debug(f"Tracked {len(tracked_detected_balls)} balls: "
                    f"{[(x, y, radius, ball_id) for x, y, radius, ball_id in tracked_detected_balls]}")
    else:
