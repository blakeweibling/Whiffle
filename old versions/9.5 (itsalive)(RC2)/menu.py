...
def _draw_settings_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    for i, item in enumerate(settings_items):
        ...
        cv2.putText(menu_frame, item.label, (smx + 5, smy + 20),
                    cv2.FONT_HERSHEY_SIMPLEX, UIConstants.FONT_SCALE_SMALL, UIConstants.WHITE, 1)  # Updated
        if item.label != "Start Timer":  # Non-toggle for Start Timer
            toggle_x, toggle_y = smx + 400, smy
            toggle_w, toggle_h = 50, UIConstants.SUBMENU_HEIGHT
            state = item.get_state()
            toggle_color = UIConstants.GREEN if state else UIConstants.RED  # Updated
            ...
            _draw_button(menu_frame, smx, smy, UIConstants.SUBMENU_WIDTH, UIConstants.SUBMENU_HEIGHT, item.label, UIConstants.CV2_BLUE)  # Updated
            game_state.submenu_items.append((smx, smy, UIConstants.SUBMENU_WIDTH, UIConstants.SUBMENU_HEIGHT, item.label, item.action))
...
def _draw_leaderboard_submenu(menu_frame: np.ndarray, game_state: Any) -> None:
    ...
    if not scores:
        ...
        cv2.putText(menu_frame, f"Source: {'Online' if online else 'Local'}", 
                    ...
                    cv2.FONT_HERSHEY_SIMPLEX, UIConstants.FONT_SCALE_SMALL, UIConstants.YELLOW, 1)  # Updated
