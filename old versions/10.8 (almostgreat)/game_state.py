...
class GameState:
    def __init__(self, supabase_url: str, supabase_key: str) -> None:
        ...
        if self.camera_available:
            ...
            if not self.cap.isOpened():
                logger.error(f"Failed to open camera at index {GameConstants.CAMERA_INDEX} with backend {GameConstants.CAMERA_BACKEND}, despite earlier success")
                self.camera_available = False
        else:
            logger.info("Camera not available based on configuration. Skipping camera initialization.")
            self.cap = None
        ...
        if self.camera_available:
            ...
            if w != UIConstants.WINDOW_WIDTH or h != UIConstants.WINDOW_HEIGHT:
                logger.warning(f"Cam res mismatch: Got {int(w)}x{int(h)}, expected {UIConstants.WINDOW_WIDTH}x{UIConstants.WINDOW_HEIGHT}")
            else:
    ...
    def _save_high_score(self):
        ...
        if self.high_score > current_saved_high:
            ...
            logger.info(f"Updating high score file for mode '{self.game_mode}' to {self.high_score} by {self.get_current_player().name}")
        else:
    ...
    def update_scoring(self) -> None:
        ...
        for ball in self.tracked_balls:
            ...
            # Clear scored status if ball leaves zone or becomes unstable
                ...
                logger.debug(f"Ball {ball_id} left/unstable zone {self.ball_scored_zones.get(ball_id)}. Clearing.")
        ...
        for ball_id in keys_to_remove:
            for d in [self.ball_states, self.previous_ball_states, self.ball_positions_history, self.ball_zone_history, self.balls_in_zone, self.ball_scored_zones, self.scored_cooldown, self.ball_trails]:
                d.pop(ball_id, None)