# Whiffle Playfield - Updated with Robust Video Capture Reinitialization
# Lines 1-700
import cv2
import numpy as np
import pygame
...
def reinitialize_camera():
    ...
    indices = [0, 1]  # Try multiple camera indices
    for backend in backends:
        for index in indices:
            ...
            if cap.isOpened():
                ...
                print(f"Successfully initialized camera with backend {backend} and index {index}.")
                return True
...
def calibrate_holes():
    ...
    # Reinitialize camera after calibration
...
while running and game.running:
    ...
    if frame_to_display is None:
        print("Warning: Frame is None, using black screen as fallback.")
    ...
    if game.show_leaderboard:
        try:
            response = requests.get(
                ...
                headers=headers,
                params={"select": "name,score,date", "order": "score.desc", "limit": "5"}  # Fixed syntax
            )
