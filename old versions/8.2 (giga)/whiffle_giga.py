...
class Window:
    def __init__(self, width, height):
        ...
        self.height = height
        self.surface = None
        self.close_button_rect = pygame.Rect(self.width - 40, 10, 30, 30)  # Initialize here
...
def detect_ball_in_hole(image, hole_coords, game_state, frame_count, scale_x=1.0, scale_y=1.0):
    ...
    # Adjusted red detection ranges to capture a broader range of reds
    lower_red1 = np.array([0, 20, 50])    # Lowered S threshold and extended H range
    upper_red1 = np.array([20, 255, 255])
    lower_red2 = np.array([160, 20, 50])  # Adjusted second range to start earlier
    ...
    # Add debug code to capture mouse position and print HSV values
    def mouse_callback(event, x, y, flags, param):
    ...
    key = cv2.waitKey(1) & 0xFF
    if key == ord('p'):  # Press 'p' to print HSV at the current mouse position
            ...
            if ball_id in game_state.confirming_balls:
                ...
                if dist < 10:
                    ...
                    if frames + 1 >= SETTINGS["CONFIRMATION_FRAMES"]:
                        ...
                        is_red = red_ratio > 0.2  # Lowered threshold from 0.3 to 0.2
