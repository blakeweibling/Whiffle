            ...
            if game_state.temp_zone:
                ...
                if (
                    ...
                    try:
                        ...
                        points = (
                            ScoringConstants.DEFAULT_POINTS
                        )  # Use default if input is bad or empty
...
def _process_menu_or_gameover_click(x: int, y: int, game_state: GameState) -> bool:
    ...
    is_outside_menu = not (
        menu_x <= x < menu_x + menu_w and menu_y <= y < menu_y + menu_h
    )
        ...
        if (
            ...
            if isinstance(action, str):
                ...
                if action == "adjust_sound_volume":
                    ...
                    new_volume = max(
                        0.0, min(1.0, click_offset / item_w if item_w > 0 else 0.0)
                    )
                    ...
                    if game_state.current_state == CurrentGameState.MENU:
                        if action == "toggle_game_sounds":
                            ...
                            game_state.menu_cache = (
                                None  # May need redraw if settings change appearance
                            )
                            ...
                            game_state.menu_cache = (
                                None  # Splash might overlay, force redraw
                            )
                        elif action == "resume":
                            ...
                            _reset_all_menu_editing_states(
                                game_state
                            )  # Reset menu state too
                        elif action == "save_zones":
                            save_zones(game_state)  # Use helper
                            game_state.menu_cache = (
                                None  # No visual change, but maybe good practice
                            )
                            ...
                            try:
                                ...
                                _reset_all_menu_editing_states(
                                    game_state
                                )  # Reset editing states, force redraw
                            ...
                            action == "back_to_manage_zones"
                        ):  # Navigate back from edit zones list
                            _reset_all_menu_editing_states(game_state)
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    if not (
                                        ...
                                        game_state.menu_cache = (
                                            None  # Redraw to show input field
                                        )
                                        logger.info(
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    game_state.previous_state = (
                                        CurrentGameState.MENU
                                    )  # Store state before edit
                                    game_state.current_state = (
                                        CurrentGameState.ZONE_EDITING
                                    )  # Switch to interactive state
                                    show_notification(
                                        game_state,
                            ...
                            "delete_zone_"
                        ):  # Delete Zone (requires confirmation)
                            try:
                            ...
                            action in known_submenu_nav_actions
