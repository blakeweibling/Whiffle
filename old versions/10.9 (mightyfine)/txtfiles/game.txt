...
# Updated imports: clean_exit from utils, mouse_callback from input_handler
from utils import clean_exit
from input_handler import mouse_callback # <-- Corrected import location
...
def main() -> None:
    ...
    if game_state is None:
        logger.info("Exiting due to None game_state from show_splash_screen (likely window closed or initialization failed).")
        return
    ...
    try:
        ...
        logger.info("Window property check failed after splash screen (cv2.error), assuming window closed.")
        clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state)
        return # <-- This is the corrected line 51
    ...
    ret, frame = game_state.cap.read() if game_state.camera_available else (True, game_state.static_frame.copy())
        ...
        if 'game_state' in locals() and game_state is not None:
            try:
                ...
                if hasattr(game_state, 'get_current_player') and hasattr(game_state, 'save_score'):
                    ...
                    logger.warning("game_state object does not have get_current_player or save_score method during exception handling.")
                # Perform cleanup
                clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state)
