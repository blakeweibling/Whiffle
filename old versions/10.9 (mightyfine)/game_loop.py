...
def _render_frame(frame: np.ndarray, game_state: Any) -> None:
    ...
    try:
        ...
        logger.warning(f"cv2.error during imshow (Window '{UIConstants.WINDOW_NAME}' may already be destroyed): {e}")
...
def _handle_input(game_state: Any) -> Optional[int]:
    ...
    try:
        ...
        if cv2.getWindowProperty(UIConstants.WINDOW_NAME, cv2.WND_PROP_AUTOSIZE) != -1:
         ...
         logger.warning(f"cv2.error during waitKey (Window '{UIConstants.WINDOW_NAME}' may already be destroyed): {e}")
    ...
    # (Rest of the input handling logic remains the same as previous version)
    # ... (omitted for brevity, use the logic from the previous full code snippet) ...
        ...
        if not key_handled:
            if key == ord('m'):
                ...
                if game_state.submenu_active is not None:
                    ...
                    game_state.editing_player_name_input = None
                    game_state.menu_cache = None
                    logger.debug(f"Backspace: back from {previous_submenu} to {game_state.submenu_active or 'main menu'}.")
                else:
                    game_state.current_state = CurrentGameState.PLAYING
                    game_state.menu_cache = None
                ...
                if game_state.submenu_active is not None:
                    ...
                    game_state.editing_player_name_input = None
                    game_state.menu_cache = None
                    logger.debug(f"ESC: back from {previous_submenu} to {game_state.submenu_active or 'main menu'}.")
                else:
    ...
    # Handle input for GAME_OVER state
        ...
        if key == ord('n'):
            ...
            try:
                ...
                logger.error("Failed to import reset_game function in _handle_input.")
            key_handled = True
        elif key == ord('l'):
    ...
    if not key_handled:
        if key == ord('d'):
            ...
            logger.info(f"General Debug toggled {'ON' if game_state.debug_mode else 'OFF'}")
            key_handled = True
        elif key == ord('b'):
    ...
    if key == -1:
        logger.debug("waitKey returned -1, potentially indicating closed window or no key press.")
...
def _check_window_close(game_state: Any) -> bool:
    ...
    try:
        ...
        logger.debug(f"Window property check: cv2.WND_PROP_AUTOSIZE = {window_autosize_property}") # <-- Log this property now

        # Check if the property indicates the window is closed (-1.0)
        if window_autosize_property == -1.0:
            logger.info("Window closed via red X (detected AUTOSIZE property == -1.0).")
            clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state)
            return True
    except cv2.error as e:
        logger.warning(f"cv2.error during AUTOSIZE window property check (Window '{UIConstants.WINDOW_NAME}' may already be destroyed): {e}")
        ...
        logger.error(f"Unexpected error during AUTOSIZE window property check: {e}")
    return False
# --- END FUNCTION ---
...
def run_game_loop(game_state: Any) -> None:
    ...
    while True:
        ...
        if frame is None:
            logger.debug("Received None frame from _capture_frame, breaking loop.")
        ...
        if key is None:
            ...
            break  # Exit signal received from input handler (e.g., 'q' key, or error during waitKey)

        # 2. Check for Window Close *after* processing events
        if _check_window_close(game_state):
             logger.debug("Window close detected by _check_window_close, breaking loop.")
