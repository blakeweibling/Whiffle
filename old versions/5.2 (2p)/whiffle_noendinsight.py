...
# Detection settings (using older version's wider range and lower thresholds)
BALL_COLOR_RANGE = {
    "lower_white": [0, 0, 180],  # Wider range for better white ball detection
    "upper_white": [180, 70, 255]
}
MIN_CONTOUR_AREA = 30  # Lower threshold for smaller balls
MIN_RADIUS = 6  # Lower threshold for smaller radii
...
# Power-up settings
POWER_UP_SPAWN_INTERVAL = 15  # Spawn a power-up every 15 seconds
POWER_UP_DURATION = 10  # Duration for power-up zone and timed effects (seconds)
POWER_UP_MULTIPLIER = 3  # Score multiplier for Score Multiplier power-up
POWER_UP_EXTRA_TIME = 10  # Extra time added in Timed Mode (seconds)
POWER_UP_TYPES = ["Score Multiplier", "Slow Motion", "Extra Time"]

# Game state
current_score = 0  # Will be overridden by player_scores in multiplayer mode
high_score = 0
...
def load_high_score():
    ...
    try:
        ...
        print(f"Fetching leaderboard with headers: {headers}")
        ...
        print(f"Fetched leaderboard: {leaderboard}")
        return leaderboard
...
def save_high_score(initials="N/A", new_score=None, leaderboard=None):
    ...
    print(f"Attempting to save high score with initials: {initials}, score: {new_score}")
    try:
        headers = {
            "apikey": SUPABASE_API_KEY,
            "Authorization": f"Bearer {SUPABASE_API_KEY}",
        ...
        data = {
            "initials": initials if initials and initials.strip() else "N/A",
            "score": new_score,
        ...
        print(f"Error uploading score to Supabase: {e.response.text if hasattr(e.response, 'text') else str(e)}")
        if leaderboard is None:
            leaderboard = load_high_score()
        leaderboard.append({"score": new_score, "initials": initials if initials and initials.strip() else "N/A"})
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            return {
                ...
                "power_ups_enabled": data.get("power_ups_enabled", True),
                "multiplayer_mode": data.get("multiplayer_mode", False)  # New config for multiplayer
            }
    return {"sound_effects_enabled": True, "tutorial_shown": False, "power_ups_enabled": True, "multiplayer_mode": False}

def save_config(sound_effects_enabled, tutorial_shown, power_ups_enabled, multiplayer_mode):
    data = {
...
def calculate_score(balls, point_zones, special_hole=None, multiplayer_mode=False, frame_width=None):
    global scored_ball_ids, current_score
    round_score = {"Player 1": 0, "Player 2": 0} if multiplayer_mode else 0
    scored_positions = []

    special_hole_triggered = {"Player 1": False, "Player 2": False} if multiplayer_mode else False

    for ball_x, ball_y, _, ball_id in balls:
        if ball_id not in scored_ball_ids:  # Only score if ball hasn't scored before
            player = "Player 1" if not multiplayer_mode or ball_x < frame_width / 2 else "Player 2"
            if special_hole:
                ...
                if distance <= special_radius:
                    special_hole_triggered[player] = True
                    scored_ball_ids.add(ball_id)
                    scored_positions.append((ball_x, ball_y))
                    print(f"Ball ID {ball_id} landed in special hole for {player}! Doubling total score.")
                    continue
            ...
            for zone_x, zone_y, zone_radius, points in point_zones:
                ...
                if distance <= zone_radius:
                    if multiplayer_mode:
                        round_score[player] += points
                    else:
                        round_score += points
                    scored_ball_ids.add(ball_id)
                    scored_positions.append((ball_x, ball_y))
                    print(f"Ball ID {ball_id} scored {points} points for {player}")
                    break

    if multiplayer_mode:
        if special_hole_triggered["Player 1"]:
            current_score = {"Player 1": current_score.get("Player 1", 0) * 2, "Player 2": current_score.get("Player 2", 0)}
            print(f"Total score doubled to {current_score} for Player 1")
        if special_hole_triggered["Player 2"]:
            current_score = {"Player 1": current_score.get("Player 1", 0), "Player 2": current_score.get("Player 2", 0) * 2}
            print(f"Total score doubled to {current_score} for Player 2")
    else:
        if special_hole_triggered:
            current_score = current_score * 2
            print(f"Total score doubled to {current_score}")

    return round_score, scored_positions
...
class PowerUp:
    def __init__(self, power_up_type, duration=None):
        ...
        self.duration = duration  # Duration in seconds (None for one-time use like Score Multiplier)
    ...
    def activate(self):
        self.start_time = time.time()
        self.active = True
        print(f"Power-up {self.power_up_type} activated")

    def deactivate(self):
        self.active = False
        print(f"Power-up {self.power_up_type} deactivated")

    def is_active(self):
        if not self.active:
            return False
        if self.duration is None:  # One-time use power-ups stay active until used
            return True
        elapsed = time.time() - self.start_time
        if elapsed > self.duration:
            self.deactivate()
...
class CustomDialog:
    ...
    def ok(self):
        ...
        if not self.result:
            self.result = "N/A"
        print(f"Dialog OK clicked, result: '{self.result}', is_special: {self.is_special}")
    ...
    def show(self):
        ...
        print(f"Dialog show returning: result: '{self.result}', is_special: {self.is_special}")
...
class TutorialWindow:
    def __init__(self, on_close_callback):
        ...
        self.steps = [
            ...
            "Step 1: Calibration\nClick on the playfield to define scoring zones (21 total).\nEach click prompts for points (e.g., 50, 100).\nYou can mark one zone as a special double-score hole after 20 zones!",
            "Step 2: Playing the Game\nDrop balls onto the playfield. White balls score normally!",
            ...
            "Step 4: Power-Ups\nLook for green power-up zones that appear every 15 seconds!\nLand a ball in them to activate bonuses like Score Multiplier, Slow Motion, or Extra Time (Timed Mode).",
            "Step 5: Game Modes\nChoose 'Classic Mode' for untimed play, or 'Timed Mode' for a 2-minute challenge!",
            "Step 6: Menus\nUse 'File' to save/load zones, 'New Game' to select mode, 'High Score' to see the global leaderboard, and 'Options' to adjust settings.",
...
class OptionsWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        self.root.geometry("600x400")  # Wider initial size
        self.root.configure(bg="#2E2E2E")
        ...
        style.configure("Custom.TButton", font=("Helvetica", 10), background="#4CAF50", foreground="white")
        style.map("Custom.TButton", background=[("active", "#45A049")], foreground=[("active", "white")])
        style.configure("Custom.TCheckbutton", foreground="white", background="#2E2E2E")
        style.configure("Custom.TLabel", foreground="white", background="#2E2E2E")
        style.configure("Custom.TRadiobutton", background="#2E2E2E", foreground="white")

        ttk.Label(self.frame, text="Options", font=("Helvetica", 14, "bold"), style="Custom.TLabel").pack(pady=10)

        # Row 1: Sliders
        slider_frame = ttk.Frame(self.frame, style="Custom.TFrame")
        ...
        for i, (label, value, min_val, max_val) in enumerate(self.fields):
            ttk.Label(slider_frame, text=label, font=("Helvetica", 10), style="Custom.TLabel").grid(row=0, column=i, padx=5, pady=2, sticky="w")
            self.slider_vars[label] = tk.DoubleVar(value=value)
            slider = ttk.Scale(slider_frame, from_=min_val, to=max_val, orient="horizontal", variable=self.slider_vars[label], length=100)
            slider.grid(row=1, column=i, padx=5, pady=2)
            value_label = ttk.Label(slider_frame, textvariable=self.slider_vars[label], font=("Helvetica", 10), style="Custom.TLabel")
            value_label.grid(row=2, column=i, padx=5, pady=2, sticky="w")

        # Row 2: Toggles and Buttons
        toggle_frame = ttk.Frame(self.frame, style="Custom.TFrame")
        toggle_frame.pack(fill="x", padx=5, pady=5)

        ttk.Label(toggle_frame, text="Resolution", font=("Helvetica", 10), style="Custom.TLabel").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        self.resolution_var = tk.StringVar(value="1920x1080")
        ttk.Radiobutton(toggle_frame, text="1080p (1920x1080)", value="1920x1080", variable=self.resolution_var, style="Custom.TRadiobutton").grid(row=1, column=0, padx=5, pady=2, sticky="w")
        ttk.Radiobutton(toggle_frame, text="720p (1280x720)", value="1280x720", variable=self.resolution_var, style="Custom.TRadiobutton").grid(row=2, column=0, padx=5, pady=2, sticky="w")

        self.music_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(toggle_frame, text="Mute Music", variable=self.music_var, command=self.toggle_music, style="Custom.TCheckbutton").grid(row=1, column=1, padx=5, pady=2, sticky="w")

        self.sound_effects_var = tk.BooleanVar(value=not self.game.sound_effects_enabled)
        ttk.Checkbutton(toggle_frame, text="Disable Sound Effects", variable=self.sound_effects_var, command=self.toggle_sound_effects, style="Custom.TCheckbutton").grid(row=2, column=1, padx=5, pady=2, sticky="w")

        self.power_up_var = tk.BooleanVar(value=self.game.power_ups_enabled)
        ttk.Checkbutton(toggle_frame, text="Disable Power-Ups", variable=self.power_up_var, command=self.toggle_power_ups, style="Custom.TCheckbutton").grid(row=1, column=2, padx=5, pady=2, sticky="w")

        self.multiplayer_var = tk.BooleanVar(value=self.game.multiplayer_mode)
        ttk.Checkbutton(toggle_frame, text="Enable Multiplayer", variable=self.multiplayer_var, command=self.toggle_multiplayer, style="Custom.TCheckbutton").grid(row=2, column=2, padx=5, pady=2, sticky="w")

        button_frame = ttk.Frame(toggle_frame, style="Custom.TFrame")
        button_frame.grid(row=3, column=0, columnspan=3, padx=5, pady=10)
    ...
    def save(self):
        ...
        try:
            BALL_COLOR_RANGE["lower_white"] = [
                int(self.slider_vars["Lower Hue (0-180)"].get()),
                int(self.slider_vars["Lower Sat (0-255)"].get()),
                int(self.slider_vars["Lower Val (0-255)"].get())
            ]
            BALL_COLOR_RANGE["upper_white"] = [
                int(self.slider_vars["Upper Hue (0-180)"].get()),
                int(self.slider_vars["Upper Sat (0-255)"].get()),
                int(self.slider_vars["Upper Val (0-255)"].get())
            ]
            save_config(not self.sound_effects_var.get(), self.game.tutorial_shown, not self.power_up_var.get(), self.multiplayer_var.get())
            
            # Apply resolution change
            resolution = self.resolution_var.get()
            if resolution == "1920x1080":
                new_width, new_height, _ = set_webcam_resolution(self.game.cap, (1920, 1080))
            else:  # 1280x720
                new_width, new_height, _ = set_webcam_resolution(self.game.cap, (1280, 720))
            if new_width and new_height:
                self.game.width = new_width
                self.game.height = new_height
                self.game.res_label.config(text=f"Res: {self.game.width}x{self.game.height}")
            self.game.power_ups_enabled = self.power_up_var.get()
            self.game.multiplayer_mode = self.multiplayer_var.get()  # Update multiplayer setting
            print("Options saved")
        except ValueError:
    ...
    def toggle_power_ups(self):
        self.game.power_ups_enabled = self.power_up_var.get()
        print(f"Power-ups {'disabled' if not self.game.power_ups_enabled else 'enabled'}")
        self.root.destroy()
        self.on_close_callback()

    def toggle_multiplayer(self):
        self.game.multiplayer_mode = self.multiplayer_var.get()
        print(f"Multiplayer {'enabled' if self.game.multiplayer_mode else 'disabled'}")
        self.root.destroy()
        self.on_close_callback()
...
class HelpWindow:
    def __init__(self, on_close_callback, game_instance):
        ...
        help_lines = [
            ...
            "Menu: File (save/load)",
            "New Game (select mode), High Score (view global rankings)",
            "Options (settings)",
            "Power-Ups: Land a ball in a green zone to activate a power-up!",
        ...
        ttk.Button(self.frame, text="Close", style="Custom.TButton", command=self.close).pack(pdy=5)
...
class WhiffleGame:
    def __init__(self, root):
        ...
        self.power_up_zone = None  # Current power-up zone
        self.power_up = None  # Current active power-up
        self.power_up_label = None  # Label for displaying power-up status
        self.last_power_up_spawn = 0  # Time of last power-up spawn
        self.frame_delay = 10  # Default frame delay (ms)
        self.power_ups_enabled = True  # Power-up toggle
        self.multiplayer_mode = False  # Multiplayer toggle
        self.player_scores = {"Player 1": 0, "Player 2": 0}  # Multiplayer scores
        self.frame_queue = queue.Queue()  # Queue for multi-threaded capture
        ...
        self.power_ups_enabled = config["power_ups_enabled"]
        self.multiplayer_mode = config["multiplayer_mode"]
        ...
        self.score_label = tk.Label(self.stats_frame, text="Score: 0 | 0", font=("Helvetica", 12), bg="#2E2E2E", fg="white")
        self.score_label.pack(side="left", padx=10)
        self.timer_label = tk.Label(self.stats_frame, text="Time: N/A", font=("Helvetica", 12), bg="#2E2E2E", fg="white")
        self.timer_label.pack(side="left", padx=10)
        self.power_up_label = tk.Label(self.stats_frame, text="Power-Up: None", font=("Helvetica", 12), bg="#2E2E2E", fg="white")
        self.power_up_label.pack(side="left", padx=10)
        ...
        self.width, self.height, initial_frame = set_webcam_resolution(self.cap, (1920, 1080))  # Default to 1080p
        ...
        leaderboard = load_high_score()
        current_score = 0  # Will be overridden by player_scores in multiplayer
        ...
        self.previous_balls = []
        self.last_red_score_time = 0.0  # Placeholder, not used in this simplified scoring
        ...
        if not self.tutorial_shown:
            ...
            save_config(self.sound_effects_enabled, self.tutorial_shown, self.power_ups_enabled, self.multiplayer_mode)
            TutorialWindow(self.resume_frame)

        self.root.after(10, self.update_frame)  # Fixed delay for consistent frame rate
    ...
    def spawn_power_up_zone(self):
        if not self.point_zones or not self.power_ups_enabled:  # Check if power-ups are enabled
            return
        current_time = time.time()
        if current_time - self.last_power_up_spawn < POWER_UP_SPAWN_INTERVAL:
            return
        if self.power_up_zone and self.power_up_zone.is_active():
            return
        # Choose a random zone to place the power-up
        ...
        print(f"Power-up zone spawned at ({zone[0]}, {zone[1]})")
    ...
    def new_game_menu(self):
        ...
        frame = ttk.Frame(new_game_window, padding=10, style="Custom.TFrame")
        frame.pack(fill="both", expand=True)

        tk.Label(frame, text="Select Game Mode", font=("Helvetica", 14, "bold"), bg="#2E2E2E", fg="white").pack(pady=10)
        tk.Button(frame, text="Classic Mode", font=("Helvetica", 10), bg="#4CAF50", fg="white", 
                  command=lambda: self.new_game(classic=True, window=new_game_window)).pack(pady=5)
        tk.Button(frame, text="Timed Mode (2m)", font=("Helvetica", 10), bg="#2196F3", fg="white", 
                  command=lambda: self.new_game(classic=False, window=new_game_window)).pack(pady=5)
    ...
    def new_game(self, classic=True, window=None):
        ...
        if self.multiplayer_mode:
            current_score = {"Player 1": 0, "Player 2": 0}  # Reset multiplayer scores
            self.player_scores = {"Player 1": 0, "Player 2": 0}
            self.score_label.config(text=f"Player 1: 0 | Player 2: 0")
        else:
            current_score = 0
            self.player_scores = {"Player 1": 0}  # Single player mode
        ...
        self.frame_delay = 10  # Reset frame delay
    ...
    def start_timer(self):
        if self.time_remaining > 0:
            ...
            self.paused = True
            if self.multiplayer_mode:
                winning_player = max(self.player_scores, key=self.player_scores.get)
                if self.player_scores[winning_player] > high_score and not self.new_high_score_prompted:
                    self.new_high_score_prompted = True
                    while True:
                        dialog = CustomDialog(self.root, "New High Score!", f"Enter initials for {winning_player} (3 letters):")
                        initials, _ = dialog.show()
                        print(f"Received initials from dialog: '{initials}'")
                        if initials and len(initials) > 0 and initials.strip():
                            initials = initials.upper()[:3]
                            save_high_score(initials, self.player_scores[winning_player])
                            print(f"Submitted new high score to online leaderboard: {initials} - {self.player_scores[winning_player]}")
                            break
                        elif initials == "N/A":
                            save_high_score("N/A", self.player_scores[winning_player])
                            print(f"Submitted new high score (no valid initials) to online leaderboard: {self.player_scores[winning_player]}")
                            break
                        else:
                            print("Invalid input or cancelled, please try again.")
            else:
                if current_score > high_score and not self.new_high_score_prompted:
                    ...
                    while True:
                        ...
                        if initials and len(initials) > 0 and initials.strip():
                            ...
                            print(f"Submitted new high score (no valid initials) to online leaderboard: {current_score}")
                            break
                        else:
                            print("Invalid input or cancelled, please try again.")
            self.paused = False
    ...
    def canvas_click(self, event):
        ...
        print(f"Calibration click: zone_count={self.zone_count}, special_hole_defined={self.special_hole_defined}, show_special_option={show_special_option}")
        self.paused = True
        ...
        if points is not None and points.strip():
            try:
                points = int(points)
                if is_special and not self.special_hole_defined and self.zone_count >= TOTAL_ZONES - 1:
                    self.special_hole = (x_frame, y_frame, ZONE_RADIUS, points)
                    self.special_hole_defined = True
                    print(f"Special hole defined at ({x_frame}, {y_frame}) with points {points}")
                    ...
                    self.zone_count += 1
                    print(f"Added regular zone {self.zone_count} at ({x_frame}, {y_frame}) with points {points}")
                    circle_id = self.canvas.create_oval(x - radius, y - radius, x + radius, y + radius, 
                ...
                if len(self.point_zones) < self.zone_count:
                    self.zone_count -= 1
        self.paused = False
    ...
    def update_frame(self):
        ...
        # Get frame from queue (multi-threaded capture)
        if not self.frame_queue.empty():
            self.frame = self.frame_queue.get()

        if self.frame is None:
            print("Failed to read frame from queue.")
            self.root.destroy()
            return
        ...
        if self.calibrating:
            ...
            self.save_button.config(state="disabled")
            # Spawn a new power-up zone if needed
            ...
            # Adjust frame delay for Slow Motion
            if self.power_up and self.power_up.is_active() and self.power_up.power_up_type == "Slow Motion":
                self.frame_delay = 50  # Increase delay to slow down processing
            else:
                self.frame_delay = 10  # Normal delay

            tracked_balls = detect_and_track_balls(self.frame, self.tracker)
            total_balls = len(tracked_balls)
            round_score, scored_positions = calculate_score(tracked_balls, self.point_zones, self.special_hole, self.multiplayer_mode, self.frame.shape[1])
            global current_score, high_score, high_score_initials
            if self.multiplayer_mode:
                self.player_scores["Player 1"] += round_score.get("Player 1", 0)
                self.player_scores["Player 2"] += round_score.get("Player 2", 0)
                self.score_label.config(text=f"Player 1: {self.player_scores['Player 1']} | Player 2: {self.player_scores['Player 2']}")
            else:
                current_score += round_score
                self.score_label.config(text=f"Score: {current_score}")

            if self.multiplayer_mode and max(self.player_scores.values()) > 0:
            ...
            elif round_score > 0 and not self.multiplayer_mode:
                if self.score_sound and self.sound_effects_enabled:
                    self.score_sound.play()
                self.canvas.configure(bg="yellow")
                self.root.after(100, lambda: self.canvas.configure(bg="#2E2E2E"))
            ...
            # Reintroduce power-up activation (simplified to work with white balls)
            power_up_activated = False
            power_up_type = None
            if self.power_ups_enabled and self.power_up_zone and self.power_up_zone.is_active():
                for ball_x, ball_y, _, ball_id in tracked_balls:
                    if ball_id not in scored_ball_ids:  # Only check unscored balls
                        zone_x, zone_y, zone_radius = self.power_up_zone.x, self.power_up_zone.y, self.power_up_zone.radius
                        distance = np.sqrt((ball_x - zone_x)**2 + (ball_y - zone_y)**2)
                        if distance <= zone_radius:
                            scored_ball_ids.add(ball_id)
                            power_up_activated = True
                            power_up_type = random.choice(POWER_UP_TYPES)
                            print(f"Power-up activated: {power_up_type}")
                            self.power_up_zone.deactivate()
                            break

            if power_up_activated and not (self.power_up and self.power_up.is_active()):
                if power_up_type == "Score Multiplier":
                    ...
                    self.timer_label.config(text=f"Time: {self.time_remaining // 60}m{self.time_remaining % 60:02d}s")
                    print(f"Extra Time power-up: Added {POWER_UP_EXTRA_TIME} seconds")
                self.power_up.activate()

            if not self.is_timed_mode:
                if self.multiplayer_mode:
                    winning_score = max(self.player_scores.values())
                    if winning_score > high_score and not self.new_high_score_prompted:
                        self.paused = True
                        self.new_high_score_prompted = True
                        winning_player = max(self.player_scores, key=self.player_scores.get)
                        while True:
                            ...
                            if initials and len(initials) > 0 and initials.strip():
                                initials = initials.upper()[:3]
                                save_high_score(initials, winning_score)
                                print(f"Submitted new high score to online leaderboard: {initials} - {winning_score}")
                                break
                            elif initials == "N/A":
                                save_high_score("N/A", winning_score)
                                print(f"Submitted new high score (no valid initials) to online leaderboard: {winning_score}")
                                break
                            else:
                                print("Invalid input or cancelled, please try again.")
                        self.paused = False
        ...
        # Draw power-up zone if active
        if self.power_ups_enabled and self.power_up_zone and self.power_up_zone.is_active():
            x_frame = self.power_up_zone.x
            y_frame = self.power_up_zone.y
            r = self.power_up_zone.radius
            x_canvas = int(x_frame * (new_width / self.frame.shape[1]) + offset_x)
            y_canvas = int(y_frame * (new_height / self.frame.shape[0]) + offset_y)
            r_canvas = int(r * (new_width / self.frame.shape[1]))
            self.power_up_zone_circle = self.canvas.create_oval(x_canvas - r_canvas, y_canvas - r_canvas, 
                                                                x_canvas + r_canvas, y_canvas + r_canvas, 
                                                                outline="green", width=2)
            self.power_up_zone_text = self.canvas.create_text(x_canvas, y_canvas, text="Power-Up", 
                                                              fill="green", font=("Helvetica", 10))

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            save_high_score(high_score_initials, max(self.player_scores.values()) if self.multiplayer_mode else current_score)
            self.root.destroy()
            return
        elif key == ord('c'):
            self.point_zones = []
