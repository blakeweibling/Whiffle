    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if game_state.drag_start_pos and game_state.zone_editing_action:
            ...
            if action == "move":
                ...
                new_x, new_y, new_w, new_h = zx + dx, zy + dy, zw - dx, zh - dy
            elif action == "resize_tr":
                new_y, new_w, new_h = zy + dy, zw + dx, zh - dy
            elif action == "resize_bl":
                new_x, new_w, new_h = zx + dx, zw - dx, zh + dy
            elif action == "resize_br":
                new_w, new_h = zw + dx, zh + dy
            if action.startswith("resize"):
                prev_w, prev_h = new_w, new_h
                new_w, new_h = max(min_size, new_w), max(min_size, new_h)
                if action == "resize_tl":
                    if new_w != prev_w:
                        new_x = (zx + zw) - new_w
                    if new_h != prev_h:
...
def _process_menu_or_modal_click(x: int, y: int, game_state: GameState) -> bool:
    ...
    if game_state.current_state == CurrentGameState.MENU:
        ...
        pad, size = (
            UIConstants.MENU_CLOSE_BUTTON_PADDING,
            UIConstants.MENU_CLOSE_BUTTON_SIZE,
        )
        ...
        if (
            ...
            # <<< MODIFIED >>> Store click feedback state
            game_state.click_feedback_state = (
                (menu_x + close_btn_rel_x1, menu_y + close_btn_rel_y1, size, size),
                time.time(),
            )
    ...
    for item_data in reversed(game_state.submenu_items):
        ...
        # Determine absolute rectangle based on context (menu vs full screen modal)
        ...
        else:  # CONFIRM_QUIT uses absolute coordinates already stored in submenu_items
        ...
        if (
            ...
            if isinstance(action, str):
                # Handle volume slider adjustments first (as they don't close the menu)
                if game_state.current_state == CurrentGameState.MENU:
                    rel_click_x = click_x_to_check - (
                        menu_x + item_x
                    )  # Calculate relative click for slider
                    if action == "adjust_sound_volume":
                        new_volume = max(
                            0.0, min(1.0, rel_click_x / item_w if item_w > 0 else 0.0)
                        )
                ...
                # Handle other actions (non-slider adjustments)
                    ...
                    if action == "quit":
                        logger.debug(
                            "Menu quit action triggered, entering CONFIRM_QUIT state."
                        )
                    ...
                    if game_state.current_state == CurrentGameState.MENU:
                        if action == "toggle_game_sounds":
                            ...
                            if total_tracks > 0:
                                change_music_track(
                                    game_state,
                                    (game_state.selected_music_track_index + 1)
                                    % total_tracks,
                                )
                                game_state.menu_cache = None
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.players):
                                    if index != game_state.current_player_index:
                                        ...
                                        logger.debug(
                                            "Selected current player, no change."
                                        )
                                else:
                                    logger.warning(
                            ...
                            if len(game_state.players) < 2:
                                game_state.players.append(
                                    Player(f"Player {len(game_state.players) + 1}")
                                )
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    logger.warning(
                                        f"Invalid zone index {index} for edit points action '{action}'"
                                    )
                                    _reset_all_menu_editing_states(game_state)
                            except (ValueError, IndexError) as e:
                                logger.error(
                                    f"Error parsing zone index from action '{action}': {e}"
                                )
                                _reset_all_menu_editing_states(game_state)
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    show_notification(
                                        game_state,
                                        "Click inside zone to move, then drag. ESC to cancel.",
                                        duration=0,
                                    )
                                else:
                                    logger.warning(
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    ...
                                    show_notification(
                                        game_state,
                                        "Click & drag corner handles to resize. ESC to cancel.",
                                        duration=0,
                                    )
                                else:
                                    logger.warning(
                            ...
                            try:
                                ...
                                if 0 <= index < len(game_state.scoring_zones):
                                    if (
                                        ...
                                        show_notification(
                                            game_state, f"Zone {index+1} Deleted"
                                        )
                                        _reset_all_menu_editing_states(game_state)
                                    else:
                    ...
                    return True  # Indicate click was handled by a button action

            # If we get here, the click was on a slider adjustment, handled above
            if volume_adjusted:
                return True  # Indicate click was handled (by slider)

    # If no button/slider was hit
    return False
...
def mouse_callback(event: int, x: int, y: int, flags: int, param: Any) -> None:
    ...
    if (
        ...
        if event in [cv2.EVENT_LBUTTONDOWN, cv2.EVENT_LBUTTONUP]:
            click_handled = True
            return  # Don't process menu clicks if drawing finished
    ...
    if (
        ...
        if click_handled:
            return  # Don't process menu button if item click handled

    # 4. Handle click on the main "Menu" button (only if playing and not drawing)
    if (
        not click_handled
        and game_state.current_state == CurrentGameState.PLAYING
        ...
        if (
            menu_button_rect[0] <= x < menu_button_rect[0] + menu_button_rect[2]
            and menu_button_rect[1] <= y < menu_button_rect[1] + menu_button_rect[3]
        ):
