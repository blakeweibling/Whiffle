...
def draw_menu_window(frame: np.ndarray, game_state: GameState) -> None:
    ...
    if game_state.submenu_active is None:  # Explicitly check for Main Menu
        ...
        logger.debug(
            f"Calculated main menu height: {menu_height_to_use} based on {num_main_items} items."
        )
        ...
        logger.warning(
            f"Submenu '{game_state.submenu_active}' did not set menu_height. Using default: {default_height}"
        )
    ...
    cache_key = tuple(filter(lambda x: x is not None,
                             cache_key_parts))  # Use only non-None parts
    ...
    cache_valid = (
        ...
        and game_state.menu_cache.shape[0] ==
        menu_height_to_use  # Check against calculated height
    ...
    if cache_valid:
        ...
        game_state.menu_height = (
            menu_height_to_use  # Store the height actually used for drawing
        )
