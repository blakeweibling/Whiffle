...
import logging
import time
import numpy as np # Used implicitly by is_ball_at_rest if it uses numpy arrays
from typing import Optional, List, Tuple, Dict, Any

# Import necessary constants and classes from the project
from constants import GameConstants, ScoringConstants, UIConstants
from scoring import is_in_scoring_zone # Assumes this is the correct location
from effects import Explosion, BallTrail
from game_types import CurrentGameState # Import from new location

# Import utility functions it depends on
from game_state_helpers import is_ball_at_rest, is_ball_zone_stable # Import helpers from new file
from game_state_helpers import play_sound, show_notification, save_score # <--- CORRECTED IMPORT
...
def update_scoring(game_state: Any) -> None:
    ...
    state_dicts_to_clean_names = [
        "ball_states", "previous_ball_states", "ball_positions_history",
        "ball_zone_history", "balls_in_zone", "ball_scored_zones",
    ...
    for ball in tracked_balls_list:
        ...
        if not hasattr(game_state, 'ball_positions_history') or not isinstance(game_state.ball_positions_history, dict):
             logger.error("game_state.ball_positions_history missing or not a dict.")
        ...
        if game_state.game_mode == "fun" and hasattr(game_state, "active_trails") and isinstance(game_state.active_trails, dict):
        ...
        for i, z in enumerate(current_scoring_zones):
            try:
                ...
                logger.error(f"Error checking if ball {ball_id} is in zone {i} ({z}): {e}")
                continue
        ...
        if not hasattr(game_state, 'ball_states') or not isinstance(game_state.ball_states, dict):
            logger.error("game_state.ball_states missing or not a dict.")
            continue
        if not hasattr(game_state, 'previous_ball_states') or not isinstance(game_state.previous_ball_states, dict):
        ...
        game_state.ball_states[ball_id] = {
            "at_rest": rest, "stable": stable, "zone": zone, "idx": zone_idx, "time": current_time,
        }

        # --- Scoring Logic ---
        if zone and stable: # Must be in a zone and stable
             # Ensure necessary dicts exist
             if not hasattr(game_state, 'zone_cooldown') or not isinstance(game_state.zone_cooldown, dict):
                  logger.error("game_state.zone_cooldown missing or not a dict.")
                  continue
             if not hasattr(game_state, 'ball_scored_zones') or not isinstance(game_state.ball_scored_zones, dict):
             ...
             if current_time < zone_cooldown_time:
                  if game_state.debug_mode:
                       logger.debug(f"Ball {ball_id} in zone {zone_idx}, but zone is on cooldown.")
             ...
             if game_state.ball_scored_zones.get(ball_id) == zone_idx:
                  if game_state.debug_mode:
                       logger.debug(f"Ball {ball_id} already scored in zone {zone_idx} this entry.")
             ...
             if is_sp:
                  ...
                  if not game_state.special_hole_hit_this_session:
                       ...
                       show_notification(game_state, "Special Hole Hit! Score will double!", duration=3.0)
             ...
             if game_state.game_mode == "survival":
                  ...
                  if game_state.game_timer is not None:
                       ...
                       logger.info(f"Survival Mode: Gained {time_gain:.1f}s. New time: {game_state.game_timer:.1f}s")
                       show_notification(game_state, f"+{time_gain:.0f} Secs!", duration=1.0, is_error=False)
                  else:
                       logger.warning("Attempted to add survival time, but timer is None.")

             # Record score event (ensure dicts exist)
             if hasattr(game_state, 'scored_balls') and isinstance(game_state.scored_balls, list):
                  game_state.scored_balls.append(ball_id) # Potentially legacy
             if hasattr(game_state, 'balls_in_zone') and isinstance(game_state.balls_in_zone, dict):
                  game_state.balls_in_zone[ball_id] = zone
             if hasattr(game_state, 'ball_scored_zones') and isinstance(game_state.ball_scored_zones, dict):
             ...
             # Fun Mode Explosion
             if game_state.game_mode == "fun" and hasattr(game_state, "active_explosions") and isinstance(game_state.active_explosions, list):
                  zone_x, zone_y, zone_w, zone_h, _ = zone
                  explosion_center_x = int(zone_x + zone_w / 2)
                  explosion_center_y = int(zone_y + zone_h / 2)
                  game_state.active_explosions.append(Explosion(explosion_center_x, explosion_center_y))
             ...
             if (
                  ...
                  try:
                       save_score(game_state, game_state.get_current_player().name)
                  except Exception as e:
                       logger.error(f"Error saving score on win condition: {e}")

        # --- Ball Left/Became Unstable After Scoring ---
        elif ball_id in game_state.ball_scored_zones: # Check if ball HAD scored previously
             # Ensure necessary dicts exist before modifying
             if not hasattr(game_state, 'ball_scored_zones') or not isinstance(game_state.ball_scored_zones, dict): continue
             if not hasattr(game_state, 'balls_in_zone') or not isinstance(game_state.balls_in_zone, dict): continue
             if not hasattr(game_state, 'scored_balls') or not isinstance(game_state.scored_balls, list): continue
             ...
             if not stable or zone_idx != last_scored_zone_idx:
                  del game_state.ball_scored_zones[ball_id] # Clear score status for this entry
                  game_state.balls_in_zone.pop(ball_id, None) # Remove from current zone tracking
                  logger.debug(f"Ball {ball_id} left/became unstable in zone {last_scored_zone_idx}. Cleared score status.")
