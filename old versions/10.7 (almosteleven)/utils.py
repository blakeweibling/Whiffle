                ...
                if game_state.current_state == CurrentGameState.MENU:
                    ...
                    if action == "show_splash":
                        ...
                        try:
                            ...
                            logger.error(f"Error parsing zone index from edit action: {action}")
                        ...
                        try:
                            ...
                            logger.error(f"Error parsing zone index from delete action: {action}")
...
def clean_exit(cap: Optional[cv2.VideoCapture], background_music: Optional[pygame.mixer.Sound], background_music_on: bool, game_state: Optional[Any] = None) -> None:
    ...
    if game_state:
        try:
            ...
            try:
                ...
                logger.error("Could not import save_achievements function from game_state_utils.")
            except AttributeError:
                logger.error("save_achievements function not found after import.")
    ...
    raise SystemExit("Exiting Whiffle Tracker.")