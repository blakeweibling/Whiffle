# Whiffle Playfield - Updated with On-Canvas Point Input
# Lines 1-700
import cv2
import numpy as np
import pygame
...
def calibrate_holes():
    ...
    while calibrating:
        ...
        # Draw existing holes with point labels
        ...
        if input_active and current_pos:
            x, y = current_pos
            # Draw input box near the clicked position
            ...
            cv2.circle(frame, (x, y), 20, (0, 255, 0), 2)  # Draw temporary circle

        cv2.imshow('Calibrate Holes', frame)
        key = cv2.waitKey(1) & 0xFF

        # Check for window close (red X)
    ...
    # Save to Supabase before exiting calibration
    if calibrated_holes:
        try:
            requests.patch(
...
def detect_ball_in_hole(image, hole_coords):
    ...
    for (x, y, radius, points) in hole_coords:
        ...
        if white_pixels > 0.5 * (np.pi * radius * radius):
            return (x, y), points  # Return position and points
    return None, None
...
while running and game.running:
    ...
    if roi is not None:
        ...
        if ball_position and points:
            ...
            cv2.circle(roi, ball_position, 20, (0, 255, 0), 2)
