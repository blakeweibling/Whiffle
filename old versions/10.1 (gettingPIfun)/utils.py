...
from constants import UIConstants, GameConstants  # Updated to class-based import
from menu import reset_game, save_zones, clear_zones, flush_scoring_zones  # Added flush_scoring_zones import
...
def _process_calibration_event(event: int, x: int, y: int, game_state: Any, frame: np.ndarray) -> None:
    """Process mouse events for calibrating HSV values for ball detection."""
    if event == cv2.EVENT_LBUTTONDOWN:
        # Ensure the click is within the frame bounds
        if 0 <= x < frame.shape[1] and 0 <= y < frame.shape[0]:
            # Convert the frame to HSV to get the pixel's HSV value
            hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            hsv_value = hsv_frame[y, x]  # HSV value at (x, y)
            game_state.calibration_point = (x, y)
            game_state.calibration_hsv = tuple(hsv_value)  # (H, S, V)
            logger.info(f"Calibration point selected at ({x}, {y}) with HSV {game_state.calibration_hsv}")
        else:
            logger.warning(f"Calibration click at ({x}, {y}) is outside frame bounds")
...
def _handle_menu_close_click(menu_x: int, menu_y: int, game_state: Any) -> bool:
    ...
    logger.debug(f"Checking close button click at menu coordinates ({menu_x}, {menu_y})")
    if (game_state.menu_width - 30 <= menu_x <= game_state.menu_width and 
        0 <= menu_y <= 30):
        game_state.menu_active = False
        game_state.submenu_active = None
        game_state.submenu_items = []  # Fixed typo: removed non-breaking space and corrected variable name
        ...
        if mx <= menu_x <= mx + mw and adjusted_my <= menu_y <= adjusted_my + mh:
            ...
            if label == "File":
                game_state.submenu_items = [
                    ...
                    (10, 100, UIConstants.SUBMENU_WIDTH, UIConstants.SUBMENU_HEIGHT, "Save Zones", lambda: save_zones(game_state.scoring_zones, game_state)),  # Updated
                    (10, 140, UIConstants.SUBMENU_WIDTH, UIConstants.SUBMENU_HEIGHT, "Clear Zones", lambda: clear_zones(game_state.scoring_zones, game_state)),  # Updated
                    (10, 180, UIConstants.SUBMENU_WIDTH, UIConstants.SUBMENU_HEIGHT, "Exit", lambda: clean_exit(game_state.cap, game_state.background_music, game_state.background_music_on, game_state))  # Updated
                ]
            elif label == "Players":
        ...
        if len(item) == 6:  # Non-toggle items
            ...
            if smx <= menu_x <= smx + smw and smy <= menu_y <= smy + smh:
                ...
                if label == "Clear Zones":
                    game_state.scoring_zones = clear_zones(game_state.scoring_zones, game_state)
                    game_state._set_special_hole()  # Update special hole after clearing zones
                else:
                    result = action()
                    if label == "Clear Zones":  # This condition is now redundant but kept for clarity
...
def _process_menu_event(event: int, x: int, y: int, game_state: Any) -> None:
    ...
    logger.debug(f"Processing menu event: {event} at ({x}, {y})")
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        logger.debug(f"Menu coordinates: ({menu_x}, {menu_y})")
...
def mouse_callback(event: int, x: int, y: int, flags: int, game_state: Any) -> None:
    """
    Handle mouse events for drawing scoring zones, calibrating HSV values, or interacting with the main game window.
    """
    logger.debug(f"Mouse event: {event} at ({x}, {y}) with flags {flags}")
    ...
    if game_state.camera_available:
        ...
        if not ret:
            logger.error("Camera read failed during mouse event, using static frame")
    ...
    if game_state.drawing_mode:
        ...
        _process_calibration_event(event, x, y, game_state, frame)
    else:
        if event == cv2.EVENT_LBUTTONDOWN:
...
def clean_exit(
    ...
    Args:
        ...
        game_state: The current game state to flush caches (Change 4).
    """
    logger.info("Cleaning up and exiting game...")
    # Flush leaderboard and scoring zones caches (Change 4)
    if game_state:
        game_state.leaderboard.flush_pending_scores()
        flush_scoring_zones(game_state)
