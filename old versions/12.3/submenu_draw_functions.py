...
import numpy as np
import logging
from typing import Any, Callable, Tuple  # <<< Added Tuple
from math import ceil

# <<< MODIFIED: Import Slider constants >>>
...
from game_state import (
    CurrentGameState,
    GameState,  # Import GameState for type hinting
)

# Import other submenu draw functions if they were separate (assuming they are for now)
# from .other_submenus import _draw_leaderboard_submenu, ... # Example if split further

logger = logging.getLogger(__name__)


# --- REMOVED ToggleItem Class ---
# class ToggleItem: ... # Removed


# --- NEW: Helper to Draw Volume Slider with Mute Toggle ---
def _draw_volume_slider(
    menu_frame: np.ndarray,
    y_pos: int,
    label: str,
    current_volume: float,  # Value 0.0 to 1.0
    is_on: bool,  # Corresponds to game_sounds_on or background_music_on
    toggle_action: str,  # Action string for the mute toggle button
    slider_action: str,  # Action string for clicking/dragging the slider bar
    game_state: GameState,
) -> int:
    """
    Draws a label, a volume slider (bar and handle), percentage text,
    and a mute toggle button. Adds clickable areas to game_state.submenu_items.

    Returns:
        The height used by this control element.
    """
    item_height = UIConstants.SLIDER_HEIGHT  # Use slider height for spacing
    slider_width = UIConstants.SLIDER_WIDTH
    slider_height = UIConstants.SLIDER_HEIGHT
    handle_width = UIConstants.SLIDER_HANDLE_WIDTH
    padding = UIConstants.SLIDER_PADDING
    button_width = 60  # Width for the Mute/Unmute button
    spacing = 15  # Spacing between elements

    # 1. Draw Label
    label_text = f"{label}:"
    label_x = 20
    label_y = y_pos + slider_height // 2 + 5  # Vertically center label roughly
    cv2.putText(
        menu_frame,
        label_text,
        (label_x, label_y),
    ...
    # Add slider bar clickable area
    # Note: Click handling logic will be added in utils.py later
    ...
    # 3. Draw Slider Handle
    # Calculate handle position (ensure it stays within the bar)
    ...
    # Ensure minimum handle width is visible even at 0% or 100%
    ...
    text_x = current_x
    text_y = y_pos + slider_height // 2 + 5  # Same y as label
    cv2.putText(
        menu_frame,
        percent_text,
        (text_x, text_y),
    ...
    _draw_button(
        ...
        mute_button_y,
        button_width,
        slider_height,  # Match slider height
        mute_text,
        mute_color,
        UIConstants.FONT_SCALE_SMALL,
    )
    # Add mute toggle clickable area
    ...
    # Return the vertical space used
    return slider_height + 10  # Height + spacing


# --- MODIFIED: Settings Submenu uses Sliders ---
def _draw_settings_submenu(menu_frame: np.ndarray, game_state: GameState) -> None:
    ...
    logger.debug("Executing _draw_settings_submenu")
    cv2.putText(
        menu_frame,
        "Settings",
        (30, 40),
    ...
    y_offset = 80
    game_state.submenu_items.clear()  # Clear items before drawing controls

    # Draw Volume Sliders using the helper
    y_offset += _draw_volume_slider(
        ...
        "toggle_game_sounds",  # Action for the mute button
        "adjust_sound_volume",  # Action for the slider bar
        game_state,
    )
    y_offset += _draw_volume_slider(
        menu_frame,
        ...
        "toggle_background_music",  # Action for the mute button
        "adjust_music_volume",  # Action for the slider bar
        game_state,
    )

    # --- Draw Music Track Selection (No Change Needed Here) ---
    # (Ensure y_offset is correctly updated from sliders above)
    button_width = 150  # Width for Track button
    item_height = 35  # Height for Track button and other toggles below
    ...
    _draw_button(
        menu_frame,
        track_button_x,
        y_offset,  # Use current y_offset
        button_width,
        item_height,  # Use standard item height
        track_display_text,
    ...
    y_offset += item_height + 5  # Move past track selection

    # --- Draw Original Toggles for Debug Options ---
    debug_toggles = [
        (
    ...
    toggle_button_width = 80  # Standard ON/OFF button width
    ...
    for label, get_state, action in debug_toggles:
        ...
        cv2.putText(
            menu_frame,
            label_text,
            (20, y_offset + 25),  # Adjust y for vertical center
            cv2.FONT_HERSHEY_SIMPLEX,
            UIConstants.FONT_SCALE_MEDIUM,
            UIConstants.WHITE,
            1,
        ...
        y_offset += item_height + 5  # Move past this toggle
...
# --- Other Submenu Drawing Functions (Assumed Unchanged) ---
# Keep the implementations for leaderboard, players, achievements, etc. as they were,
# unless they need adjustments based on other changes. Ensure they correctly use
# game_state attributes and UIConstants.


def _draw_leaderboard_submenu(menu_frame: np.ndarray, game_state: GameState) -> None:
    """Draw the Leaderboard submenu."""
    # (Content unchanged from previous version - including survival mode display)
    cv2.putText(
        menu_frame,
        "Leaderboard",
        (30, 40),
    ...
    # Mode Buttons (Classic, Timed, Survival)
...
def _draw_players_submenu(menu_frame: np.ndarray, game_state: GameState) -> None:
    ...
    for i, player in enumerate(game_state.players):
        ...
        max_name_len = name_width // 8  # Estimate
        cv2.putText(
            menu_frame,
...
def _draw_edit_zones_submenu(menu_frame: np.ndarray, game_state: GameState) -> None:
    ...
    list_width = (
        menu_frame.shape[1] - 40 - actions_width - button_spacing
    )  # Width for zone label
    game_state.submenu_items.clear()
