...
def reset_game(game_state: GameState) -> None:  # Added GameState type hint
    ...
    if game_state.players and 0 <= game_state.current_player_index < len(
        ...
        logger.warning(
            "Player index out of bounds or no players during reset.")
    ...
    logger.info(
        f"Game state reset for player: {game_state.get_current_player().name}")
    ...
    if os.path.exists(GameConstants.ZONES_FILE):
        try:
            if os.path.getsize(GameConstants.ZONES_FILE) == 0:
                logger.warning(
                    f"{GameConstants.ZONES_FILE} is empty. Clearing zones.")
            ...
            if isinstance(loaded_data, list) and all(
                ...
                logger.info(
                    f"Scoring zones loaded from {GameConstants.ZONES_FILE}")
                ...
                game_state.show_notification(
                    "Invalid Zone File Format", is_error=True)
                game_state.scoring_zones = []
                game_state.special_hole = None
        ...
        logger.warning(
            f"Scoring zones file '{GameConstants.ZONES_FILE}' not found.")
        game_state.scoring_zones = []
        game_state.special_hole = None
...
def draw_menu_window(
    ...
    if not game_state.submenu_active:
        game_state.menu_height = 60 + \
            len(MenuConstants.MAIN_MENU_ITEMS) * 40 + 20
    else:
    ...
    if roi_h > 0 and roi_w > 0 and menu_end_y <= menu_h and menu_end_x <= menu_w:
        try:
            ...
            if roi.shape == menu_frame_slice.shape:
                alpha = 0.85
                cv2.addWeighted(menu_frame_slice, alpha,
                                roi, 1.0 - alpha, 0, roi)
            else:
                logger.warning(
        ...
        logger.debug(
            "Menu overlay ROI has zero or negative size, skipping overlay.")
    else:
        logger.warning(
