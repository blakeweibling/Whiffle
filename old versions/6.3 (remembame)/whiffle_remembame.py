# Whiffle Playfield - Fixed Ball ID for Stable Detection
# Lines 1-700
import cv2
import numpy as np
import pygame
...
class GameState:
    ...
    def get_nearest_hole(self, pos):
        ...
        for (x, y, radius, points) in self.hole_positions:
            ...
            if dist < min_dist and dist <= radius * 1.5:  # Increased radius check to 1.5x for flexibility
                min_dist = dist
...
def detect_ball_in_hole(image, hole_coords, game_state, frame_count):
    ...
    for contour in contours:
        ...
        print(f"Contour area: {area}")  # Debug: Check contour areas
        if 0.05 * np.pi * 20 * 20 < area < np.pi * 20 * 20:  # Relaxed lower bound from 0.1 to 0.05
            ...
            if len(approx) > 4:
                ...
                if M["m00"] != 0:
                    ...
                    print(f"Detected ball at {ball_pos}, area: {area}")  # Debug: Log detected balls

                    # Generate a stable ball_id based on rounded position
                    rounded_x = round(ball_x / 10) * 10  # Round to nearest 10 pixels
                    ...
                    if nearest_hole:
                        ...
                        print(f"Nearest hole at {hole_pos}, points: {points}")  # Debug: Log nearest hole
                        ...
                        if ball_id in game_state.confirming_balls:
                            ...
                            frames = current名前_data["frames"]

                            # Check if the ball has moved significantly
                            dist = np.hypot(ball_x - current_pos[0], ball_y - current_pos[1])
                            print(f"Ball ID {ball_id}: Distance moved: {dist}, Frames: {frames}")  # Debug: Log movement and frames
                            if dist < 15:  # Increased tolerance to 15 pixels
                                ...
                                print(f"Resetting confirmation for Ball ID {ball_id} due to movement: {dist}")
...
while running and game.running:
    ...
    if not ret:
        ...
        retry_count += 1
        if retry_count >= max_retries:
            print("Max retries reached for frame capture. Reinitialize camera...")
            if not reinitialize_camera():
    ...
    if frame is None or frame.size == 0 or frame.shape != (HEIGHT, WIDTH, 3) or frame.mean() < 1:
        ...
        if frame_count % 30 == 0:  # Ensure this runs only every 30 frames
    ...
    frame_count += 1  # Ensure frame_count increments
    if frame_count % 30 == 0:
